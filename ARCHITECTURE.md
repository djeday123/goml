...
**Единственное законное исключение из read-only — безопасность.** Memory-safety фиксы (OOB read/write, use-after-free, race data-corruption) разрешены — но по одному, с обоснованием, отдельным коммитом. Прецедент: B-impl-1 доработка (2026-07-23) вскрыла OOB read в `BatchedMatMulF32` при broadcast shapeB (одна логическая правка: параметр `broadcastB bool` → `strideB=0`).
