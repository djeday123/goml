#!/usr/bin/env python3
"""
Benchmark: Tri Dao FA2 vs GoML v7 on identical configs.
...
# Build GoML .so
so_path = "libs/flash_attention_v7.so"
if not os.path.exists(so_path):
    os.system("nvcc -O3 -arch=sm_89 -std=c++17 --shared -Xcompiler -fPIC libs/flash_attention_v7.cu -o " + so_path)

goml_lib = ctypes.CDLL(so_path)
goml_lib.flash_attention_v7_forward.restype = ctypes.c_int
goml_lib.flash_attention_v7_forward.argtypes = [
...
# GoML benchmark
def run_goml():
    ret = goml_lib.flash_attention_v7_forward(
...
def main():
    ...
    print(f"=== FA2 vs GoML v7 — Head-to-Head ===")
