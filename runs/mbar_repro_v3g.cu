                      ...
                      int seq_len, int head_dim, int causal, int window)
{
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        if (!is_producer) {
            asm volatile("bar.sync 7, 96;");      // [SYNC L3] v3g: id 1→7 (far id)
        }
...
int main()
{
    // v3g: minimal — only sl=300 ca=0 wnd=0 (trigger config), single run, return rc
    setvbuf(stdout, NULL, _IONBF, 0);
    int rc = run_one(1, 300, 0, 0, 5.0);
    printf("sl=300 ca=0 wnd=0: %s\n", rc==0 ? "OK" : "HANG");
    return rc == 0 ? 0 : 1;
}
