...
TS=$(date +%s)
LOG="$RUNS/a0_speed_${TS}.log"
setsid nohup go test -run TestA0_Battle_Speed -timeout 30m -v ./internal/abjexam/ \
    > "$LOG" 2>&1 < /dev/null &
PID=$!
echo $PID > /tmp/a0_speed.pid
echo "$LOG" > /tmp/a0_speed.log_path
sleep 2
echo "PID: $PID"
echo "LOG: $LOG"
