// mbar_repro_v3: add ALL __syncthreads / bar.sync from v111-mbarrier in exact order.
// Step from M2 → M3:
//   * __syncthreads after mbar_init (v111 line 374) — already in v2
//   * __syncthreads after Q-load-prelude (v111 line 385) — NEW
//   * __syncthreads after mbar_wait (v111 line 464) — already in v2
//   * __syncthreads after transpose_v position (v111 line 470) — already in v2
//   * bar.sync 1, 96 consumer-only after QK+softmax (v111 line 722) — NEW
//   * bar.sync 2, 96 consumer-only after smP STS (v111 line 747) — NEW
// Work between syncs simulated by __nanosleep.
// NO MMA, NO softmax, NO transpose.
                      ...
                      int seq_len, int head_dim, int causal, int window)
{
    ...
    // [Q-load prelude simulation]
    // v111 lines 382-385: load_tile_fp8(smQ, ...); cpa_commit(); cpa_wait<0>(); __syncthreads();
    // No Q-load here, but match the structural sync placement.
    if (wid == 0) {
        for (int i = 0; i < 10; i++) __nanosleep(50);
    }
    __syncthreads();                              // [SYNC P2] v111 line 385 — post Q-prelude
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v111 line 747: bar.sync 2, 96 consumer-only after smP STS
        if (!is_producer) {
            asm volatile("bar.sync 2, 96;");      // [SYNC L4]
        }
...
int main()
{
    ...
    printf("Sync mix: 2 prologue __syncthreads + 2 loop __syncthreads + 2 bar.sync (consumer-only)\n\n");
