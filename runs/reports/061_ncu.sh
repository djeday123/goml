...
LOG=/data/lib/podman-data/projects/goml/runs/reports/061_ncu_data.txt
> "$LOG"
...
for label in BASE CAND; do
    ...
    echo "===== $label kernel_dk_new =====" | tee -a "$LOG"
    "$NCU" --kernel-name kernel_dk_new --launch-count 1 --metrics "$STALLS" "$BIN" 2>&1 | tail -14 | tee -a "$LOG"
    "$NCU" --kernel-name kernel_dk_new --launch-count 1 --metrics "$LSU" "$BIN" 2>&1 | tail -12 | tee -a "$LOG"
done
