...
- GPU Фаза II, prefill через FA (ядро ждёт равные BH у Q и KV): **repeat-kv
  транзиентно на слой**. Числа: развёрнутый KV слоя при S=2048 F32 =
  2×32×2048×64×4B = **67.1 MB** (буфер переиспользуется между слоями, ×22 НЕ
  умножается). Хранение кэша всегда в H_kv=4: 88.0 MiB против 704 MiB
