...
| **FA-F16-вход (Q/K/V half вместо e4m3)** — обоснование: индустриальный warmup = сотни-тысячи шагов, 938ms F32-recon на шаг непозволительны как боевой путь (F32-recon легален только как тестовая проводка warmup-звена). Математика ядер сохраняется, меняется входной тракт (2x трафик/SMEM) + HMMA вместо QMMA; на sm_120a FP16-пик = FP8-пику. Прогноз скорости: между 127T (v58-класс) и 260T. Первый аргумент карточки — вилка скейлинга из A-LLM-4 п.2.4b. Второй аргумент (Н4, 2026-08-03): F16-вход не имеет квантизационного контракта вообще — нет e4m3-упаковки, нет магнитудной ловушки FP16-акков от decoded ±448, нет amax-инфраструктуры в стыке; плюс к простоте/надёжности боевого warmup-пути. Возврат в fa-blackwell-fp8 с полной NCu-дисциплиной | ТЗ A-LLM-4 (ревью 2026-08-03) + Н4 `c319212` | MID — после warmup-теста, перед индустриализацией |
...
## 2026-08-03 (`fffe6ea`) — F1-HF Фаза I: hf-загрузка + CPU-F64 инференс-линейка, все гейты зелёные

Карточка HF-INFERENCE, Фаза I (paper-first, GPU не тронут ни разу). Модель: TinyLlama-1.1B-Chat-v1.0 (hd=64 ✓, BF16 2.2GB, Apache-2.0, GQA 32/4; md5 весов 59e1605b3af5f1673eb8396251d6bc46). Деливераблы: пакет `hf/` (safetensors-парсер c полной валидацией раскладки; config-загрузчик с guard-отказами rope_scaling/bias/tie; токенизатор sentencepiece-BPE из tokenizer.json — вердикт I.3 «новый загрузчик», наш BPE несовместим по формату/раскладке/алгоритму) + `fwdLlamaHFF64` (расширение fwdBattleAF64-примитивов: GQA-индексация kvh=h/G, SwiGLU, causal префикс-softmax — чисто CPU). Гейты (все с первого прогона): I.2 — 201/201 тензоров shapes/dtype/конечность/md5; I.3 — 100/100 encode==HF, 100/100 decode==HF (реф tokenizers 0.21.4, зафиксирован); I.5 — top-10 логитов множество+порядок 10/10, greedy «Paris» (прогноз подтверждён), **worst rel 4.279e-07 при floor 1e-3** (floor'ы записаны до прогона, F1_predictions.md; оба прогноза-класса — точность ~1e-4 и время 20-120s — промахнулись в консервативную сторону: 4.3e-7 и 3.7s, разбор в бумаге §6). Ключевые подтверждения: ropeF64 half-split == HF rotate_half бит-в-бит по конвенции; HF-layout [out,in] ложится на matF64_ABT без транспонирования; BF16→F32 битово точен. Долги Фазы II в реестре бумаги (F2-1..F2-7; HIGH: causal-серт FA, GPU-RoPE posOffset). Отчёт: `F1_HF_INFERENCE_recon.md`, raw: `raw_f1hf/`, прогнозы: `runs/hf_inference/F1_predictions.md`. **Процедурная сноска (Б-0):** артефакты сессии вошли в коммит `fffe6ea` (session-close A-LLM-5 параллельной дуги — широкий git add в общем дереве подобрал стейдж этой сессии); содержимое сверено с финальными версиями побайтово (diff HEAD пуст), авторство разделов зафиксировано этим параграфом. Фаза II — только после ревью бумаги И поверх зелёного П.6 A-LLM-5 (квант-контракт CLOSED — триггер-условие выполнено дугой). СТОП.

## 2026-08-03 (A-LLM-6, кодовый коммит сессии) — хвосты-фантомы закрыты, Этап 2 красный смоук

П.1а-проба: slice-канал ЖИВ (3/3 операции Δ=0) — «мёртвый plain-канал» A-LLM-5 был ложноположительным выводом (совпадение worst при FA-amax-нулях); recon dQ/dK живы (zero=0/1M), гейт П.5 пройден, SECONDARY здоров. П.2а-репро: 0/10 (прогноз ≥1/10 — гонку добил Н3-Sync A-LLM-5; цикл — постоянная проба). П.3 faScale-FP16-normal в камне. П.5 канарейка перецентрована [652,656] (якорь 654.22T). Этап 2: снапшот-обвязка написана (v1-упрощения удалены), смоук 4б КРАСНЫЙ 7/10 — DEmbed 18.7 блоу-ап при идеальных DWout/DWq (систематика dX-цепочки) + рецидив amax=0 в новом паттерне; стоп-линия, траектория/скорость не гонялись, тесты за GOML_STAGE2-гейтом. Регрессия зелёная. Отчёт A_LLM6_stage2.md. A-LLM-7 = позвенная D-hop диагностика.
