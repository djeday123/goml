...
git log -1
# ожидается subject: "A-LLM-5 session close: отчет + хроника + HANDOFF (контракт в камне)"
# это doc-only коммит; цепочка вниз: c4a8c2e (КОДОВЫЙ, A-LLM-5 квант-контракт) ->
# a530ad9 -> b98bc89 -> c319212 (A-LLM-4) -> ... Пары: gotorch 927c795,
# fa-blackwell-fp8 7d2e9a0.
# проверка: git log -6 --oneline должен содержать c4a8c2e и a530ad9
...
## ЗАДАНИЕ СЛЕДУЮЩЕЙ FRESH-СЕССИИ: A-LLM-6 (скелет; детальное ТЗ выдаёт ревьюер)

1. Хвост-б (первым): расследовать slice-канал (подозрение ToDevice(CPU0,
   sliceStore) -> нули); оживить SECONDARY; гейт П.5 (recon dQ/dK vs
   g64.D{Q,K}PermAttn живые).
2. Хвост-а: добить amax-гонку standalone (кандидат: uploadInto-fallback Copy
   на непокрытом Sync'ом stream); контракт-проба на повторяемость amax 5x.
3. Этап 2 по рамкам ТЗ A-LLM-4 (2.1-2.5): боевой bwdBattleA до зеркала,
   case AttnBwdFA, траектория 20 шагов (плато = правильность, ножницы числом),
   скорость 30-run CV<1% с вилкой-прогнозом, пятая карта блоков, стоп-линия та же.
4. Перецентровка канарейки-скрипта (652+-2 -> фактический медиан-дрейф 653.7-654.0)
   отдельным решением Вугара.
5. Опция-Б (decoded<=4, запас 32x) — после зелёного Этапа 2.
