#!/bin/bash
# 052 §e NCu-post: base (040) vs cand (052 smQ prefetch), merged isolated
export HOME=/tmp
...
LOG=/data/lib/podman-data/projects/goml/runs/reports/052_ncu_data.txt
> "$LOG"
...
LSU="l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_ld.sum,l1tex__data_bank_conflicts_pipe_lsu_mem_shared_op_st.sum,l1tex__data_pipe_lsu_wavefronts_mem_shared.sum,dram__bytes.sum,sm__warps_active.avg.pct_of_peak_sustained_active,launch__block_size.avg,launch__registers_per_thread"

for label in BASE CAND; do
    if [ "$label" = "BASE" ]; then BIN=$BASE; else BIN=$CAND; fi
    echo "===== $label ($BIN) =====" | tee -a "$LOG"
    echo "-- stalls kernel_merged_v1 --" | tee -a "$LOG"
