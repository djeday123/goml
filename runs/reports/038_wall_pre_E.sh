#!/bin/bash
# 038 pre-E baseline wall (isolated, той же сессии)
...
LOG=/data/lib/podman-data/projects/goml/runs/reports/038_wall_pre_E_data.txt
> "$LOG"
"$GATE" >> "$LOG" 2>&1 || { echo "gate failed"; exit 1; }
echo "=== WARMUP (4 runs, discarded) ==="
for i in 1 2 3 4; do "$BIN" 2>&1 | grep avg_ms > /dev/null; done
echo "=== PRE-E MEASURED (5 runs, isolated, no NCu) ===" | tee -a "$LOG"
