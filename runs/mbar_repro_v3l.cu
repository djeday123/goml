// mbar_repro_v3l: v3e + фикс из M7 — expected_phase = {0, 0}.
// На sm_120a SYNCS.PHASECHK реализует «pass if current_parity != arg».
// Свежеинициализированный bar имеет parity=0, поэтому первый wait должен
// быть с arg=0 (тогда блокирует до flip parity→1 после 32 arrives).
// XOR-flip после каждого wait сохраняет инвариант:
//   slot=0: 0,1,0,1,...  slot=1: 0,1,0,1,...
                      ...
                      int seq_len, int head_dim, int causal, int window)
{
    ...
    uint32_t expected_phase[K_STAGES] = {0, 0};  // M7 fix: parity конвенция sm_90+
...
int main()
{
    ...
    printf("v3l: M7 fix — expected_phase = {0, 0}\n\n");

    setvbuf(stdout, NULL, _IONBF, 0);
    printf("=== Phase 1: Matrix sl x wnd (9 configs) ===\n");
    ...
    printf("=== Phase 2: sl=300 ca=0 wnd=0 x 100 runs ===\n");
    flush_stdout();
    int n_hang_p2 = 0, n_ok_p2 = 0;
    for (int i = 0; i < 100; i++) {
        int r = run_one(bh, 300, 0, 0, timeout);
        if (r == -1) n_hang_p2++; else n_ok_p2++;
        if (((i+1) % 10) == 0 || r == -1) {
            printf("  run %3d/100: %s (OK=%d HANG=%d)\n",
    ...
    printf("=== Phase 3: sl=300 ca=1 wnd=96 x 100 runs ===\n");
    flush_stdout();
    int n_hang_p3 = 0, n_ok_p3 = 0;
    for (int i = 0; i < 100; i++) {
        int r = run_one(bh, 300, 1, 96, timeout);
        if (r == -1) n_hang_p3++; else n_ok_p3++;
        if (((i+1) % 10) == 0 || r == -1) {
            printf("  run %3d/100: %s (OK=%d HANG=%d)\n",
                   i+1, (r==0)?"OK":"HANG", n_ok_p3, n_hang_p3);
            flush_stdout();
        }
    }
    printf("\nPhase 3 summary: %d OK, %d HANG out of 100 runs\n",
           n_ok_p3, n_hang_p3);

    int total_hang = n_hang_p1 + n_hang_p2 + n_hang_p3;
    printf("\n=== TOTAL: %d HANG / %d runs ===\n",
           total_hang, 9 + 100 + 100);
