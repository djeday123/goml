// mbar_repro_v3h: v3e + canonical sm_90+ mbarrier forms:
//   .shared::cta qualifier on all three (init/arrive/test_wait)
//   .release.cta on arrive (producer side)
//   .acquire.cta on test_wait (consumer side)
// Hypothesis: relaxed memory ordering on the deprecated .shared.b64 form lets
// parity flip propagation stall indefinitely on sm_120 SYNCS engine,
// causing consumers' test_wait spin to never observe the producer arrive.
...
int main()
{
    ...
    printf("v3h: canonical sm_90+ mbarrier forms (.shared::cta + .release.cta / .acquire.cta)\n\n");
