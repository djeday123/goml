...
TS=$(date +%s)
LOG="$RUNS/a1_accuracy_${TS}.log"
setsid nohup go test -run 'TestA1_CE_Correctness_Battle|TestA1_Battle_Accuracy' -timeout 60m -v ./internal/abjexam/ \
    > "$LOG" 2>&1 < /dev/null &
PID=$!
echo $PID > /tmp/a1_accuracy.pid
echo "$LOG" > /tmp/a1_accuracy.log_path
sleep 2
