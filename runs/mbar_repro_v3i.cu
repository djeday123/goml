// mbar_repro_v3i: v3h + replace test_wait hot-spin with try_wait.parity.acquire.
// PTX try_wait parks the warp (HW timeout, ~10us by default) instead of
// hot-spinning the SYNCS.PHASECHK loop. Hypothesis: scheduler-starvation of
// the spin loop prevents the producer warp from completing its
// arrive→flip→propagate sequence; parking via try_wait yields scheduling
// cycles to the producer.
...
// v3i: try_wait.parity parks the warp (HW timeout-based) instead of hot-spin
...
int main()
{
    ...
    printf("v3i: try_wait.parity.acquire (warp-park HW timeout) instead of test_wait spin\n\n");
