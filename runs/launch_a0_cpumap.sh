...
TS=$(date +%s)
LOG="$RUNS/a0_cpumap_${TS}.log"
setsid nohup go test -run TestA0_Battle_CPUMap -timeout 15m -v ./internal/abjexam/ \
    > "$LOG" 2>&1 < /dev/null &
PID=$!
echo $PID > /tmp/a0_cpumap.pid
echo "$LOG" > /tmp/a0_cpumap.log_path
sleep 2
echo "PID: $PID"
echo "LOG: $LOG"
