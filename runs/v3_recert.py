#!/usr/bin/env python3
"""V3 RE-CERT: 30-run same-thermal for E2E-nc/c + isolated dQ/dK/dV.
   Uses V2 fingerprint gate — each run's first line reports numRegs so log-diff
   proves *which* build produced the numbers.
   Per-run JSON checkpoint (survives interruption).
"""
...
# Expected fingerprints per V2 gate — replay proof.
EXPECT = {
    "kernel_dq":         196,
    "kernel_dk":         248,
...
CONFIGS = [
    ("v3_e2e_nc", [f"{LIBS}/bench_e2e", "128","8192","0","0","5","20"], "weighted"),
    ("v3_e2e_c",  [f"{LIBS}/bench_e2e", "128","8192","1","0","5","20"], "weighted"),
    ("v3_dq_iso", [f"{LIBS}/bench_dq",  "128","8192","0","0","5","20"], "iso"),
    ("v3_dk_iso", [f"{LIBS}/bench_dk",  "128","8192","0","0","5","20"], "iso"),
    ("v3_dv_iso", [f"{LIBS}/bench_dv",  "128","8192","0","0","5","20"], "iso"),
]

def check_fingerprint(out):
    """Verify each FINGERPRINT line matches EXPECT; return None if OK, else msg."""
...
def main():
    print("V3 RE-CERT — 30-run per config, fingerprint-gated", flush=True)
