    ...
    float scale)
{
    ...
    for (int qt = qt_start; qt < n_qt; ++qt) {
        ...
        // 036-r variant 0: byte-scatter Q_T (без pack, без π_V) — 64 STS.U8/lane/qt
        //   Восстанавливает pre-018 sealed byte scatter.
        #pragma unroll
        for (int ks = 0; ks < KS_QK; ++ks) {
            int m_lo_s = wid * 16 + l_div4 + 0;
            int m_hi_s = wid * 16 + l_div4 + 8;
            int k_lo_s = ks * 32 + l_mod4 * 4 + 0;
            int k_hi_s = ks * 32 + l_mod4 * 4 + 16;
            #pragma unroll
            for (int bt = 0; bt < 4; ++bt) {
                smQ_T[(k_lo_s + bt) * QT_STRIDE + m_lo_s] = (Qr[ks][0] >> (bt * 8)) & 0xFF;
                smQ_T[(k_lo_s + bt) * QT_STRIDE + m_hi_s] = (Qr[ks][1] >> (bt * 8)) & 0xFF;
                smQ_T[(k_hi_s + bt) * QT_STRIDE + m_lo_s] = (Qr[ks][2] >> (bt * 8)) & 0xFF;
                smQ_T[(k_hi_s + bt) * QT_STRIDE + m_hi_s] = (Qr[ks][3] >> (bt * 8)) & 0xFF;
            }
        }
        __syncthreads();
        ...
        for (int kb = 0; kb < KB_DK; ++kb) {
            ...
            for (int ni = 0; ni < NI_DK; ++ni) {
                int n_d = ni * 8 + l_div4;
                // 036 variant B: π_V СНЯТА (identity — mirror snятой Phase D)
