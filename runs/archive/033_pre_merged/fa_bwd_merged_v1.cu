    ...
    for (int qt = qt_start; qt < n_qt; ++qt) {
        ...
        for (int np = 0; np < NI_DP / 2; ++np) {
            ...
            *reinterpret_cast<uint16_t*>(&smdS_stage[i_local_hi * SMDS_STAGE_STRIDE + jb_lo]) = dsb_hi_fp8;
            ...
            // dS_T path — STS.b8 per byte to smdS_T_stage (006-II)
            uint8_t bal = dsa_lo_fp8 & 0xFF;
            uint8_t bbl = dsb_lo_fp8 & 0xFF;
            uint8_t bah = (dsa_lo_fp8 >> 8) & 0xFF;
            uint8_t bbh = (dsb_lo_fp8 >> 8) & 0xFF;
            uint8_t cal = dsa_hi_fp8 & 0xFF;
            uint8_t cbl = dsb_hi_fp8 & 0xFF;
            uint8_t cah = (dsa_hi_fp8 >> 8) & 0xFF;
            uint8_t cbh = (dsb_hi_fp8 >> 8) & 0xFF;
        ...
        {
            uint8_t *dS_T_b = dS_T_out + (size_t)b * sl * stride_ds;
            constexpr int CHUNK = 16;
            ...
            for (int c = tid; c < total; c += FA_M_THREADS) {
                ...
                if (j_g < sl && i_start < stride_ds) {
                    ...
                    *reinterpret_cast<uint4*>(&dS_T_b[(size_t)j_g * stride_ds + i_start]) = chunk;
                }
            }
        }
