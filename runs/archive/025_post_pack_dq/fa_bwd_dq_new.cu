    ...
    float scale)
{
    ...
    for (int kt = 0; kt < kt_end; ++kt) {
        ...
        cpa_commit();
        cpa_wait<0>();
        __syncthreads();   // BARRIER #1: smK_area (K natural swizzled) + smdS ready
        ...
        //   ks = wid (per-warp), read swizzled smK into kr_lo/kr_hi, then write natural K_T (aliased overwrite).
        {
            ...
            //   ЗАПРЕТ V[] array — V0-V3 named регистры (LDL/STL детектор в гейтах)
            {
                ...
                for (int slot = 0; slot < 4; ++slot) {
                    ...
                    // Phase D — 4 STS.32 (dq K_T mapping, unit-test 8192/8192)
                    const int base_row = ks_local * 32 + 4 * c + p + 16 * slot_half;
                    *reinterpret_cast<uint32_t*>(&smK_area[base_row * KT_STRIDE + (ni_base + 0) * 8 + 4 * h]) = OUT0;
                    *reinterpret_cast<uint32_t*>(&smK_area[base_row * KT_STRIDE + (ni_base + 1) * 8 + 4 * h]) = OUT1;
                    *reinterpret_cast<uint32_t*>(&smK_area[base_row * KT_STRIDE + (ni_base + 2) * 8 + 4 * h]) = OUT2;
                    *reinterpret_cast<uint32_t*>(&smK_area[base_row * KT_STRIDE + (ni_base + 3) * 8 + 4 * h]) = OUT3;
                }
            }
        }
        __syncthreads();   // BARRIER #3: smK_T ready for MMA-C
        ...
        for (int kb = 0; kb < KB_DQ; ++kb) {
            ...
            for (int ni = 0; ni < NI_DQ; ++ni) {
                int n_d = ni * 8 + l_div4;
                uint32_t B0 = *reinterpret_cast<uint32_t*>(&smK_area[n_d * KT_STRIDE + k_j_lo]);
                uint32_t B1 = *reinterpret_cast<uint32_t*>(&smK_area[n_d * KT_STRIDE + k_j_hi]);
