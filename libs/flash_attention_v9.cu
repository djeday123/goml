// =============================================================================
// FlashAttention v9 — Persistent Kernel + Dynamic Tile Scheduling
// =============================================================================
// Changes vs v7 (109 TFLOPS corrected):
//   Persistent kernel: launch num_SMs * occupancy blocks (128*2 = 256)
//   Each block loops, atomically grabs next (bh, q_tile) work unit
//   Blocks that finish early (fewer causal KV tiles) immediately get more work
//
//   Before: block for q_tile=0 does 1 KV iter, q_tile=63 does 64 KV iters
//   ~50% of SMs idle while last blocks finish their long loops
//
//   After: all SMs stay busy — fast blocks pick up more q_tiles
//
// Everything else identical to v7 (ldmatrix, swizzle, optimized softmax).
// =============================================================================
...
#define FA9_BR 64
#define FA9_BC 64
#define FA9_THREADS 256

#define FA9_Q_STRIDE 128
#define FA9_KV_STRIDE 128
#define FA9_S_STRIDE 72
    ...
    const __half *smem, int stride, int n_base, int k_base, int lane)
{
    ...
    int physical_n = swizzle_col(k, n_base);
    ...
    int tile_start, int seq_len, int head_dim)
{
    ...
    constexpr int TOTAL_CHUNKS = FA9_BC * CHUNKS_PER_ROW;

#pragma unroll 4
    for (int c = threadIdx.x; c < TOTAL_CHUNKS; c += FA9_THREADS)
    {
        int row = c / CHUNKS_PER_ROW;
        int logical_chunk = c % CHUNKS_PER_ROW;
        int grow = tile_start + row;
        int physical_chunk = logical_chunk ^ (row & 7);
        __half *dst = &smem[row * FA9_KV_STRIDE + physical_chunk * 8];
...
// =============================================================================
// Process one Q tile (extracted for persistent kernel loop)
// =============================================================================

__device__ void process_q_tile(
    const __half *Q_head, const __half *K_head, const __half *V_head,
    __half *O_head,
    int q_start, int seq_len, int head_dim, int causal, float scale,
    __half *Q_s, __half *KV_s, __half *S_s, float *m_smem, float *l_smem)
{
    ...
    {
        constexpr int CHUNKS_PER_ROW = 16;
        constexpr int TOTAL = FA9_BR * CHUNKS_PER_ROW;
#pragma unroll 4
        for (int c = threadIdx.x; c < TOTAL; c += FA9_THREADS)
        {
            int row = c / CHUNKS_PER_ROW;
            int logical_chunk = c % CHUNKS_PER_ROW;
            int grow = q_start + row;
            int physical_chunk = logical_chunk ^ (row & 7);
            cp_async_cg_16(&Q_s[row * FA9_Q_STRIDE + physical_chunk * 8],
    ...
    // Init m/l
    for (int i = threadIdx.x; i < FA9_BR; i += FA9_THREADS)
    {
        m_smem[i] = -1e30f;
        l_smem[i] = 0.0f;
    }
...
#pragma unroll
    ...
    int num_kv_tiles = (seq_len + FA9_BC - 1) / FA9_BC;
    ...
    {
        int kv_start = kv_tile * FA9_BC;
        if (causal && kv_start > q_start + FA9_BR - 1)
            break;
...
#pragma unroll
        ...
        for (int ks = 0; ks < k_steps; ks++)
        {
            ...
            load_a_ldm_sw(a0, a1, a2, a3, Q_s, FA9_Q_STRIDE,
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                load_b_kt_ldm_sw(b0, b1, KV_s, FA9_KV_STRIDE,
...
#pragma unroll
        for (int nt = 0; nt < 4; nt++)
        {
            ...
            store_d_to_smem(S_s, FA9_S_STRIDE, m_row_base, col_base,
                            s_acc[nt][0], s_acc[nt][1], s_acc[nt][2], s_acc[nt][3],
                            lane_id);
        }
...
#pragma unroll
        for (int rr = 0; rr < 8; rr += 2)
        {
            ...
            float va0 = __half2float(S_s[srow_a * FA9_S_STRIDE + lane_id * 2]);
            float vb0 = __half2float(S_s[srow_b * FA9_S_STRIDE + lane_id * 2]);
            float va1 = __half2float(S_s[srow_a * FA9_S_STRIDE + lane_id * 2 + 1]);
            float vb1 = __half2float(S_s[srow_b * FA9_S_STRIDE + lane_id * 2 + 1]);
            ...
            S_s[srow_a * FA9_S_STRIDE + lane_id * 2] = __float2half(ea0);
            S_s[srow_b * FA9_S_STRIDE + lane_id * 2] = __float2half(eb0);
            S_s[srow_a * FA9_S_STRIDE + lane_id * 2 + 1] = __float2half(ea1);
            S_s[srow_b * FA9_S_STRIDE + lane_id * 2 + 1] = __float2half(eb1);
        }
...
#pragma unroll
            ...
            {
                uint32_t a0, a1, a2, a3;
                load_a_ldm_plain(a0, a1, a2, a3, S_s, FA9_S_STRIDE,
                                 m_row_base, ks * 16, lane_id);

                uint32_t b0, b1;
                load_b_v_ldm_sw(b0, b1, KV_s, FA9_KV_STRIDE,
                                n_col, ks * 16, lane_id);
        ...
        int next_start = next_tile * FA9_BC;
        bool has_next = (next_tile < num_kv_tiles) &&
                        (!causal || next_start <= q_start + FA9_BR - 1);
        if (has_next)
        {
    ...
    // Final: O / l → global
    {
        ...
        float l_inv9 = (l_smem[my_row8] > 0.0f) ? 1.0f / l_smem[my_row8] : 0.0f;
...
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
            ...
            if (grow8 < seq_len && col0 < head_dim)
                O_head[grow8 * head_dim + col0] = __float2half(o_acc[nt][2] * l_inv9);
            if (grow8 < seq_len && col1 < head_dim)
                O_head[grow8 * head_dim + col1] = __float2half(o_acc[nt][3] * l_inv9);
        }
    }

    // Sync before next tile (ensure O writes visible + SMEM can be reused)
    __syncthreads();
}
...
__global__ void __launch_bounds__(FA9_THREADS, 2)
    flash_attention_v9_kernel(
        ...
        int num_q_tiles,
        unsigned int *tile_counter) // global atomic counter
{
    extern __shared__ char smem_raw[];
    __half *Q_s = (__half *)smem_raw;
    __half *KV_s = (__half *)(smem_raw + FA9_BR * FA9_Q_STRIDE * sizeof(__half));
    __half *S_s = (__half *)(smem_raw + FA9_BR * FA9_Q_STRIDE * sizeof(__half) + FA9_BC * FA9_KV_STRIDE * sizeof(__half));
    float *m_smem = (float *)(smem_raw + FA9_BR * FA9_Q_STRIDE * sizeof(__half) + FA9_BC * FA9_KV_STRIDE * sizeof(__half) + FA9_BR * FA9_S_STRIDE * sizeof(__half));
    float *l_smem = m_smem + FA9_BR;

    int hstride = seq_len * head_dim;

    // Persistent loop: keep grabbing work until all tiles done
    while (true)
    {
        ...
        // But shfl only works within a warp — need block-level broadcast
        __shared__ unsigned int shared_tile_idx;
        if (threadIdx.x == 0)
        {
            shared_tile_idx = tile_idx;
        }
        __syncthreads();
        tile_idx = shared_tile_idx;

        if (tile_idx >= (unsigned int)total_tiles)
            break;

        int bh = tile_idx / num_q_tiles;
        int q_tile = tile_idx % num_q_tiles;
        int q_start = q_tile * FA9_BR;
        ...
        process_q_tile(Q_head, K_head, V_head, O_head,
                       q_start, seq_len, head_dim, causal, scale,
                       Q_s, KV_s, S_s, m_smem, l_smem);
    }
}

// =============================================================================
// Non-persistent fallback (same as v7, for comparison)
// =============================================================================

__global__ void __launch_bounds__(FA9_THREADS, 2)
    flash_attention_v9_static_kernel(
        const __half *__restrict__ Q,
        const __half *__restrict__ K,
        const __half *__restrict__ V,
        __half *__restrict__ O,
        int seq_len, int head_dim, int causal, float scale)
{
    ...
    int num_q_tiles = (seq_len + FA9_BR - 1) / FA9_BR;
...
static int g_fa9_smem_max = 0;

static int get_smem_bytes()
{
    return FA9_BR * FA9_Q_STRIDE * (int)sizeof(__half) + FA9_BC * FA9_KV_STRIDE * (int)sizeof(__half) + FA9_BR * FA9_S_STRIDE * (int)sizeof(__half) + FA9_BR * 2 * (int)sizeof(float);
}

extern "C"
{

    int flash_attention_v9_forward(
        ...
        int causal, void *stream)
    {
        ...
        int smem_bytes = get_smem_bytes();

        // Set smem for both kernels
        if (smem_bytes > g_fa9_smem_max)
        {
            cudaFuncSetAttribute(flash_attention_v9_kernel,
                                 cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes);
            cudaFuncSetAttribute(flash_attention_v9_static_kernel,
                                 cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes);
            g_fa9_smem_max = smem_bytes;
        }
        ...
        // Use persistent kernel when causal AND enough tiles to benefit
        // For non-causal, all tiles do equal work → no load imbalance
        if (causal && total_tiles > 256)
        {
            // Allocate and zero counter
            unsigned int *d_counter;
            cudaError_t err = cudaMallocAsync(&d_counter, sizeof(unsigned int),
                                              (cudaStream_t)stream);
            if (err != cudaSuccess)
            {
                // Fallback to static
            ...
            cudaMemsetAsync(d_counter, 0, sizeof(unsigned int), (cudaStream_t)stream);

            // Launch persistent: num_SMs * occupancy blocks
            int num_sms;
            cudaDeviceGetAttribute(&num_sms, cudaDevAttrMultiProcessorCount, 0);
            int persistent_blocks = num_sms * 2; // occupancy = 2

            flash_attention_v9_kernel<<<persistent_blocks, FA9_THREADS, smem_bytes,
                                        (cudaStream_t)stream>>>(
                (const __half *)Q, (const __half *)K, (const __half *)V, (__half *)O,
                total_heads, seq_len, head_dim, causal, scale_val,
                total_tiles, num_q_tiles, d_counter);

            cudaFreeAsync(d_counter, (cudaStream_t)stream);
            return (int)cudaGetLastError();
        }

    static_launch:
        flash_attention_v9_static_kernel<<<total_tiles, FA9_THREADS, smem_bytes,
                                           (cudaStream_t)stream>>>(
            (const __half *)Q, (const __half *)K, (const __half *)V, (__half *)O,
    ...
    int flash_attention_v9_forward_bhsd(
        const void *Q, const void *K, const void *V, void *O,
        int batch, int num_heads, int seq_len, int head_dim,
        int causal, void *stream)
    {
        return flash_attention_v9_forward(Q, K, V, O,
