...
// v80e: producer-warp-only (warp 0). 32 threads handle all cp.async (4× iters each).
...
#pragma unroll
    ...
    // v80e: producer-warp-only cp.async (warp 0). All 4 warps still compute (Br=64).
    if (wid == 0) {
...
#pragma unroll
        ...
        // v80e: end-of-iter K cp.async — producer warp only.
        int kv_p_k = kv + 1;
...
int main()
{
    ...
    printf("=== FA v80e (warp spec on Br=64) — EXPERIMENT: 4 blocks/SM via Br=64 + door 2 + single-K ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa80e_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        CK(cudaFuncSetAttribute(fa80e_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa80e_kernel<<<bh * nqt, FA_THREADS, smem>>>(
