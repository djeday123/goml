...
// v77: cooperative 4-lane transpose smV [seq_k=Bc, head_dim] → smV_T [head_dim, seq_k=Bc].
// Breaks v69's leftover 4-way bank conflict on STS.
//
// v69 per-thread 4×4 tile: within one STS, all 32 lanes had row[T] = T*4 (step 4).
// Bank step across warp = 4*68/4 mod 32 = 4. gcd(4,32) = 4 → 4-way conflict.
// v68's 64→68 padding only broke the 32-way; the leftover 4-way went unnoticed until
// NCu --set full flagged 62.5% conflict rate.
//
// Fix: 4 lanes cooperate via __shfl_sync. Lane j of a 4-lane group writes 1 row
// (instead of 4). Row step within warp becomes 1 → bank step = 17 → coprime with 32 →
// all 32 banks distinct per STS, no conflict.
__device__ __forceinline__ void transpose_v(
    uint8_t *smV_T, const uint8_t *smV, int head_dim)
{
    constexpr int TILE = 4;
    int tiles_k = FA_BC / TILE;             // 16 for FA_BC=64
    int tiles_n = head_dim / TILE;           // 32 for hd=128
    int total_tiles = tiles_k * tiles_n;     // 512
    const int groups_per_warp = 8;
    const int total_groups = groups_per_warp * (FA_THREADS / 32);  // 32
    int iters = total_tiles / total_groups;  // 16
    ...
    {
        int gid = it * total_groups + warp * groups_per_warp + group;
        int tk = gid / tiles_n;
        int tn = gid % tiles_n;
        int k0 = tk * TILE;
        int n0 = tn * TILE;

        // Lane j reads smV[k0+j][n0..n0+3] = 1 b32 (4 contiguous n-bytes).
        uint32_t v = *(uint32_t *)&smV[swz_byte(k0 + j, n0)];

        // Cooperative 4×4 transpose: lane j gathers byte j from each of the 4 group lanes.
        // After: lane j's out = [smV[k0+0][n0+j], smV[k0+1][n0+j], smV[k0+2][n0+j], smV[k0+3][n0+j]].
        uint32_t v0 = __shfl_sync(0xFFFFFFFF, v, group_base + 0);
        uint32_t v1 = __shfl_sync(0xFFFFFFFF, v, group_base + 1);
        uint32_t v2 = __shfl_sync(0xFFFFFFFF, v, group_base + 2);
        uint32_t v3 = __shfl_sync(0xFFFFFFFF, v, group_base + 3);
        int shift = j * 8;
        uint32_t out = ((v0 >> shift) & 0xff)
                     | (((v1 >> shift) & 0xff) << 8)
                     | (((v2 >> shift) & 0xff) << 16)
                     | (((v3 >> shift) & 0xff) << 24);

        // Lane j writes smV_T[n0 + j][k0..k0+3] = 1 b32.
        // Within warp: row step = 1 → bank step = 17 mod 32 → all 32 banks distinct.
        *(uint32_t *)&smV_T[swz_byte_smvt(n0 + j, k0)] = out;
    }
}
...
int main()
{
    ...
    printf("=== FA v77 FP8 forward — v69 + cooperative transpose_v (bank-conflict fix) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa77_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa77_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa77_kernel<<<bh * nqt, FA_THREADS, smem>>>(
