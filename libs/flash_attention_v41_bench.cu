// flash_attention_v41_bench.cu — v41 (ldm4 K) vs v20 (ldm2 K)
...
extern "C"
{
    ...
    int flash_attention_v41_forward(const void *, const void *, const void *, void *,
                                    int, int, int, int, void *);
...
{
    printf("--- Correctness (v41 vs v1-scalar) ---\n");
    struct
    {
        int h, s, d;
    } cfgs[] = {
    ...
    for (auto &c : cfgs)
    {
        ...
        CK(cudaMemset(dO_v41, 0, bytes));
        flash_attention_v41_forward(dQ, dK, dV, dO_v41, c.h, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());

        std::vector<__half> hr(n), hv(n);
        CK(cudaMemcpy(hr.data(), dO_ref, bytes, cudaMemcpyDeviceToHost));
        CK(cudaMemcpy(hv.data(), dO_v41, bytes, cudaMemcpyDeviceToHost));
...
void bench()
{
    printf("\n--- Performance: v20 vs v41 (ldm4 K) ---\n");
    printf("%-12s %10s %10s  %6s  %6s\n", "Config", "v20", "v41", "ratio", "peak%%");
    printf("---------------------------------------------------------\n");
    ...
    for (auto &c : cfgs)
    {
        ...
        // v41
        for (int i = 0; i < 3; i++)
            flash_attention_v41_forward(dQ, dK, dV, dO, heads, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());
        t.start();
        for (int i = 0; i < it; i++)
            flash_attention_v41_forward(dQ, dK, dV, dO, heads, c.s, c.d, 1, nullptr);
        float ms41 = t.stop();
        CK(cudaGetLastError());
        double t41 = flops / (ms41 / it / 1000.0) / 1e12;

        printf("%-12s %8.2f T %8.2f T  %5.2fx  %5.1f%%\n",
               c.l, t20, t41, t41 / t20, t41 / peak * 100.0);
...
int main()
{
    ...
    printf("=== FlashAttention v41 (ldm4 K) ===\n");
