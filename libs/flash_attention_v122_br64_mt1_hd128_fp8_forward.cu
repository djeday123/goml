...
// v122 EXPERIMENT: Br=64, M_TILES=1 геометрия. 4 warps × 1 тайл × 16 строк = 64 ✓.
// Прогноз: regs ~155-170 (от 244 у v121), SMEM 40.5 KB, 2 blocks/SM.
// ILP −12% на MMA, overhead +5-10% от удвоения Q-тайлов → wall −5..−15%.
#define FA_BR 64
#define FA_BC 64
#define FA_THREADS 128   // 4 warps × M_TILES=1 × 16 rows = Br=64 ✓
#define FA_STRIDE 128
#define M_TILES 1        // v122: один m16 тайл на warp
        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    int mrb = wid * 16;  // v122: M_TILES=1 → each warp owns 16 rows (4 warps × 16 = 64)
...
#pragma unroll
    ...
    float rmax[M_TILES][2];
    float rsexp[M_TILES][2];
#pragma unroll
    for (int _mi = 0; _mi < M_TILES; _mi++) {
        rmax[_mi][0] = rmax[_mi][1] = -1e30f;
        rsexp[_mi][0] = rsexp[_mi][1] = 0.0f;
    }
...
#pragma unroll
        ...
        float ns[M_TILES][2];
#pragma unroll
        for (int _mi = 0; _mi < M_TILES; _mi++) { ns[_mi][0] = ns[_mi][1] = 0.0f; }
...
#pragma unroll
        ...
        // v121 step1c: smP STS hoist.
        // swz_byte_bc(mr+gid, col) = (mr+gid)*64 + ((col>>4)^(gid&3))<<4 + (col&15)
        // mr=mrb+mi*16. row*64 = mrb*64 + 1024*mi + 64*gid. row&3 = gid&3 (const по mi/nt).
        // base_row0 = (mrb+gid)*64, base_row8 = base_row0+512, оба уже посчитаны выше
        // как smP_row_g0_base / smP_row_g8_base в PV-блоке. Но PV сейчас НИЖЕ — здесь
        // отдельно. Перевычислим, чтобы не нарушать порядок «жить только в своём батче».
