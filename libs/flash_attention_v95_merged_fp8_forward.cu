...
#pragma unroll
        ...
        // v95 merge: P_top/P_bot ARRAYS eliminated. Computed as scalar temps inside a
        // single merged loop, immediately converted to fp8 and packed into Pf_pair[nt][mi].
        // The 32-uint32 intermediate storage (P_top[8][2] + P_bot[8][2]) that lived
        // between softmax inner loop and quantize loop in v89 is now scalar-local —
        // dies each iteration. Real computation reorder (like v87 −8) — changes SSA.
...
#pragma unroll
            for (int mi = 0; mi < M_TILES; mi++)
            {
                ...
                __half2 p_top_h2 = *reinterpret_cast<__half2 *>(&p_top_u);
                __half2 p_bot_h2 = *reinterpret_cast<__half2 *>(&p_bot_u);
                ns[mi][0] += __low2float(p_top_h2) + __high2float(p_top_h2);
                ns[mi][1] += __low2float(p_bot_h2) + __high2float(p_bot_h2);
                // Inline Pf_pair packing — no array storage, dies after assignment.
...
// Runs ONE warmup + ONE measured launch of fa95_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa95_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        // warmup (NCu skips with --launch-skip)
        fa95_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        // measured (NCu profiles this one)
        fa95_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa95_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa95_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa95_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v95 merged softmax+quantize (kill P_top/P_bot arrays) ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa95_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa95_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        fa95_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
        ...
        CK(cudaFuncSetAttribute(fa95_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa95_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa95_kernel<2>);
            r3[v] = measure(fa95_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
