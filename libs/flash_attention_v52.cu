// =============================================================================
// FlashAttention v21 — Pre-scaled Q registers (over v20)
// =============================================================================
// Change: multiply Q registers by scale ONCE after load, remove per-tile scale.
//
// v20: each KV-tile does Sr[nt][i] *= scale (32 FMUL per tile)
// v21: Qr loaded once, then Qr[ks][i] *= __float2half(scale) → QK^T already scaled
//      Saves 32 FMUL × num_kv_tiles, costs 32 FP16 MUL once.
//
// Subtlety: Qr is uint32_t (packed half2). We need to scale each half2 element.
//           Do it by unpacking, scaling, repacking — or better, use hmul2.
//
// Build:
...
// Scale a packed half2 register by half2 scale factor
__device__ __forceinline__ uint32_t scale_h2(uint32_t packed, __half2 s)
{
...
// =============================================================================
// v20 kernel: original (scale in softmax loop)
// =============================================================================
                ...
                int seq_len, int head_dim, int causal, float scale)
{
    ...
    int bh = blockIdx.x / nqt, qt = blockIdx.x % nqt, qs = qt * FA20_BR;
...
// =============================================================================
// v21 kernel: pre-scaled Q registers (no per-tile scale)
// =============================================================================
...
#pragma unroll
    ...
    // >>> v21: pre-scale Q registers — ONCE, not per KV-tile <<<
    __half2 h2_scale = __float2half2_rn(scale);
#pragma unroll
    for (int ks = 0; ks < 8; ks++)
    {
        Qr[ks][0] = scale_h2(Qr[ks][0], h2_scale);
        Qr[ks][1] = scale_h2(Qr[ks][1], h2_scale);
        Qr[ks][2] = scale_h2(Qr[ks][2], h2_scale);
        Qr[ks][3] = scale_h2(Qr[ks][3], h2_scale);
    }
...
#pragma unroll
        ...
        // >>> v21: NO per-tile scale — QK^T already scaled <<<
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
            if (causal)
                ...
                int th, int sl, int hd, int ca)
{
    ...
    int nqt = (sl + FA20_BR - 1) / FA20_BR;
    fa20_kernel<<<th * nqt, FA20_THREADS, smem>>>(Q, K, V, O, sl, hd, ca, sc);
}
                ...
                int th, int sl, int hd, int ca)
{
    ...
    float sc = 1.0f / sqrtf((float)hd);
    int nqt = (sl + FA20_BR - 1) / FA20_BR;
    fa21_kernel<<<th * nqt, FA20_THREADS, smem>>>(Q, K, V, O, sl, hd, ca, sc);
}

void test_correctness()
{
    printf("--- Correctness: v20 vs v21 (pre-scaled Q) vs CPU ---\n");
...
void bench()
{
    printf("\n--- Performance: v20 (per-tile scale) vs v21 (pre-scaled Q) ---\n");
    ...
    printf("Saved: 32 FMUL/tile (scale). Cost: 32 HMUL once (Q pre-scale)\n");
}
...
int main()
{
    ...
    printf("=== FlashAttention v21 — Pre-scaled Q over v20 ===\n");
