...
// v108 warp-spec step 1 constants
#define FA_BR 64         // Br halved vs v96 to free SMEM for deeper buffer
#define FA_BC 64
#define FA_THREADS 128   // 4 warps total: 2 producer + 2 consumer
#define FA_STRIDE 128
#define FA_PRODUCERS 2   // wid 0,1: pure cp.async (warp 0=K, warp 1=V)
#define FA_CONSUMERS 2   // wid 2,3: pure compute. 2 cons × M_TILES=2 × 16 = Br=64 ✓
#define M_TILES 2        // each consumer warp owns 2 m16 sub-tiles = 32 rows
#define K_STAGES 2       // double-buffered K
#define V_STAGES 2       // double-buffered V (+1 over v96)
        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    // v108 warp-spec: wid 0,1 = producer (cp.async), wid 2,3 = consumer (compute)
    bool is_producer = (wid < FA_PRODUCERS);
    int cwid = wid - FA_PRODUCERS;  // consumer index 0,1 (undefined for producers)
    int mrb = cwid * 32;  // consumer warps own 32 rows each; M_TILES=2 × 16 = 32

    extern __shared__ uint8_t raw[];
    uint8_t *smQ = raw;
    // v108 SMEM layout: smQ + 2×smK + 2×smV + smV_T = 8K + 16K + 16K + 8.5K = 48.5K
    uint8_t *smK[K_STAGES] = {
        smQ + FA_BR * FA_STRIDE,
        smQ + FA_BR * FA_STRIDE + FA_BC * FA_STRIDE,
    };
    uint8_t *smV[V_STAGES] = {
    ...
    // for softmax output). smP layout = FA_BR × FA_BC bytes = 4 KB ≤ smQ's 8 KB.
    uint8_t *smP = smQ;
...
#pragma unroll
    ...
    // v108: PRE-LOAD via producer warps only.
    // wid=0 → K[kv_min] into smK[kv_min&1]
    // wid=1 → V[kv_min] into smV[kv_min&1]
    if (wid == 0) {
        load_tile_fp8_warp(smK[kv_min_blocks & 1], Kh, kv_min_blocks * FA_BC, FA_BC,
                           seq_len, head_dim, lane);
    } else if (wid == 1) {
        load_tile_fp8_warp(smV[kv_min_blocks & 1], Vh, kv_min_blocks * FA_BC, FA_BC,
                           seq_len, head_dim, lane);
    }
    cpa_commit();
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v108: transpose_v reads smV[buf] (consumer's V slot) → smV_T (consumer reads).
        // ALL warps participate (transpose_v uses FA_THREADS stride).
        transpose_v(smV_T, cur_V, head_dim);
        __syncthreads();

        // v108: producer-only mid-iter prefetch for K[kv+1] AND V[kv+1].
        // Consumer warps fall through to compute. cp.async runs in background
        // during consumer's heavy QK+softmax+PV phase.
        int kv_p = kv + 1;
        ...
        if (wid == 0) {
            ...
            load_tile_fp8_warp(smV[nxt_buf], Vh, kv_p * FA_BC, rows_p,
                               seq_len, head_dim, lane);
        }
        cpa_commit();
...
// Runs ONE warmup + ONE measured launch of fa108_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + 2 * FA_BC * FA_STRIDE
             + hd * SMV_T_STRIDE;
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa108_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa108_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa108_kernel<<<bh * nqt, FA_THREADS, smem>>>(
