// =============================================================================
// FlashAttention v84 — FP8 Forward, hd=64 + TMA-K + SWIZZLE_64B (bank-conflict fix)
// =============================================================================
// v83 base. Change two things:
//   1. encode_k_tmap: SWIZZLE_NONE → SWIZZLE_64B (HW pattern XORs sector with row&3)
//   2. QK MMA K reads: restore swz_byte (was plain in v83)
//
// Hypothesis: HW SWIZZLE_64B pattern matches our manual swz_byte:
//   manual: row * 64 + (chunk ^ (row & 3)) * 16 + within
//   HW:     row * 64 + (sector ^ (row & 3)) * 16 + ...  (per CUDA docs for sm_90+)
// If patterns match → TMA writes data the readers expect → no bank conflicts.
//
// v83 NCu showed No Eligible +6pp WORSE than v81 with TMA-K + plain reads.
// If v84 (SWIZZLE_64B + swizzled reads) comes back to v81's No Eligible 43%:
//   → bank conflicts WERE the driver (close that path, TMA path also closed).
// If v84 still has elevated No Eligible:
//   → mbarrier wait latency / single-lane dispatch is the driver, not bank conflicts.
//
...
#pragma unroll
            ...
            {
                int br = nt * 8;
                // v84: TMA writes swizzled (SWIZZLE_64B HW pattern = chunk^(row&3))
                // matches manual swz_byte → restore swizzled reads, no bank conflicts.
...
static void encode_k_tmap(CUtensorMap *tmap, uint8_t *K_base, int bh, int seq_len, int hd)
{
    ...
    CUresult res = cuTensorMapEncodeTiled(
        ...
        CU_TENSOR_MAP_SWIZZLE_64B,   // v84: HW pattern (sector ^ (row&3))*16 = matches swz_byte
...
// Runs ONE warmup + ONE measured launch of fa84_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa84_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa84_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, k_tmap, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa84_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, k_tmap, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa84_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa84_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, k_tmap, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa84_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, k_tmap, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v84 hd=64 + TMA-K + SWIZZLE_64B — bank-conflict-fixed reads ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa84_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa84_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        fa84_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
        ...
        CK(cudaFuncSetAttribute(fa84_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa84_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, k_tmap, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa84_kernel<2>);
            r3[v] = measure(fa84_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
