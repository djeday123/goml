...
#pragma unroll
        for (int mi = 0; mi < M_TILES; mi++)
        {
            // v121r SP1: skip rescale при rsc==1.0f (uniform по warpу после shfl-reduce).
            // IEEE: rmax_old == nm битово (после fmaxf), rmax_old-nm = +0.0f, exp2f(+0.0f)=1.0f.
            if (rsc[mi][0] != 1.0f || rsc[mi][1] != 1.0f) {
