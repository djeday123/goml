        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    // v79e: mbarrier toolchain test. Allocate barrier in static shared memory.
    // Tests if mbarrier PTX works correctly on sm_120a in our kernel context.
    __shared__ cuda::barrier<cuda::thread_scope_block> bar;
    if (threadIdx.x == 0) {
        init(&bar, FA_THREADS);
    }
    __syncthreads();  // ensure bar init visible
...
#pragma unroll
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v79e: replace __syncthreads with mbarrier arrive+wait. Toolchain test.
        // Same full-block semantics as __syncthreads.
        {
            auto tok = bar.arrive();
            bar.wait(cuda::std::move(tok));
        }
...
int main()
{
    ...
    printf("=== FA v79e FP8 forward — mbarrier toolchain test — v69 + V[N+1] mid-iter prefetch into dead smK[buf] ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa79e_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa79e_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa79e_kernel<<<bh * nqt, FA_THREADS, smem>>>(
