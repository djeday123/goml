...
// v80h: Br=80 + 1 producer + 5 consumers = 6 warps. FA_THREADS=192.
// 5 consumer warps × M_TILES=1 × 16 = 80 rows. Producer (wid=0) only does cp.async.
#define FA_BR 80
#define FA_BC 64
#define FA_THREADS 192            // 6 warps (1 producer + 5 consumers)
...
// v80h: producer warp (wid=0) only, 32 threads.
...
// v80h: launch_bounds(192, 2) → reg cap 170 for 2 blocks/SM (FA_THREADS=192).
// SMEM 34.5KB × 2 = 69KB ≤ 100KB. Br=80 + 1 prod + 5 cons.
        ...
        int window)  // 0 = no window. >0 = causal sliding window.
{
    ...
    // v80h: wid=0 producer (no compute); wids 1..5 consumers. compute_wid = wid - 1.
    int compute_wid = wid - 1;
    int mrb = compute_wid * 16;  // consumer warps cover 16 rows each (5 × 16 = 80 = Br)
    ...
    // v80h: Qr load — CONSUMER WARPS ONLY (wid >= 1).
...
#pragma unroll
    ...
    // v80h: pre-load via PRODUCER WARP (wid=0) only.
    if (wid == 0) {
        load_tile_fp8_warp0(smK, Kh, kv_min_blocks * FA_BC, FA_BC, seq_len, head_dim);
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v80h: QK MMA — CONSUMER WARPS ONLY.
...
#pragma unroll
        ...
        // Next iter's transpose_v will read smV (containing V[kv+1]).
        {
            ...
            // v80h: V[kv+1] cp.async — PRODUCER WARP only.
            if (wid == 0) {
                load_tile_fp8_warp0(smV, Vh, kv_p * FA_BC, rows_p, seq_len, head_dim);
            }
        }
        cpa_commit();

        // v80h: softmax + P_top/P_bot — CONSUMER WARPS ONLY.
...
#pragma unroll
        ...
        // v80h: P_top/P_bot declared above (cross-block scope).
...
#pragma unroll
        ...
        }  // end softmax `if (wid >= 1)`

        __syncthreads();

        // v80h: smP STS — CONSUMER WARPS ONLY.
        if (wid >= 1)
        {
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
...
#pragma unroll
        ...
        }  // end smP STS `if (wid >= 1)`
        __syncthreads();

        // v80h: PV MMA — CONSUMER WARPS ONLY.
        if (wid >= 1)
        {
#pragma unroll
...
#pragma unroll
        ...
        // v80h: end-of-iter K cp.async — PRODUCER WARP only.
        int kv_p_k = kv + 1;
        int rows_p_k = (kv_p_k < kv_max_blocks) ? FA_BC : 0;
        if (wid == 0) {
            load_tile_fp8_warp0(smK, Kh, kv_p_k * FA_BC, rows_p_k, seq_len, head_dim);
        }
        cpa_commit();
    }

    // v80h: output writeback — CONSUMER WARPS ONLY.
    if (wid >= 1)
    {
#pragma unroll
    for (int mi = 0; mi < M_TILES; mi++)
...
int main()
{
    ...
    printf("=== FA v80h (Br=80, 1-prod + 5-cons warp spec) — EXPERIMENT: 4 blocks/SM via Br=64 + door 2 + single-K ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa80h_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        CK(cudaFuncSetAttribute(fa80h_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa80h_kernel<<<bh * nqt, FA_THREADS, smem>>>(
