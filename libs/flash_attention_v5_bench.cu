// flash_attention_v5_bench.cu — v5 (ldmatrix) vs v4 vs v2-DB
...
{
    printf("--- Correctness (v5 vs v2-DB) ---\n");
    struct
    {
        int h, s, d;
    } cfgs[] = {
    ...
    for (auto &c : cfgs)
    {
        ...
        CK(cudaMemset(dO_v5, 0, bytes));
        flash_attention_v5_forward(dQ, dK, dV, dO_v5, c.h, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());

        std::vector<__half> hr(n), hv(n);
        CK(cudaMemcpy(hr.data(), dO_ref, bytes, cudaMemcpyDeviceToHost));
        CK(cudaMemcpy(hv.data(), dO_v5, bytes, cudaMemcpyDeviceToHost));
...
void bench()
{
    printf("\n--- Performance: v2-DB vs v4 vs v5 (ldmatrix) ---\n");
    printf("%-12s %10s %10s %10s  %6s\n", "Config", "v2-DB", "v4", "v5-ldm", "v5/v4");
    printf("---------------------------------------------------------------\n");
    ...
    for (auto &c : cfgs)
    {
        ...
        float ms_db = t.stop();
        CK(cudaGetLastError());
        double db_t = flops / (ms_db / it / 1000.0) / 1e12;
        ...
        float ms_v4 = t.stop();
        CK(cudaGetLastError());
        double v4_t = flops / (ms_v4 / it / 1000.0) / 1e12;
        ...
        float ms_v5 = t.stop();
        CK(cudaGetLastError());
        double v5_t = flops / (ms_v5 / it / 1000.0) / 1e12;

        printf("%-12s %8.2f T %8.2f T %8.2f T  %5.2fx\n",
               c.l, db_t, v4_t, v5_t, v5_t / v4_t);
...
int main()
{
    ...
    printf("=== FlashAttention v5 (ldmatrix) Benchmark ===\n");
