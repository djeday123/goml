...
// v72: launch_bounds(128, 4) → ptxas forced to ≤128 regs/thread for 4 blocks/SM.
...
#pragma unroll
    ...
    // Pre-load iter kv_min_blocks: K via cp.async, V via direct LDG into smV_T.
    load_tile_fp8(smK, Kh, kv_min_blocks * FA_BC, FA_BC, seq_len, head_dim);
    cpa_commit();
    load_v_transposed(smV_T, Vh, kv_min_blocks * FA_BC, seq_len, head_dim);
...
#pragma unroll
        ...
        // v80a: branch-free K/V prefetch (Lever 3 from v79).
        // load_tile_fp8 with rows=0 → empty for-loop → no cp.async issued.
        // load_v_transposed always runs FA_BC×chunks scattering loop, but internal
        // (gr < seq_len) check makes LDGs return zero for past-end. Writes zero data
        // to smV_T which is never read in last iter. Wasteful but correct, no branch.
        int kv_p = kv + 1;
        int rows_p = (kv_p < kv_max_blocks) ? FA_BC : 0;
        load_tile_fp8(smK, Kh, kv_p * FA_BC, rows_p, seq_len, head_dim);
        cpa_commit();
        load_v_transposed(smV_T, Vh, kv_p * FA_BC, seq_len, head_dim);
    }
...
int main()
{
    ...
    printf("=== FA v80a — EXPERIMENT: 4 blocks/SM via Br=64 + door 2 + single-K ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa80a_kernel));
    ...
    printf("Per-thread reg check: numRegs * 4 * 128 = %d (≤ 65536 for 4 blocks)\n\n",
           attr.numRegs * 4 * 128);
    ...
    for (auto &c : configs)
    {
        ...
        CK(cudaFuncSetAttribute(fa80a_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa80a_kernel<<<bh * nqt, FA_THREADS, smem>>>(
