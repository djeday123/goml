// =============================================================================
// FlashAttention v81 — FP8 Forward, hd=64 specialized (probe variant)
// =============================================================================
// v79b base, head_dim=64 hardcoded. Tests whether hd=64 layout naturally yields
// 3 blocks/SM (SMEM 24.25KB × 3 = 72.75KB ≤ 100KB cap). Register budget likely
// still pins to 2 blocks/SM unless ptxas frees enough state from the smaller
// Or_p[8] / Qr[2] / fewer ks-steps. Measured by ptxas -v.
//
...
// Runs ONE warmup + ONE measured launch of fa81_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa81_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        // warmup (NCu skips with --launch-skip)
        fa81_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        // measured (NCu profiles this one)
        fa81_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa81_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa81_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa81_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v81 hd=64 — reg-count probe (LB=2 vs LB=3) ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa81_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa81_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        fa81_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
        ...
        CK(cudaFuncSetAttribute(fa81_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa81_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa81_kernel<2>);
            r3[v] = measure(fa81_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
