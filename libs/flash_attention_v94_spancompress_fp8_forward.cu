        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    // v94 span-compress: Oh declaration MOVED to after kv-loop (line 632).
    // R8/R9 in v89 held output write base from function entry through entire
    // kv-loop — wasted 2 slots at peak pressure. SASS confirmed R8 born at
    // 0x0340, used as STG.E.U16 [R8.64] at 0xb710 (epilogue).
...
#pragma unroll
    ...
    // v94: Oh declaration deferred from line 234 to here. Same arithmetic.
    __half *Oh = O + bh * hs;
...
// Runs ONE warmup + ONE measured launch of fa94_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa94_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        // warmup (NCu skips with --launch-skip)
        fa94_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        // measured (NCu profiles this one)
        fa94_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa94_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa94_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa94_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v94 span-compress (Oh decl moved post-loop) ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa94_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa94_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        fa94_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
        ...
        CK(cudaFuncSetAttribute(fa94_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa94_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa94_kernel<2>);
            r3[v] = measure(fa94_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
