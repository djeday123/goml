...
#pragma unroll
            for (int mi = 0; mi < M_TILES; mi++)
            {
                // v79 lever 4: log2-space d → ex2.approx.f16x2 (2× MUFU throughput).
                // d already in log2 space because fs *= log2(e). 2 issues vs 4 ex2.f32 issues.
                ...
                __half2 p_top = *reinterpret_cast<__half2 *>(&p_top_u);
                __half2 p_bot = *reinterpret_cast<__half2 *>(&p_bot_u);
                float p0 = __low2float(p_top),  p1 = __high2float(p_top);
                float p2 = __low2float(p_bot),  p3 = __high2float(p_bot);
                ns[mi][0] += p0 + p1;
                ns[mi][1] += p2 + p3;
                P_local[nt][mi][0] = p0;
                P_local[nt][mi][1] = p1;
...
int main()
{
    ...
    printf("=== FA v79 FP8 forward — v69 + V[N+1] mid-iter prefetch into dead smK[buf] ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa79_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa79_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa79_kernel<<<bh * nqt, FA_THREADS, smem>>>(
