// flash_attention_v45_bench.cu - test v45 (adaptive rescaling) vs v54 vs ref
#include <cstdint>
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
extern "C"
{
    ...
    int flash_attention_v45_forward(const void *, const void *, const void *, void *,
                                    int, int, int, int, void *);
    int flash_attention_forward(const void *, const void *, const void *, void *,
                                int, int, int, int, void *);
}

// Use v54 via the self-contained fa54_kernel in v54's .cu file
// For simplicity, link against the v54 object that has its own launch wrapper
...
int main()
{
    ...
    printf("=== FlashAttention v45 — Adaptive Rescaling + FP16 ex2 ===\n");
    ...
    for (auto &c : cfgs)
    {
        ...
        flash_attention_v45_forward(dQ, dK, dV, dO45, c.h, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());

        std::vector<__half> hr(n), h20(n), h45(n);
        CK(cudaMemcpy(hr.data(), dO_ref, bytes, cudaMemcpyDeviceToHost));
        CK(cudaMemcpy(h20.data(), dO20, bytes, cudaMemcpyDeviceToHost));
        CK(cudaMemcpy(h45.data(), dO45, bytes, cudaMemcpyDeviceToHost));

        float max45 = 0, max45_20 = 0;
        int err45 = 0;
        for (size_t i = 0; i < n; i++)
        {
            float rv = __half2float(hr[i]);
            float v20 = __half2float(h20[i]);
            float v45 = __half2float(h45[i]);
            float ae = fabsf(rv - v45);
            float ae_cross = fabsf(v20 - v45);
            if (ae > max45)
                max45 = ae;
            if (ae_cross > max45_20)
                max45_20 = ae_cross;
            float thr = fmaxf(0.003f, fabsf(rv) * 0.08f);
            if (ae > thr)
                err45++;
        }
        printf("  %dh x %4ds: v45_vs_ref=%.4f err=%d %s  v45_vs_v20=%.4f\n",
               c.h, c.s, max45, err45, err45 == 0 ? "PASS" : "FAIL", max45_20);
    ...
    // Performance
    printf("\n--- Performance: v20 vs v45 ---\n");
    printf("%-12s %10s %10s %8s %7s\n", "Config", "v20 (T)", "v45 (T)", "delta", "peak%");
    printf("------------------------------------------------------\n");
    Timer t;
    struct
    {
    ...
    for (auto &c : pcfgs)
    {
        ...
        for (int i = 0; i < 3; i++)
            flash_attention_v45_forward(dQ, dK, dV, dO, c.h, c.s, 128, 1, nullptr);
        CK(cudaDeviceSynchronize());
        t.start();
        for (int i = 0; i < it; i++)
            flash_attention_v45_forward(dQ, dK, dV, dO, c.h, c.s, 128, 1, nullptr);
        ...
        double t45 = flops / (ms45 / it / 1000.0) / 1e12;
        printf("%-12s %8.2f T %8.2f T %+7.2f %6.1f%%\n",
               c.l, t20, t45, t45 - t20, t45 / 165.2 * 100);
        cudaFree(dQ);
        cudaFree(dK);
        cudaFree(dV);
        cudaFree(dO);
