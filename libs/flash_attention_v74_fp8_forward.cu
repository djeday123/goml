// =============================================================================
// FlashAttention v74 — STRESS TEST: launch_bounds(_,12) on v69 base
// =============================================================================
// Pure register-compression experiment. KEEP everything v69:
//   - Br=128, Bc=64, M_TILES=2, FA_THREADS=128
//   - SMEM still 48.5 KB (smQ 16 + smK[2] 16 + smV 8 + smV_T 8.5)
//   - Single-V (smP overlaps smV after transpose_v)
//
// ONLY change: __launch_bounds__(128, 12) tells ptxas target 12 blocks/SM.
// Reg cap = 65536 / (12 × 128) = 42 regs/thread (v69 used 242 → -200 cut).
//
// SMEM math: 12 × 48.5 = 582 KB ≫ 100 KB cap. Actual occupancy will STILL be
// SMEM-limited to 2 blocks/SM (same as v69). Combined:
//   - ptxas already compressed regs to 42/thread (massive spill expected)
//   - But occupancy gain not materialized (SMEM binds at 2 blocks)
// → Pure local-memory thrashing cost study. Expected severe regression.
// =============================================================================
...
// v74 STRESS: launch_bounds(_, 12) → ptxas reg cap = 65536/(12*128) = 42/thread.
// v69 used 242 regs. Cut by 200 → massive spill expected.
...
int main()
{
    ...
    printf("=== FA v74 STRESS — launch_bounds(_, 12), force 42 regs/thread ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa74_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa74_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa74_kernel<<<bh * nqt, FA_THREADS, smem>>>(
