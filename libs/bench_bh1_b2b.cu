// 046 Part I b2b: bh=1 sl=8192 с cudaAccessPolicyWindow (Persisting) на dS_nat.
// Все 4 ядра ЗАПУСКАЮТСЯ В ОДНОМ ПОТОКЕ back-to-back БЕЗ cudaDeviceSynchronize/reset
// между ними. Sync ТОЛЬКО ОДНАЖДЫ после dq перед чтением результатов.
