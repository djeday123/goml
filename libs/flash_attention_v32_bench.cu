...
extern "C"
{
    ...
    int flash_attention_v20_forward(const void *, const void *, const void *, void *,
                                    int, int, int, int, void *);
    int flash_attention_v32_forward(const void *, const void *, const void *, void *,
                                    int, int, int, int, void *);
}
...
{
    printf("--- Correctness (v32 vs v1-scalar) ---\n");
    struct
    {
        int h, s, d;
    } cfgs[] = {
    ...
    for (auto &c : cfgs)
    {
        ...
        flash_attention_v32_forward(dQ, dK, dV, dT, c.h, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());
...
void bench()
{
    printf("\n--- Performance: v20 (K-first) vs v32 (N-first) ---\n");
    printf("%-12s %10s %10s  %6s  %6s %6s\n",
           "Config", "v20", "v32", "ratio", "v20%", "v32%");
    printf("--------------------------------------------------------------\n");
    ...
    for (auto &c : cfgs)
    {
        ...
        for (int i = 0; i < 3; i++)
            flash_attention_v32_forward(dQ, dK, dV, dO, hds, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());
        t.start();
        for (int i = 0; i < it; i++)
            flash_attention_v32_forward(dQ, dK, dV, dO, hds, c.s, c.d, 1, nullptr);
        float m32 = t.stop();
        double t32 = flops / (m32 / it / 1e3) / 1e12;

        printf("%-12s %8.2f T %8.2f T  %5.2fx  %5.1f%% %5.1f%%\n",
               c.l, t20, t32, t32 / t20, t20 / pk * 100, t32 / pk * 100);
        cudaFree(dQ);
        cudaFree(dK);
        cudaFree(dV);
        cudaFree(dO);
...
int main()
{
    ...
    printf("=== FlashAttention v32 (N-first QK^T loop) ===\n");
