// =============================================================================
// FlashAttention v69_singleV — Door 1: single-buffer V, keep smV_T (occupancy)
// =============================================================================
// After byte-gather V proved a dead end (4 LDS.U8/b32 → spill + perf loss),
// switched to Door 1: free 8 KB by single-buffering V instead of removing
// smV_T. smV_T stays = V read pattern unchanged = no byte-gather pain.
//
// SMEM layout:
//   smQ:    16 KB
//   smK[2]: 16 KB  (K stays double-buffered)
//   smV:     8 KB  (was 16 KB = double-buffered)
//   smV_T:   8 KB  (padded stride 68 from v68)
//   smP overlaps smV after transpose_v (same trick as v68)
//   Total:  48 KB  →  2 blocks × 48 = 96 KB ≤ 100 KB cap → 2 blocks/SM
//
// Sequencing: V prefetch moves to END of iter (after smP consumed and freed).
// K prefetch stays at MID-iter (overlaps with compute). Cost: V prefetch
// loses overlap, but v64 datapoint and v68 NCu (mem busy 22%) → kernel
// NOT memory-bound on these shapes → loss expected ~0-5%.
//
// Gain: occupancy 8.33% → 16.67% (2 blocks/SM × 4 warps/block = 8 warps/SM)
// → directly attacks MMA pipeline latency floor (1.06 cycles per issued inst
// in v68 = 37% of stall time, only hideable via more in-flight warps).
//
...
int main()
{
    ...
    printf("=== FA v69_singleV — Door 1: single-buffer V → 2 blocks/SM ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa69_singleV_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa69_singleV_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa69_singleV_kernel<<<bh * nqt, FA_THREADS, smem>>>(
