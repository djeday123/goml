...
// SP0 probe: counters[kv_iter][grew_or_not] — global counter of max-growth events.
// Shape: counters[256 × 2] uint64_t. Index 0 = not grew, 1 = grew.
#define SP0_MAX_KV_ITERS 256
__global__ void __launch_bounds__(FA_THREADS, 2)
    fa96b_kernel(
        ...
        int window,
        unsigned long long *sp0_counters = nullptr)
{
...
#pragma unroll
            ...
            // === SP0 PROBE: count whether max grew vs rmax_old ===
            // Only thread 0 of consumer warp 0 logs (one event per warp per (kv, mi, side))
            if (threadIdx.x == 0 && sp0_counters != nullptr) {
                int kv_idx = (kv < SP0_MAX_KV_ITERS) ? kv : (SP0_MAX_KV_ITERS - 1);
                // Side 0 (top half rows)
                int grew0 = (nm[mi][0] > rmax[mi][0]) ? 1 : 0;
                atomicAdd(&sp0_counters[kv_idx * 4 + mi * 2 + 0], (unsigned long long)(grew0 ^ 1));  // not grew
                atomicAdd(&sp0_counters[kv_idx * 4 + mi * 2 + 1], (unsigned long long)grew0);       // grew
                // Side 1 (bottom half rows) — same kv-iter, same mi, just other half — bundle into same counters
                int grew1 = (nm[mi][1] > rmax[mi][1]) ? 1 : 0;
                atomicAdd(&sp0_counters[kv_idx * 4 + mi * 2 + 0], (unsigned long long)(grew1 ^ 1));
                atomicAdd(&sp0_counters[kv_idx * 4 + mi * 2 + 1], (unsigned long long)grew1);
            }
...
// Runs ONE warmup + ONE measured launch of fa96b_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa96b_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa96b_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
// W1.0 time-mode: ./fa_v121r_diet --time bh sl wnd [N]
// Прогоняет N (default 20) запусков, печатает median time_ms + TFLOPS.
static int time_mode(int bh, int sl, int wnd, int N)
{
    int hd = 128;
    int ca = (wnd > 0) ? 1 : 0;
    size_t n_elems = (size_t)bh * sl * hd;
    ...
    float med = times[N/2];
    double ops = (double)bh * sl * (ca ? (sl/2.0) : (double)sl) * hd * 4.0;
    double tflops = ops / (med * 1e-3) / 1e12;
    printf("bh=%d sl=%d wnd=%d grid=%d  median=%.4fms  TFLOPS=%.2f  (N=%d, min=%.4f max=%.4f)\n",
           bh, sl, wnd, grid, med, tflops, N, times.front(), times.back());
    cudaFree(Q_d); cudaFree(K_d); cudaFree(V_d); cudaFree(O_d);
    return 0;
}

// SP0 probe: --sp0 bh sl wnd
// Runs ONE launch with sp0_counters allocated, prints per-(kv,mi) growth histogram.
// Q/K/V init randomly to make max-growth pattern realistic (not synthetic).
static int sp0_mode(int bh, int sl, int wnd)
{
    int hd = 128;
    int ca = (wnd > 0) ? 1 : 0;
    size_t n_elems = (size_t)bh * sl * hd;
    ...
    CK(cudaMalloc(&Q_d, n_elems)); CK(cudaMalloc(&K_d, n_elems));
    CK(cudaMalloc(&V_d, n_elems)); CK(cudaMalloc(&O_d, n_elems * 2));
    // Random FP8 e4m3 byte distribution — realistic-ish (centered on small magnitudes)
    uint8_t *host_buf = (uint8_t*)malloc(n_elems);
    srand(42);
    for (size_t i = 0; i < n_elems; i++) {
        // Mix of small FP8 values (mantissa bits varied, exponent low to avoid overflow)
        host_buf[i] = (rand() & 0x3F) | ((rand() & 0x03) << 5);  // exponent ∈ [0,3], mantissa varied
    }
    CK(cudaMemcpy(Q_d, host_buf, n_elems, cudaMemcpyHostToDevice));
    for (size_t i = 0; i < n_elems; i++) host_buf[i] = (rand() & 0x3F) | ((rand() & 0x03) << 5);
    CK(cudaMemcpy(K_d, host_buf, n_elems, cudaMemcpyHostToDevice));
    for (size_t i = 0; i < n_elems; i++) host_buf[i] = (rand() & 0x3F) | ((rand() & 0x03) << 5);
    CK(cudaMemcpy(V_d, host_buf, n_elems, cudaMemcpyHostToDevice));
    free(host_buf);
    size_t counters_bytes = SP0_MAX_KV_ITERS * 4 * sizeof(unsigned long long);
    unsigned long long *sp0_d, *sp0_h;
    CK(cudaMalloc(&sp0_d, counters_bytes));
    CK(cudaMemset(sp0_d, 0, counters_bytes));
    sp0_h = (unsigned long long*)malloc(counters_bytes);
    int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
             + hd * SMV_T_STRIDE;
    int nqt = (sl + FA_BR - 1) / FA_BR;
    int grid = bh * nqt;
    ...
    CK(cudaFuncSetAttribute(fa96b_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa96b_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd, sp0_d);
    CK(cudaDeviceSynchronize());
    CK(cudaMemcpy(sp0_h, sp0_d, counters_bytes, cudaMemcpyDeviceToHost));
    int nkv = (sl + FA_BC - 1) / FA_BC;
    printf("SP0 max-growth histogram (bh=%d sl=%d wnd=%d, %d kv-iters per row, grid=%d):\n",
           bh, sl, wnd, nkv, grid);
    printf("kv-iter |  mi=0 not_grew  mi=0 grew  miss%%  |  mi=1 not_grew  mi=1 grew  miss%%\n");
    for (int kv = 0; kv < nkv && kv < SP0_MAX_KV_ITERS; kv++) {
        unsigned long long n00 = sp0_h[kv*4+0], n01 = sp0_h[kv*4+1];
        unsigned long long n10 = sp0_h[kv*4+2], n11 = sp0_h[kv*4+3];
        double m0 = (n00+n01>0) ? 100.0 * n01 / (n00+n01) : 0;
        double m1 = (n10+n11>0) ? 100.0 * n11 / (n10+n11) : 0;
        printf("  %3d   |  %10llu  %10llu  %5.1f%%  |  %10llu  %10llu  %5.1f%%\n",
               kv, n00, n01, m0, n10, n11, m1);
    }
    cudaFree(Q_d); cudaFree(K_d); cudaFree(V_d); cudaFree(O_d); cudaFree(sp0_d);
    free(sp0_h);
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 5 && strcmp(argv[1], "--time") == 0) {
        int bh = atoi(argv[2]), sl = atoi(argv[3]), wnd = atoi(argv[4]);
        int N = (argc >= 6) ? atoi(argv[5]) : 20;
        return time_mode(bh, sl, wnd, N);
    }
    if (argc >= 5 && strcmp(argv[1], "--sp0") == 0) {
        int bh = atoi(argv[2]), sl = atoi(argv[3]), wnd = atoi(argv[4]);
        return sp0_mode(bh, sl, wnd);
    }
    ...
    CK(cudaFuncGetAttributes(&attr, fa96b_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa96b_kernel<<<bh * nqt, FA_THREADS, smem>>>(
