        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    uint8_t *smQ = raw;
    // v117b LOCAL-MEM FIX (port v96b/v118): заменить `uint8_t *smK[K_STAGES]` массив
    // (16-байтный stack frame + LDL.64 на dynamic index в hot loop) на base + arithmetic
    // stride. smK[s] inline → (smK_base + s * K_STRIDE_BYTES) на месте вызова.
    constexpr int K_STRIDE_BYTES = FA_BC * FA_STRIDE;
    uint8_t *smK_base = smQ + FA_BR * FA_STRIDE;
    uint8_t *smV = smK_base + 2 * K_STRIDE_BYTES;
...
#pragma unroll
    ...
    if (wid == 0) {
        load_tile_fp8_warp(smK_base + (kv_min_blocks & 1) * K_STRIDE_BYTES, Kh,
                           kv_min_blocks * FA_BC, FA_BC, seq_len, head_dim, lane);
        load_tile_fp8_warp(smV, Vh, kv_min_blocks * FA_BC, FA_BC,
                           seq_len, head_dim, lane);
    }
    cpa_commit();
    // v117 pre-loop one-time __syncthreads: ensures pre-loaded V[0] visible to
    // consumer at iter 0 transpose. Without this, consumer reads stale smV at
    // iter 0 (v117 has no top sync to acquire producer's cpa_wait fence).
    // Cost: amortizes to negligible (1 sync vs many iter syncs removed).
    cpa_wait<0>();
    __syncthreads();
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v117: partial top sync. Producer SKIPS top sync entirely.
        //   Consumer: bar.sync 5, 96 — consumer-only top sync (for K memory ordering).
        //   Producer: no top sync. Issues K[kv+1] cp.async IMMEDIATELY after cpa_wait.
        //     This is safe — K[kv+1] writes to smK[nxt_buf], a DIFFERENT buffer than
        //     smK[buf] that consumer reads in this iter.
        //   Mid-iter bar.sync 4, 128 (full block): producer waits for consumer transpose
        //     to finish (consumer arrives at bar 4 after transpose_v). After this sync,
        //     producer issues V[kv+1] cp.async (safe — consumer done reading smV).
        //   Replaces v111's bar.sync 3, 96 post-transpose (consumer-only) — bar 4 covers
        //     both protections.
        // Race analysis:
        //   - smK[nxt_buf]: producer writes via cp.async. Consumer doesn't read until
        //     NEXT iter's top sync. K LANDS during current iter (HBM ~700 cycles), well
        //     before next iter consumer reads. K memory ordering via producer's cpa_wait
        //     in next iter (waits for K land) + bar.sync 4 fence (this iter, before V).
        //   - smV: producer writes V[kv+1] cp.async AFTER bar.sync 4. Consumer's
        //     transpose READS happened BEFORE consumer's bar.sync 4 arrival. Strict
        //     happens-before. NO race.
        // NOTE: cpa_wait already done by pre-loop sync at iter 0; for iter > 0,
        //   producer's mid-iter bar.sync 4 (prev iter) ensures consumer sees V[X]
        //   via fence. Consumer's cpa_wait is redundant but kept for safety.
        cpa_wait<0>();
        if (!is_producer) {
            asm volatile("bar.sync 5, 96;");  // consumer-only top sync
        }

        // Producer: issue K[kv+1] cp.async IMMEDIATELY — overlaps with consumer transpose.
        int kv_p = kv + 1;
        int rows_p = (kv_p < kv_max_blocks) ? FA_BC : 0;
        if (wid == 0) {
            load_tile_fp8_warp(smK_base + nxt_buf * K_STRIDE_BYTES, Kh,
                               kv_p * FA_BC, rows_p, seq_len, head_dim, lane);
        }

        // Consumer: transpose_v. Producer skips (just falls through to bar.sync 4).
        if (!is_producer) {
            transpose_v_consumer(smV_T, smV, head_dim);
        }

        // v117 mid-iter SYNC: producer + consumer (full block 128).
        // Ensures producer's V cp.async happens-after consumer's smV reads.
        // For consumer: acquires producer's K cp.async issue fence (next-iter K visible).
        asm volatile("bar.sync 4, 128;");

        // Producer: NOW safe to issue V cp.async (consumer done with smV reads).
        if (wid == 0) {
...
// Runs ONE warmup + ONE measured launch of fa117_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa117_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa117_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    CK(cudaFuncGetAttributes(&attr, fa117_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa117_kernel<<<bh * nqt, FA_THREADS, smem>>>(
