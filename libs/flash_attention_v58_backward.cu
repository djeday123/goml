// =============================================================================
// FlashAttention v58 — Backward, FP16 accumulator on per-block MMAs
//
// Experiment: switch S = QKᵀ and dP = dO·Vᵀ MMAs from f32 accum (v57) to
// f16 accum. Hypothesis: half the register pressure on the largest two
// register arrays (Sr/dPr 32 floats each → 16 packed uint32_t each) →
// potentially higher occupancy or faster steady state.
//
// Risk: f16 accum on a single m16n8k16 MMA reduces 16 fp16×fp16 products
// into one fp16 — should be fine numerically since we don't cross-accumulate
// (each Sr[nt] is a fresh accumulator). FP16 range issue would surface on
// extreme dynamic range Q·Kᵀ before LSE-subtraction; we already mask the
// non-causal tail, so worst case the LSE re-normalization handles it.
//
// dQ/dK/dV accumulators STAY f32 — they cross K-block (Pass 1) / Q-block
// (Pass 2) boundaries and accumulate many MMAs, where FP16 rolloff would
// matter.
//
...
// FP16 accumulator variant — same shape, less register pressure.
// d0/d1 are packed FP16 (each holds 2 fp16 values). Layout matches the
// output of the f16-accum form per PTX ISA: d0 = (row gid, col tid*2 | row gid, col tid*2+1),
//                                            d1 = (row gid+8, ...).
__device__ __forceinline__ void mma16816_f16acc(
...
// Unpack a __half2-as-uint32_t into 2 floats (low, high).
...
#pragma unroll
    ...
    for (int kv = 0; kv < kv_max_blocks; kv++)
    {
        ...
        // ── S = Q · Kᵀ · scale  (per warp: 16 rows × Bc=64 cols, FP16 accum) ──
        uint32_t Sr_p[8][2];
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
            Sr_p[nt][0] = Sr_p[nt][1] = 0u;
#pragma unroll
        for (int ks = 0; ks < 8; ks++)
        {
#pragma unroll
            for (int nt = 0; nt < 8; nt++)
            {
                ...
                mma16816_f16acc(Sr_p[nt][0], Sr_p[nt][1],
                                Qr[ks][0], Qr[ks][1], Qr[ks][2], Qr[ks][3],
                                b0, b1, Sr_p[nt][0], Sr_p[nt][1]);
            }
        }

        // Unpack FP16 accum to FP32 for softmax / mask / dS math.
        float Sr[8][4];
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
            unpack_h2(Sr_p[nt][0], Sr[nt][0], Sr[nt][1]);
            unpack_h2(Sr_p[nt][1], Sr[nt][2], Sr[nt][3]);
        }
...
#pragma unroll
            for (int nt = 0; nt < 8; nt++)
            {
                ...
                mma16816_f16acc(dPr_p[nt][0], dPr_p[nt][1],
                                dOr[ks][0], dOr[ks][1], dOr[ks][2], dOr[ks][3],
                                b0, b1, dPr_p[nt][0], dPr_p[nt][1]);
            }
        }
        float dPr[8][4];
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
            unpack_h2(dPr_p[nt][0], dPr[nt][0], dPr[nt][1]);
            unpack_h2(dPr_p[nt][1], dPr[nt][2], dPr[nt][3]);
        }
...
#pragma unroll
        ...
        // ── dP = V · dOᵀ   (V resident in Vr; dO transposed via ld_b_sw, FP16 accum) ──
...
extern "C" void launch_v58_backward_dq(
    ...
    int th, int sl, int hd, int ca)
{
    ...
    if (smem > g_smem_dq)
    {
        cudaFuncSetAttribute(fa58_backward_dq_kernel,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
        g_smem_dq = smem;
    }

    dim3 grid(th, nqt);
    fa58_backward_dq_kernel<<<grid, FA_THREADS, smem>>>(
...
extern "C" void launch_v58_backward_dkdv(
    const __half *Q, const __half *K, const __half *V,
    const __half *dO, const float *LSE, const float *D,
    ...
    int th, int sl, int hd, int ca)
{
    ...
    if (smem > g_smem_dkdv)
    {
        cudaFuncSetAttribute(fa58_backward_dkdv_kernel,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
        g_smem_dkdv = smem;
    }

    dim3 grid(th, nkv);
    fa58_backward_dkdv_kernel<<<grid, FA_THREADS, smem>>>(
...
extern "C" void launch_v58_backward(
    const __half *Q, const __half *K, const __half *V,
    ...
    int th, int sl, int hd, int ca)
{
    ...
    launch_v58_backward_dq(Q, K, V, dO, LSE, D_scratch, dQ_out, th, sl, hd, ca);
    launch_v58_backward_dkdv(Q, K, V, dO, LSE, D_scratch, dK_out, dV_out, th, sl, hd, ca);
}
...
void test_correctness()
{
    ...
    for (auto &c : configs)
    {
        ...
        launch_v58_backward_dq(Q_d, K_d, V_d, dO_d, LSE_d, D_d, dQ_d, th, sl, hd, ca);
        launch_v58_backward_dkdv(Q_d, K_d, V_d, dO_d, LSE_d, D_d, dK_d, dV_d, th, sl, hd, ca);
...
int main()
{
    ...
    printf("=== FlashAttention v58 backward — FP16 accumulator on S, dP MMAs ===\n");
