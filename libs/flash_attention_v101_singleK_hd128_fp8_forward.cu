        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    uint8_t *smQ = raw;
    // v101 single-K: ONE smK buffer (was 2). Saves 8 KB SMEM → 40.5 KB/block.
    // K[kv+1] cp.async issued AFTER QK done (smK is dead, K[kv+1] overwrites K[kv]).
    // V always lands in smV (no alternating prev_V_slot).
...
#pragma unroll
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v101: V always in smV. transpose_v reads smV (no alternating prev_V_slot).
...
// Runs ONE warmup + ONE measured launch of fa101_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa101_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa101_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v101 hd=128 + single-K (drop smK[1], step 1 of 3-block path) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa101_kernel));
    ...
    int smem_v96   = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                   + 128 * SMV_T_STRIDE;
    printf("v101 single-K SMEM (hd=128): %d B (%.2f KB) vs v96=%d B (delta=%+d B)\n",
           smem_hd128, smem_hd128 / 1024.0, smem_v96, smem_hd128 - smem_v96);
    printf("2 blocks × %.2f KB = %.2f KB ≤ 100 KB cap → 2 blocks/SM\n",
           smem_hd128 / 1024.0, 2 * smem_hd128 / 1024.0);
    printf("3 blocks × %.2f KB = %.2f KB %s → %s\n\n",
           smem_hd128 / 1024.0, 3 * smem_hd128 / 1024.0,
           (3 * smem_hd128 <= 102400) ? "≤ 100 KB cap" : "> 100 KB cap (still blocked)",
           (3 * smem_hd128 <= 102400) ? "3 blocks/SM IF reg-budget allows" : "v102 needed (smV_T elim)");
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa101_kernel<<<bh * nqt, FA_THREADS, smem>>>(
