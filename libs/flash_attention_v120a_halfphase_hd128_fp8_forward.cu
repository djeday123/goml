        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    // v120a P2: half-phase static offset. SMID-probe (RTX PRO 6000, 188 SMs,
    // launch_bounds(_, 2)) показал: первая wave даёт по 1 блоку на SM, вторая
    // wave (blockIdx.x + 188) приходит вторым блоком на тот же SM. Predicate
    // (blockIdx.x / 188) & 1 различает «первый-на-SM» (=0) и «второй» (=1).
    // Half-phase = cycles_per_iter / 2 ≈ 2 μs / 2 = 1000 ns (one-shot в прологе).
    if (((blockIdx.x / 188) & 1) == 1) {
        __nanosleep(1000);
    }
