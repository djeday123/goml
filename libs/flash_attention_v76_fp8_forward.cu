// =============================================================================
// FlashAttention v76 — REAL 12 blocks/SM via smQ↔smV_T overlap
// =============================================================================
// First actual 12 blocks/SM achievement. Key trick: smQ slot is dead after
// Q→Qr registers (Q only loaded once before main loop), so V_T loads into the
// same slot. SMEM budget per block:
//   - Slot A (smQ → smV_T after Q registered): max(Br*128, hd*32) = 4 KB
//   - Slot B (smK → smP after Q·K^T): max(Bc*128, Br*Bc) = 4 KB
//   - Total: 8 KB × 12 = 96 KB ≤ 100 ✓
//
// Br=32, Bc=32, FA_THREADS=64, Door 2 V load, single-K, launch_bounds(64, 12).
// Reg cap = 85/thread (v75 fit in 80 → likely no spill).
// =============================================================================
// v73 = Br=32 + Bc=32 + FA_THREADS=64 + launch_bounds(_, 8). Used 123 regs, no spill.
// v75 ups bounds to 12 → reg cap = 65536/(12*64) = 85 regs/thread.
// SMEM still 12 KB × 12 = 144 KB > 100 → actual occupancy SMEM-limited at 8 blocks.
// So launch_bounds(_, 12) ONLY tightens reg cap without increasing real occupancy.
// Cost: -38 regs from v73's 123 → ~spill. Compared to v74's catastrophe at 42 regs.
// =============================================================================
...
// v76: launch_bounds(64, 12) for REAL 12 blocks/SM (SMEM cut to 8 KB/block via overlap).
__global__ void __launch_bounds__(FA_THREADS, 12)
    fa76_kernel(
        ...
        float qk_descale, float v_descale)
{
    ...
    // v76: 8 KB total = 4 KB Slot A (smQ → smV_T) + 4 KB Slot B (smK → smP).
    uint8_t *smQ = raw;             // = Slot A initially (used for Q load only)
    ...
    uint8_t *smV_T = raw;           // OVERLAPS Slot A (smQ slot reused after Q→Qr)
    uint8_t *smK   = raw + FA_BR * FA_STRIDE;  // Slot B starts at +4 KB
...
#pragma unroll
    ...
    // v76: SYNC needed — all threads must finish Qr loads from smQ before V load
    // (which writes to overlapping smV_T = smQ slot). Without sync, race condition.
    __syncthreads();

    // Pre-load: K via cp.async, V via direct LDG transposed into smV_T (= old smQ slot).
...
int main()
{
    ...
    printf("=== FA v76 — REAL 12 blocks/SM via smQ↔smV_T overlap (8 KB/block) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa76_kernel));
    printf("Kernel attrs: numRegs=%d  binSize=%d  sharedSizeBytes(static)=%zu\n",
           attr.numRegs, attr.binaryVersion, attr.sharedSizeBytes);
    int slotA_hd128 = (int)(FA_BR * FA_STRIDE > 128 * SMV_T_STRIDE
                            ? FA_BR * FA_STRIDE : 128 * SMV_T_STRIDE);
    int smem_hd128 = slotA_hd128 + FA_BC * FA_STRIDE;
    ...
    for (auto &c : configs)
    {
        ...
        // v76: 8 KB = 4KB Slot A (smQ→smV_T overlap) + 4KB Slot B (smK→smP)
        int slotA = max((int)(FA_BR * FA_STRIDE), (int)(hd * SMV_T_STRIDE));  // 4 KB
        int slotB = FA_BC * FA_STRIDE;                                          // 4 KB
        int smem = slotA + slotB;
        CK(cudaFuncSetAttribute(fa76_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa76_kernel<<<bh * nqt, FA_THREADS, smem>>>(
