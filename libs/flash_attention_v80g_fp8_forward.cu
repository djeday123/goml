...
// v80g: Br=80 with 5 compute warps. SMEM 34.5KB, 2 blocks/SM.
#define FA_BR 80                  // 5 warps × M_TILES=1 × 16 rows = 80
#define FA_BC 64
#define FA_THREADS 160            // 5 warps (was 4)
...
// v80g: launch_bounds(160, 2) → reg cap 204 for 2 blocks/SM (FA_THREADS=160).
// SMEM 34.5KB × 2 = 69KB ≤ 100KB. Br=80 with 5 compute warps.
...
int main()
{
    ...
    printf("=== FA v80g (Br=80, 5 compute warps) — EXPERIMENT: 4 blocks/SM via Br=64 + door 2 + single-K ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa80g_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        CK(cudaFuncSetAttribute(fa80g_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa80g_kernel<<<bh * nqt, FA_THREADS, smem>>>(
