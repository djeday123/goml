// =============================================================================
// FlashAttention v75 — STRESS: v73 base + launch_bounds(_, 12)
// =============================================================================
...
// v75: launch_bounds(64, 12) → reg cap = 65536/(12*64) = 85/thread (v73 was 128).
// SMEM still limits actual occupancy to 8 blocks/SM. Just tighter regs.
...
int main()
{
    ...
    printf("=== FA v75 STRESS — v73 base + launch_bounds(_, 12), 85 regs/thread ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa75_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        CK(cudaFuncSetAttribute(fa75_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa75_kernel<<<bh * nqt, FA_THREADS, smem>>>(
