...
#pragma unroll
        ...
        // v79c2: P-in-registers (CORRECT shfl pattern).
        // NO smP STS, NO 2 __syncthreads, NO smP LDS.
        // 4-lane quad-group shfl reconstructs Pr fragments from P_top/P_bot.
        //
        // CORRECT pattern: outer loop on nt_offset is UNIFORM across all lanes,
        // so each shfl call has all lanes providing the SAME P_top[ks*4+nt_off][mi] expression.
        // Each lane then selects whether this nt matches its own nt_cl or nt_ch.
        //
        // From v79b smP write pattern:
        //   - Writer (gid_w, tid_w) writes fp8x2_top at row=mrb+mi*16+gid_w, col0=nt*8+tid_w*2
        //   - For reader (gid_r, tid_r) reading cl=ks*32+tid_r*4, row=mrb+mi*16+gid_r:
        //     bytes [4*tid_r, 4*tid_r+3] of row gid_r = (tid_w=2*tid_r mod 4 nt=cl/8) bytes
        //   - For reader tid_r: nt_cl = ks*4 + (tid_r >> 1), src lanes:
        //     if tid_r & 1: (group_base+2, group_base+3)
        //     else:         (group_base+0, group_base+1)
        //   - For ch (col+16): same src lanes, nt_ch = nt_cl + 2
...
#pragma unroll
        for (int ks = 0; ks < 2; ks++)
        {
            int k_off = ks * 32;
            int cl = k_off + tid * 4, ch = cl + 16;
            int nt_cl_target = (tid >> 1);          // tid_r/2 in [0..1]
            int nt_ch_target = nt_cl_target + 2;     // [2..3]
...
#pragma unroll
            for (int mi = 0; mi < M_TILES; mi++)
            {
                // Iterate over 4 possible nt values (0..3 within this ks).
                // For each, do uniform-nt shfl. All lanes provide same expression.
                // Each lane picks if this nt matches its nt_cl_target or nt_ch_target.
#pragma unroll
                for (int nt_off = 0; nt_off < 4; nt_off++)
                {
                    int nt_idx = ks * 4 + nt_off;
                    // All lanes provide P_top[nt_idx][mi]:
                    uint32_t Pt_a = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_top[nt_idx][mi]), src_a);
                    uint32_t Pt_b = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_top[nt_idx][mi]), src_b);
                    uint32_t Pb_a = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_bot[nt_idx][mi]), src_a);
                    uint32_t Pb_b = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_bot[nt_idx][mi]), src_b);

                    // Quantize each FP16x2 → FP8x2 (uint16_t).
                    uint16_t fp_t_a = fp16x2_to_e4m3x2(Pt_a);
                    uint16_t fp_t_b = fp16x2_to_e4m3x2(Pt_b);
                    uint16_t fp_b_a = fp16x2_to_e4m3x2(Pb_a);
                    uint16_t fp_b_b = fp16x2_to_e4m3x2(Pb_b);

                    // Pack into b32 fragments.
                    uint32_t b32_top = (uint32_t)fp_t_a | ((uint32_t)fp_t_b << 16);
                    uint32_t b32_bot = (uint32_t)fp_b_a | ((uint32_t)fp_b_b << 16);

                    // Each lane uses this nt only if it matches its own nt_cl or nt_ch.
                    // Use predicated assigns (compiler will pick the right branch per lane).
                    if (nt_off == nt_cl_target) {
                        Pr[mi][0] = b32_top;  // cl, row g0
                        Pr[mi][1] = b32_bot;  // cl, row g8
                    }
                    if (nt_off == nt_ch_target) {
                        Pr[mi][2] = b32_top;  // ch, row g0
                        Pr[mi][3] = b32_bot;  // ch, row g8
                    }
                }
            }
#pragma unroll
...
int main()
{
    ...
    printf("=== FA v79c2 FP8 forward — P-in-regs (CORRECT shfl pattern) — v69 + V[N+1] mid-iter prefetch into dead smK[buf] ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa79c2_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa79c2_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa79c2_kernel<<<bh * nqt, FA_THREADS, smem>>>(
