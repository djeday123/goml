// =============================================================================
// FlashAttention v55 — Backward pass with tensor cores, FA2-style two-pass
//
// Major upgrade over the minimal-viable v54_backward:
//   • All five matmuls (S=QKᵀ, dP=dO·Vᵀ, dQ=dS·K, dV=Pᵀ·dO, dK=dSᵀ·Q) use
//     m16n8k16 tensor-core MMA instead of scalar FP32.
...
//   • SMEM tiles of Q, K, V, dO, double-buffered with cp.async.
//   • Reuses helpers (ldmatrix, swizzle, hexp2x2) from forward v54.
//
// Targets sm_80+ (works on Ampere/Ada/Hopper/Blackwell). Same fat-binary
// build conventions as the rest of libs/.
//
// Standalone test: nvcc -O3 -arch=sm_120 flash_attention_v55_backward.cu -o fa_v55_bw
        ...
        int seq_len, int head_dim, int causal, float scale)
{
    ...
    // SMEM layout:
    //   [0,            BR*STRIDE  )   Q tile (static)
    //   [BR*STRIDE,    2*BR*STRIDE)   dO tile (static)
    //   [2*BR*STRIDE,  +BC*STRIDE )   K tile (double-buffered idx 0)
    //   [...        ,  +BC*STRIDE )   K tile (double-buffered idx 1)
    //   [...        ,  +BC*STRIDE )   V tile (double-buffered idx 0)
    //   [...        ,  +BC*STRIDE )   V tile (double-buffered idx 1)
    __half *smQ = (__half *)raw;
    __half *smdO = smQ + FA_BR * FA_STRIDE;
    __half *smK0 = smdO + FA_BR * FA_STRIDE;
    __half *smK1 = smK0 + FA_BC * FA_STRIDE;
    __half *smV0 = smK1 + FA_BC * FA_STRIDE;
    __half *smV1 = smV0 + FA_BC * FA_STRIDE;
    ...
    // Stage Q and dO once (resident the whole kernel).
    ...
    // Load Q and dO into register fragments (resident).
    int mrb = wid * 16;
...
#pragma unroll
    ...
    load_tile(smK0, Kh, 0, FA_BC, seq_len, head_dim);
    load_tile(smV0, Vh, 0, FA_BC, seq_len, head_dim);
    cpa_commit();
    ...
    {
        int kvs = kv * FA_BC;
        __half *smK = (kv & 1) ? smK1 : smK0;
        __half *smV = (kv & 1) ? smV1 : smV0;
        __half *smKn = (kv & 1) ? smK0 : smK1;
        __half *smVn = (kv & 1) ? smV0 : smV1;
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_sw(b0, b1, smK, FA_STRIDE, nt * 8, ks * 16, lane);
...
#pragma unroll
        ...
        // Schedule next K, V loads now to overlap with compute.
        if (kv + 1 < kv_max_blocks)
        {
            load_tile(smKn, Kh, (kv + 1) * FA_BC, FA_BC, seq_len, head_dim);
            load_tile(smVn, Vh, (kv + 1) * FA_BC, FA_BC, seq_len, head_dim);
        }
        cpa_commit();
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_sw(b0, b1, smV, FA_STRIDE, nt * 8, ks * 16, lane);
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_vt(b0, b1, smK, FA_STRIDE, nt * 8, ks * 16, lane);
        ...
        int seq_len, int head_dim, int causal, float scale)
{
    ...
    //   K tile (static)
    //   V tile (static)
    //   Q0 / Q1 (double-buffered)
    //   dO0 / dO1 (double-buffered)
    __half *smK = (__half *)raw;
    __half *smV = smK + FA_BC * FA_STRIDE;
    __half *smQ0 = smV + FA_BC * FA_STRIDE;
    __half *smQ1 = smQ0 + FA_BR * FA_STRIDE;
    __half *smdO0 = smQ1 + FA_BR * FA_STRIDE;
    __half *smdO1 = smdO0 + FA_BR * FA_STRIDE;
...
#pragma unroll
    ...
    // Prefetch first Q, dO tiles.
    load_tile(smQ0, Qh, q_start * FA_BR, FA_BR, seq_len, head_dim);
    load_tile(smdO0, dOh, q_start * FA_BR, FA_BR, seq_len, head_dim);
    cpa_commit();
    ...
    {
        int qs = qb * FA_BR;
        int buf = (qb - q_start) & 1;
        __half *smQ = buf ? smQ1 : smQ0;
        __half *smdO = buf ? smdO1 : smdO0;
        __half *smQn = buf ? smQ0 : smQ1;
        __half *smdOn = buf ? smdO0 : smdO1;
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_sw(b0, b1, smQ, FA_STRIDE, nt * 8, ks * 16, lane);
                mma16816(Sr[nt][0], Sr[nt][1], Sr[nt][2], Sr[nt][3],
...
#pragma unroll
        ...
        // Schedule next Q, dO loads.
        if (qb + 1 < nq)
        {
            load_tile(smQn, Qh, (qb + 1) * FA_BR, FA_BR, seq_len, head_dim);
            load_tile(smdOn, dOh, (qb + 1) * FA_BR, FA_BR, seq_len, head_dim);
        }
        cpa_commit();
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_vt(b0, b1, smdO, FA_STRIDE, nt * 8, ks * 16, lane);
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_sw(b0, b1, smdO, FA_STRIDE, nt * 8, ks * 16, lane);
                mma16816(dPr[nt][0], dPr[nt][1], dPr[nt][2], dPr[nt][3],
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_vt(b0, b1, smQ, FA_STRIDE, nt * 8, ks * 16, lane);
...
#pragma unroll
    for (int nt = 0; nt < 16; nt++)
    {
        ...
        if (gr0 < seq_len && c1 < head_dim)
        {
            dKh[gr0 * head_dim + c1] = __float2half(dKr[nt][1]);
            dVh[gr0 * head_dim + c1] = __float2half(dVr[nt][1]);
        }
        if (gr8 < seq_len && c0 < head_dim)
        {
        ...
        if (gr8 < seq_len && c1 < head_dim)
        {
            dKh[gr8 * head_dim + c1] = __float2half(dKr[nt][3]);
            dVh[gr8 * head_dim + c1] = __float2half(dVr[nt][3]);
        }
    }
}
...
extern "C" void launch_v55_backward_dq(
    ...
    int th, int sl, int hd, int ca)
{
    ...
    if (smem > g_smem_dq)
    {
        cudaFuncSetAttribute(fa55_backward_dq_kernel,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
        g_smem_dq = smem;
    }

    dim3 grid(th, nqt);
    fa55_backward_dq_kernel<<<grid, FA_THREADS, smem>>>(
...
extern "C" void launch_v55_backward_dkdv(
    const __half *Q, const __half *K, const __half *V,
    const __half *dO, const float *LSE, const float *D,
    ...
    int th, int sl, int hd, int ca)
{
    ...
    if (smem > g_smem_dkdv)
    {
        cudaFuncSetAttribute(fa55_backward_dkdv_kernel,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
        g_smem_dkdv = smem;
    }

    dim3 grid(th, nkv);
    fa55_backward_dkdv_kernel<<<grid, FA_THREADS, smem>>>(
...
extern "C" void launch_v55_backward(
    const __half *Q, const __half *K, const __half *V,
    ...
    int th, int sl, int hd, int ca)
{
    ...
    launch_v55_backward_dq(Q, K, V, dO, LSE, D_scratch, dQ_out, th, sl, hd, ca);
    launch_v55_backward_dkdv(Q, K, V, dO, LSE, D_scratch, dK_out, dV_out, th, sl, hd, ca);
}
...
void test_correctness()
{
    ...
    for (auto &c : configs)
    {
        ...
        launch_v55_backward_dq(Q_d, K_d, V_d, dO_d, LSE_d, D_d, dQ_d, th, sl, hd, ca);
        launch_v55_backward_dkdv(Q_d, K_d, V_d, dO_d, LSE_d, D_d, dK_d, dV_d, th, sl, hd, ca);
...
int main()
{
    ...
    printf("=== FlashAttention v55 backward — full Tri Dao-style two-pass ===\n");
