...
// Runs ONE warmup + ONE measured launch of fa89v_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa89v_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        // warmup (NCu skips with --launch-skip)
        fa89v_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        // measured (NCu profiles this one)
        fa89v_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa89v_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa89v_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa89v_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v89 hd=64 VALIDATION on large grids (bh=64, bh=128) ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa89v_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa89v_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        fa89v_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
        ...
        CK(cudaFuncSetAttribute(fa89v_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa89v_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    // -------------------------------------------------------------------------
    printf("\n--- Phase 2: VALIDATION on large production-scale grids ---\n");
    printf("Goal: confirm v89 (413T) and LB=3 +27%% wave-reduction hold at bh=64, bh=128\n\n");
    int bench_configs[][4] = {
        // Reference configs (already measured in v89 final bench)
        {16, 4096, 64, 0},   // grid 512, was champion WIN (413T) — re-measure for variance check
        {32, 2048, 64, 0},   // grid 512, WIN (389T)
        {8, 8192, 64, 1024}, // grid 512, WIN sliding window (280T)
        // === NEW: bh=64 validation (production batch×heads) ===
        {64, 1024, 64, 0},   // grid 512  (LB=3 wave 0.91 — boundary win region)
        {64, 2048, 64, 0},   // grid 1024 (LB=3 wave 1.81 — multi-wave)
        {64, 4096, 64, 0},   // grid 2048 (LB=3 wave 3.63 — many waves)
        {64, 8192, 64, 0},   // grid 4096 (LB=3 wave 7.26)
        // === NEW: bh=128 validation (extreme batch×heads) ===
        {128, 1024, 64, 0},  // grid 1024 (LB=3 wave 1.81)
        {128, 2048, 64, 0},  // grid 2048 (LB=3 wave 3.63)
        {128, 4096, 64, 0},  // grid 4096 (LB=3 wave 7.26)
        // Production sliding window at scale (Gemma3-style)
        {64, 4096, 64, 1024},
        {64, 8192, 64, 1024},
        {128, 4096, 64, 1024},
    };
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa89v_kernel<2>);
            r3[v] = measure(fa89v_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
