...
// v80d: producer-warp-only cp.async (warp 0). 32 threads do 4× more iters.
...
#pragma unroll
    ...
    // v80d: producer-warp-only cp.async. All 4 warps still do compute.
    if (wid == 0) {
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v80d: K[kv+1] cp.async — producer warp only.
        int kv_p = kv + 1;
...
#pragma unroll
        ...
        // v80d: V[kv+1] cp.async — producer warp only.
...
int main()
{
    ...
    printf("=== FA v80d FP8 forward — Br=128 + concentrated cp.async (Interpretation B) — v69 + V[N+1] mid-iter prefetch into dead smK[buf] ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa80d_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa80d_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa80d_kernel<<<bh * nqt, FA_THREADS, smem>>>(
