// =============================================================================
// FlashAttention v44 — exp2f Softmax + FFMA Fusion (SM89)
// =============================================================================
// Based on v20 (151T, 92%). Changes:
//   1. exp2f() instead of __expf() — single MUFU.EX2, no mul+fixup
//   2. Fused scale: pre-multiply scale * M_LOG2E, use ffma(x, scale_log2, -max_scaled)
//      This merges scaling + subtraction into one ffma before exp2f.
//   3. Same approach as FA2 softmax.h
//
// Before (v20):
//   Sr[nt][0] *= scale;              // fmul
//   Sr[nt][0] = __expf(Sr[nt][0] - rmax[0]);  // fsub + fmul(log2e) + MUFU.EX2 + fixup
//   Total: ~4-5 instructions per element
//
// After (v44):
//   Sr[nt][0] = exp2f(Sr[nt][0] * scale_log2 - max_scaled);  // ffma + MUFU.EX2
//   Total: 2 instructions per element (ffma + EX2)
//
// For 16 elements per thread × 2 rows = 32 exp operations per softmax.
// Saving ~2-3 instructions each = 64-96 fewer instructions.
// =============================================================================
...
#define FA44_BR 64
#define FA44_BC 64
#define FA44_THREADS 128
#define FA44_STRIDE 128
    ...
    int seq_len, int head_dim)
{
    ...
    int total = rows * CPR;
#pragma unroll 4
    for (int c = threadIdx.x; c < total; c += FA44_THREADS)
    {
        int row = c / CPR, lch = c % CPR, gr = start + row;
        int pch = lch ^ (row & 7);
        cpa16(&dst[row * FA44_STRIDE + pch * 8],
              &src[gr * head_dim + lch * 8], (gr < seq_len) ? 16 : 0);
    }
}

// =============================================================================
__global__ void __launch_bounds__(FA44_THREADS, 2)
    flash_attention_v44_kernel(
        const __half *__restrict__ Q, const __half *__restrict__ K,
        const __half *__restrict__ V, __half *__restrict__ O,
        int seq_len, int head_dim, int causal, float scale)
{
    int nqt = (seq_len + FA44_BR - 1) / FA44_BR;
    int bh = blockIdx.x / nqt, qt = blockIdx.x % nqt, qs = qt * FA44_BR;
    int wid = threadIdx.x / 32, lane = threadIdx.x % 32;
    int gid = lane >> 2, tid = lane & 3;

    extern __shared__ char raw[];
    __half *buf0 = (__half *)raw;
    __half *buf1 = (__half *)(raw + FA44_BC * FA44_STRIDE * sizeof(__half));

    int hs = seq_len * head_dim;
    const __half *Qh = Q + bh * hs, *Kh = K + bh * hs, *Vh = V + bh * hs;
    __half *Oh = O + bh * hs;

    // Pre-compute exp2f scale: scale * log2(e)
    float scale_log2 = scale * 1.4426950408889634f; // M_LOG2E

    // Load Q -> registers
    load_tile(buf0, Qh, qs, FA44_BR, seq_len, head_dim);
    cpa_commit();
    cpa_wait<0>();
    __syncthreads();
...
#pragma unroll
    for (int ks = 0; ks < 8; ks++)
        ld_a_sw(Qr[ks][0], Qr[ks][1], Qr[ks][2], Qr[ks][3],
                buf0, FA44_STRIDE, mrb, ks * 16, lane);
    __syncthreads();
...
#pragma unroll
    ...
    int nkv = (seq_len + FA44_BC - 1) / FA44_BC;

    load_tile(buf0, Kh, 0, FA44_BC, seq_len, head_dim);
    cpa_commit();

    for (int kv = 0; kv < nkv; kv++)
    {
        int kvs = kv * FA44_BC;
        if (causal && kvs > qs + FA44_BR - 1)
            break;
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_sw(b0, b1, cur, FA44_STRIDE, nt * 8, ks * 16, lane);
        ...
        __syncthreads();
        load_tile(cur, Vh, kvs, FA44_BC, seq_len, head_dim);
        cpa_commit();
        int nkvs = (kv + 1) * FA44_BC;
        bool has_nxt = (kv + 1 < nkv) && (!causal || nkvs <= qs + FA44_BR - 1);
        if (has_nxt)
            load_tile(nxt, Kh, nkvs, FA44_BC, seq_len, head_dim);
        cpa_commit();

        // Causal mask (BEFORE scaling — mask with large negative)
        if (causal)
        {
#pragma unroll
            for (int nt = 0; nt < 8; nt++)
        ...
        // ==== Online Softmax with exp2f + ffma ====
        // Find row max (on raw scores, not scaled)
...
#pragma unroll
        ...
        // Rescale O with exp2f
        // rsc = exp(rmax_old - nm_new) = exp2f((rmax_old - nm_new) * log2e)
        float rsc0 = exp2f((rmax[0] - nm[0]) * 1.4426950408889634f);
        float rsc1 = exp2f((rmax[1] - nm[1]) * 1.4426950408889634f);
#pragma unroll
        for (int t = 0; t < 16; t++)
        {
            Or[t][0] *= rsc0;
        ...
        // Pre-compute max * scale_log2 for ffma: exp2f(x * scale_log2 - max_scaled)
        float max_scaled0 = nm[0] * scale_log2;
        float max_scaled1 = nm[1] * scale_log2;
...
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
            // exp2f(x * scale_log2 - max_scaled) = exp(scale * (x - max))
            // ffma can fuse: x * scale_log2 - max_scaled into one instruction
            Sr[nt][0] = exp2f(Sr[nt][0] * scale_log2 - max_scaled0);
            Sr[nt][1] = exp2f(Sr[nt][1] * scale_log2 - max_scaled0);
            Sr[nt][2] = exp2f(Sr[nt][2] * scale_log2 - max_scaled1);
            Sr[nt][3] = exp2f(Sr[nt][3] * scale_log2 - max_scaled1);
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_vt(b0, b1, cur, FA44_STRIDE, nt * 8, ks * 16, lane);
...
// =============================================================================
static int g_FA44_smem = 0;
extern "C"
{
    int flash_attention_v44_forward(
        const void *Q, const void *K, const void *V, void *O,
        int total_heads, int seq_len, int head_dim, int causal, void *stream)
    {
        if (head_dim != 128)
            return -1;
        int smem = 2 * FA44_BC * FA44_STRIDE * (int)sizeof(__half);
        if (smem > g_FA44_smem)
        {
            cudaError_t e = cudaFuncSetAttribute(flash_attention_v44_kernel,
        ...
        int nqt = (seq_len + FA44_BR - 1) / FA44_BR;
        flash_attention_v44_kernel<<<total_heads * nqt, FA44_THREADS, smem, (cudaStream_t)stream>>>(
