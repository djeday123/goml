// =============================================================================
// FlashAttention v107 — v96 + persistent (grid-stride) kernel
// =============================================================================
// Goal: wave-tail equalization. v96 launches grid = bh × nqt which can leave
// the last wave under-occupied (e.g. grid=512, 376 SMs×2 blocks → wave 1 = 376
// blocks 100%, wave 2 = 136 blocks 36% SM efficiency).
// v107 launches exactly min(total_tiles, 2 × SMs) blocks. Each block processes
// multiple Q-tiles via outer grid-stride loop. No wave tail.
//
// Expected: +3-10% on mid grids (bh=16 sl=4096, bh=8 sl=8192) where wave-tail
// loss is significant. Negligible on large grids (already wave-saturated).
//
// Risks: outer loop may add 1-3 regs (loop counter, index recompute) → may
// push v96's 242 reg count above 256 budget = breaks 2 blocks/SM.
//
// All kernel internals identical to v96. Only launch-site + kernel-prologue/
// epilogue wrapped in `for (tile_id = blockIdx.x; tile_id < total_tiles; ...)`.
// =============================================================================
//
// (original v69 docs follow)
// =============================================================================
...
__global__ void __launch_bounds__(FA_THREADS, 2)
    fa107_kernel(
        ...
        int window,
        int total_tiles)  // v107: persistent — total Q-tile count across all (bh, qt).
{
    ...
    // v107 PERSISTENT outer loop: grid-stride over (bh, qt) tile pairs.
    // Each block processes one tile per iter, then advances by gridDim.x.
    // Wave-tail equalization: launch grid = min(total_tiles, 2 × SMs).
    ...
    // v107: sync at top of each Q-tile to ensure prev iter's PV phase is done
    // (epilogue writes to gmem only, but warps may still be inside kv-loop tail).
    // First iter (tile_id == blockIdx.x): unnecessary but cheap.
    __syncthreads();
...
#pragma unroll
    ...
    } // v107: end of grid-stride outer loop
}

#define CK(c) do { cudaError_t e = (c); if (e != cudaSuccess) { \
    fprintf(stderr, "CUDA %s:%d: %s\n", __FILE__, __LINE__, cudaGetErrorString(e)); exit(1); }} while(0)

// v107: persistent grid sizing. Launch min(total_tiles, 2 × SMs) blocks so every
// SM gets exactly 2 blocks (matches launch_bounds LB=2). No wave tail.
static int g_sm_count = 0;
static inline int persistent_grid(int total_tiles) {
    if (g_sm_count == 0) {
        cudaDeviceProp p; cudaGetDeviceProperties(&p, 0);
        g_sm_count = p.multiProcessorCount;
    }
    int max_blocks = g_sm_count * 2;
    return (total_tiles < max_blocks) ? total_tiles : max_blocks;
}
...
// Runs ONE warmup + ONE measured launch of fa107_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    int total_tiles = bh * nqt;
    int grid = persistent_grid(total_tiles);
    float scale = 1.0f / sqrtf((float)hd);
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d total_tiles=%d persistent_grid=%d\n",
            bh, sl, hd, wnd, total_tiles, grid);
    CK(cudaFuncSetAttribute(fa107_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa107_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd, total_tiles);
...
static int loop_mode(int cfg_idx, int seconds)
{
    ...
    CK(cudaFuncSetAttribute(fa107_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fprintf(stderr, "Loop: cfg=%d bh=%d sl=%d wnd=%d total_tiles=%d persistent_grid=%d, running for %d sec\n",
            cfg_idx, bh, sl, wnd, total_tiles, grid, seconds);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v107 hd=128 = v96 + persistent (grid-stride) kernel for wave-tail equalization ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa107_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int grid = persistent_grid(total_tiles);
        float scale = 1.0f / sqrtf((float)hd);
        fa107_kernel<<<grid, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd, total_tiles);
        CK(cudaDeviceSynchronize());
    ...
    for (auto &c : bench_configs)
    {
        ...
        for (int i = 0; i < 5; i++)
            fa107_kernel<<<grid, FA_THREADS, smem>>>(
                Q_d, K_d, V_d, O_d, sl, hd, ca_bench, scale, 1.0f, 1.0f, wnd, total_tiles);
        CK(cudaDeviceSynchronize());
