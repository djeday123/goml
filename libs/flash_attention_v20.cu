// =============================================================================
// FlashAttention v10 — Register-based S/P (gau-nernst / FA2 approach)
// =============================================================================
// Q permanent in registers. S/P entirely in registers. No S_s shared memory.
// Softmax: 4-thread butterfly (not full warp reduce).
// Br=64, Bc=64, D=128, 4 warps (128 threads), occupancy hint=2.
// SMEM: 32KB = 2 × 16KB double-buffer (no S_s, no m/l smem).
// =============================================================================
...
// B from swizzled SMEM transposed (V)
...
#pragma unroll
    ...
    for (int kv = 0; kv < nkv; kv++)
    {
        ...
        // ============================================================
        // QK^T: Q from Qr (regs), K from cur (shared)
        // 16 rows × 64 cols = 8 n-tiles, 8 k-steps
        // ============================================================
        float Sr[8][4];
#pragma unroll
...
#pragma unroll
        ...
        // Row max: reduce across 8 n-tiles, then 4-thread butterfly
        // ============================================================
...
#pragma unroll
        ...
        // Butterfly across 4 threads sharing same row (XOR 1, 2)
...
#pragma unroll
        ...
        uint32_t Pr[4][4]; // [Bc/16][4] packed P for 2nd MMA
...
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
            ...
            // Pack two m16n8 tiles → one m16k16 A-tile
            // S[nt=2i] → left half, S[nt=2i+1] → right half
        ...
        // Butterfly sum across 4 threads
        ...
        // FMA: accumulate sumexp
        rsexp[0] = rsexp[0] * rsc0 + ns[0];
        rsexp[1] = rsexp[1] * rsc1 + ns[1];

        // ============================================================
        // Wait for V[kv], then PV: O += P @ V
        // P from Pr (regs), V from cur (shared via ldmatrix)
        // 16 rows × 128 D-cols = 16 n-tiles, 4 k-steps (Bc/16)
        // ============================================================
...
// =============================================================================
static int g_FA20_smem = 0;

extern "C"
{
    int flash_attention_v20_forward(
        ...
        int causal, void *stream)
    {
        ...
        // 2 buffers × Bc × stride × sizeof(half)
        int smem = 2 * FA20_BC * FA20_STRIDE * (int)sizeof(__half);

        if (smem > g_FA20_smem)
        {
            cudaError_t e = cudaFuncSetAttribute(flash_attention_v20_kernel,
        ...
        flash_attention_v20_kernel<<<total_heads * nqt, FA20_THREADS, smem,
                                     (cudaStream_t)stream>>>(
