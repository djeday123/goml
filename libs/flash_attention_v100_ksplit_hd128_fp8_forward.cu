...
// v100: load only ROWS [row_start, row_end) of K tile. For split cp.async.
__device__ __forceinline__ void load_tile_fp8_range(
    uint8_t *dst, const uint8_t *src, int start, int row_start, int row_end,
    int seq_len, int head_dim)
{
    constexpr int CHUNK = 16;
    int chunks_per_row = head_dim / CHUNK;
...
#pragma unroll 4
    for (int c = threadIdx.x; c < total; c += FA_THREADS)
    {
        int row = row_start + c / chunks_per_row;
        int col_bytes = (c % chunks_per_row) * CHUNK;
        int gr = start + row;
        int dst_off = swz_byte(row, col_bytes);
        cpa16(&dst[dst_off], &src[gr * head_dim + col_bytes], (gr < seq_len) ? 16 : 0);
...
#pragma unroll
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v100: SPLIT K[kv+1] cp.async into 2 commits (half-tile each).
        // Goal: see if separate K groups (rows 0..31 and 32..63) change long_scb
        // behaviour. NCu will show if scheduler/HBM differ.
        int rows_p_half = rows_p / 2;
        load_tile_fp8_range(nxt_K, Kh, kv_p * FA_BC, 0, rows_p_half, seq_len, head_dim);
        cpa_commit();
        load_tile_fp8_range(nxt_K, Kh, kv_p * FA_BC, rows_p_half, rows_p, seq_len, head_dim);
        cpa_commit();
...
// Runs ONE warmup + ONE measured launch of fa100_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa100_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa100_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v100 hd=128 + K cp.async split (2 commits per K[kv+1] = 3 groups/iter) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa100_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa100_kernel<<<bh * nqt, FA_THREADS, smem>>>(
    ...
    int bench_configs[][4] = {
        ...
        {16, 4096, 128, 0},
        {32, 2048, 128, 0},
        // Large-grid validation (parity with hd=64 v89 large bench):
        {64, 4096, 128, 0},
        {64, 8192, 128, 0},    // hd=64 peak lived at this shape (466T)
        {128, 2048, 128, 0},
        {128, 4096, 128, 0},
        ...
        {16, 8192, 128, 1024}, // larger sliding window
    };
    for (auto &c : bench_configs)
    {
        ...
        printf("  bh=%-3d sl=%-4d hd=%d ca=%d wnd=%-4d  best=%.2f med=%.2f worst=%.2f  mean=%.2f sd=%.2f  RAW:",
               bh, sl, hd, ca_bench, wnd,
               tf_runs[VARIANCE_RUNS-1], tf_runs[VARIANCE_RUNS/2], tf_runs[0], mean, sd);
        for (int v = 0; v < VARIANCE_RUNS; v++) printf(" %.2f", tf_runs[v]);
        printf("\n");
        cudaFree(Q_d); cudaFree(K_d); cudaFree(V_d); cudaFree(O_d);
    }
