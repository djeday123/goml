// =============================================================================
// FlashAttention v86 — FP8 Forward, hd=64 + Option C tree reductions
// =============================================================================
// v85 base. Two no-reg-cost loop transformations:
//   1. Phase B rmax: sequential fmaxf chain (8 deep) → log2 tree (3 deep)
//   2. Phase E2 ns sum: sequential add chain (8 deep) → log2 tree
//
// All cost ZERO registers (compiler may use extra temps but no extra arrays).
// Targets wait 27% via reduced dep chain depth in softmax. Honest test:
//   if wait drops + perf rises → Phase B/E2 chains were drivers, copy tactic
//   if wait unchanged → softmax chains not material, true source is MMA chain
//
// =====  v85 history (kept for reference)  =====
// v81 base. ONE change: split softmax Phase E into two loops.
//
// v81 structure (RAW dep chain forces serialization):
//   for nt: for mi: compute_d → ex2_top → ex2_bot → store P_top/P_bot → ns += sum
// Each iter has ex2 → P → ns chain inside, so compiler keeps the 16 ex2 calls
// sequential, ~12 cycle latency × 16 = ~190 cycle critical path.
//
// v85 structure:
//   Phase E1: for nt: for mi: compute_d + issue both ex2 + store P_top/P_bot
//   Phase E2: for nt: for mi: ns[mi][k] += sum(P_top, P_bot)  [separate loop]
// Phase E1 has no inter-iter ns dependency → scheduler can pipeline 16 ex2 calls
// (issue one per cycle, drain in parallel). Phase E2 reads already-stored P_X.
//
// NCu target (validated stall composition in v81 WIN LB=3):
//   wait = 28.40%  (fixed-latency: MMA single-step + ex2.approx.f16x2 deps)
//   Expected wait drop: 28.40% → 15-20% after ILP refactor
//   Expected perf gain: +5-10% in WIN regime (grid > 376)
//
// Register cost: ZERO. P_top[8][M_TILES] and P_bot[8][M_TILES] are already live
// for the subsequent smP store phase. v85 just changes the order of operations
// on the same data.
//
...
#pragma unroll
        for (int mi = 0; mi < M_TILES; mi++)
        {
            // v86 lever 1: tree reduction (log2(8)=3 depth) instead of sequential chain.
            // Each Sr[nt][mi] pair max is independent → 8 leaf maxes parallel.
            // Then 4 → 2 → 1 reduction with RAW dep only between levels (3 deep).
            float l0[8], l1[8];  // leaf maxes (independent)
#pragma unroll
            for (int nt = 0; nt < 8; nt++) {
                l0[nt] = fmaxf(Sr[nt][mi][0], Sr[nt][mi][1]);
                l1[nt] = fmaxf(Sr[nt][mi][2], Sr[nt][mi][3]);
            }
            // Level 1: 8 → 4
            float a0 = fmaxf(l0[0], l0[1]); float a1 = fmaxf(l0[2], l0[3]);
            float a2 = fmaxf(l0[4], l0[5]); float a3 = fmaxf(l0[6], l0[7]);
            float b0 = fmaxf(l1[0], l1[1]); float b1 = fmaxf(l1[2], l1[3]);
            float b2 = fmaxf(l1[4], l1[5]); float b3 = fmaxf(l1[6], l1[7]);
            // Level 2: 4 → 2
            float c0 = fmaxf(a0, a1); float c1 = fmaxf(a2, a3);
            float d0 = fmaxf(b0, b1); float d1 = fmaxf(b2, b3);
            // Level 3: 2 → 1
            nm[mi][0] = fmaxf(c0, c1);
            nm[mi][1] = fmaxf(d0, d1);
...
#pragma unroll
        ...
        // v85 ILP softmax: separate ex2 issue loop from ns accumulation.
        // v81 had RAW dep ex2 → P_X → ns_+= within each iteration → compiler serialized
        // 16 ex2 calls × ~12cyc latency = ~190 cycle critical path.
        // After split: Phase E1 issues all 16 ex2 back-to-back (scheduler pipelines them),
        // Phase E2 reads P_X (already in registers) and accumulates ns separately.
        // Target: wait stall 28.40% → 15-20% in WIN LB=3, +5-10% perf.
        __half2 P_top[8][M_TILES], P_bot[8][M_TILES];

        // Phase E1: compute d + issue ALL ex2.approx.f16x2 (no ns dep)
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
#pragma unroll
        ...
        // Phase E2: v86 lever 2 — tree reduction for ns sum (log2(8)=3 depth).
        // Compute per-(nt, mi) partial = low + high in parallel (no chain).
        // Then 8 → 4 → 2 → 1 add tree reduces dep depth.
        float ns[M_TILES][2];
#pragma unroll
        for (int mi = 0; mi < M_TILES; mi++)
        {
            float p0[8], p1[8];  // partial sums per nt, independent
#pragma unroll
            for (int nt = 0; nt < 8; nt++) {
                p0[nt] = __low2float(P_top[nt][mi]) + __high2float(P_top[nt][mi]);
                p1[nt] = __low2float(P_bot[nt][mi]) + __high2float(P_bot[nt][mi]);
            }
            // 8 → 4
            float a0 = p0[0]+p0[1]; float a1 = p0[2]+p0[3];
            float a2 = p0[4]+p0[5]; float a3 = p0[6]+p0[7];
            float b0 = p1[0]+p1[1]; float b1 = p1[2]+p1[3];
            float b2 = p1[4]+p1[5]; float b3 = p1[6]+p1[7];
            // 4 → 2 → 1
            ns[mi][0] = (a0+a1) + (a2+a3);
            ns[mi][1] = (b0+b1) + (b2+b3);
        }
...
// Runs ONE warmup + ONE measured launch of fa86_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa86_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        // warmup (NCu skips with --launch-skip)
        fa86_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        // measured (NCu profiles this one)
        fa86_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa86_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa86_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa86_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v86 hd=64 + tree-reduced rmax+ns — Option C softmax chain attack ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa86_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa86_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        fa86_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
        ...
        CK(cudaFuncSetAttribute(fa86_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa86_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa86_kernel<2>);
            r3[v] = measure(fa86_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
