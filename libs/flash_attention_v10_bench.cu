...
extern "C"
{
    ...
    int flash_attention_v7_forward(const void *, const void *, const void *, void *,
                                   int, int, int, int, void *);
    int flash_attention_v10_forward(const void *, const void *, const void *, void *,
                                    int, int, int, int, void *);
}
...
{
    printf("--- Correctness (v10 vs v1-scalar) ---\n");
    struct
    {
        int h, s, d;
    } cfgs[] = {
    ...
    for (auto &c : cfgs)
    {
        ...
        flash_attention_v10_forward(dQ, dK, dV, dT, c.h, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());
        ...
        printf("  %dh×%ds×%dd  maxabs=%.4f err=%d -> %s\n",
...
void bench()
{
    printf("\n--- Performance: v7 (Br=64,8w) vs v10 (Br=128,16w) ---\n");
    printf("%-12s %10s %10s  %6s  %6s\n", "Config", "v7", "v10", "ratio", "%%peak");
    printf("---------------------------------------------------------\n");
    ...
    for (auto &c : cfgs)
    {
        ...
        for (int i = 0; i < 3; i++)
            flash_attention_v7_forward(dQ, dK, dV, dO, hds, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());
        t.start();
        for (int i = 0; i < it; i++)
            flash_attention_v7_forward(dQ, dK, dV, dO, hds, c.s, c.d, 1, nullptr);
        float m7 = t.stop();
        double t7 = flops / (m7 / it / 1e3) / 1e12;

        CK(cudaMemset(dO, 0, bytes));
        for (int i = 0; i < 3; i++)
            flash_attention_v10_forward(dQ, dK, dV, dO, hds, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());
        t.start();
        for (int i = 0; i < it; i++)
            flash_attention_v10_forward(dQ, dK, dV, dO, hds, c.s, c.d, 1, nullptr);
        float m10 = t.stop();
        double t10 = flops / (m10 / it / 1e3) / 1e12;

        printf("%-12s %8.2f T %8.2f T  %5.2fx  %5.1f%%\n",
               c.l, t7, t10, t10 / t7, t10 / pk * 100);
...
int main()
{
    ...
    printf("=== FlashAttention v10 (Br=128, 16 warps, 8M×2N) ===\n");
