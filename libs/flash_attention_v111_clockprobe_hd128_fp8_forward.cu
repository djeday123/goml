...
__global__ void __launch_bounds__(FA_THREADS, 2)
    fa111_kernel(
        ...
        unsigned long long *clk_dbg_before,    // [block_id*8 + warp_id] - clock right BEFORE top sync
        unsigned long long *clk_dbg_after,     // [block_id*8 + warp_id] - clock right AFTER top sync
        int probe_iter)                        // which kv iter to probe (-1 disables)
{
...
#pragma unroll
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        cpa_wait<0>();
        // CLOCK PROBE: sample BEFORE top sync (per-warp lane 0)
        if (clk_dbg_before && kv == probe_iter && lane == 0) {
            clk_dbg_before[blockIdx.x * 8 + wid] = clock64();
        }
        __syncthreads();
        // CLOCK PROBE: sample AFTER top sync (per-warp lane 0)
        if (clk_dbg_after && kv == probe_iter && lane == 0) {
            clk_dbg_after[blockIdx.x * 8 + wid] = clock64();
        }
...
static int ncu_mode(int cfg_idx)
{
    ...
    fa111_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd, nullptr, nullptr, -1);
...
// =================== CLOCK PROBE MODE =====================
// Measure per-warp arrival skew at top-of-iter __syncthreads in v111.
// Hypothesis: barrier 8.6% stall = fast warps waiting slow warps.
// Test: clock64() before AND after top sync at probe_iter; compute
//   skew_before = max(clk_before) - min(clk_before)  → arrival skew (the cause)
//   skew_after  = max(clk_after) - min(clk_after)    → post-sync alignment (sanity)
//   wait_per_warp = clk_after[w] - clk_before[w]     → how long each warp waited
static int clkprobe_mode(int cfg_idx, int probe_iter_arg)
{
    // cfg=9 equivalent: bh=64 sl=8192
    int bh = 64, sl = 8192, hd = 128, ca = 0, wnd = 0;
    if (cfg_idx == 2)      { bh = 8;  sl = 2048; }
    else if (cfg_idx == 6) { bh = 16; sl = 4096; }
    else if (cfg_idx == 9) { bh = 64; sl = 8192; }
    int probe_iter = (probe_iter_arg >= 0) ? probe_iter_arg : 5;

    size_t n_elems = (size_t)bh * sl * hd;
    uint8_t *Q_h = (uint8_t *)malloc(n_elems);
    uint8_t *K_h = (uint8_t *)malloc(n_elems);
    uint8_t *V_h = (uint8_t *)malloc(n_elems);
    srand(42);
    for (size_t i = 0; i < n_elems; i++) {
        Q_h[i] = float_to_e4m3(((float)(rand() % 16) - 8.0f) * 0.125f);
        K_h[i] = float_to_e4m3(((float)(rand() % 16) - 8.0f) * 0.125f);
        V_h[i] = float_to_e4m3(((float)(rand() % 16) - 8.0f) * 0.125f);
    }
    uint8_t *Q_d, *K_d, *V_d; __half *O_d;
    CK(cudaMalloc(&Q_d, n_elems)); CK(cudaMalloc(&K_d, n_elems));
    CK(cudaMalloc(&V_d, n_elems)); CK(cudaMalloc(&O_d, n_elems * 2));
    CK(cudaMemcpy(Q_d, Q_h, n_elems, cudaMemcpyHostToDevice));
    CK(cudaMemcpy(K_d, K_h, n_elems, cudaMemcpyHostToDevice));
    CK(cudaMemcpy(V_d, V_h, n_elems, cudaMemcpyHostToDevice));
    CK(cudaMemset(O_d, 0, n_elems * 2));

    int nqt = (sl + FA_BR - 1) / FA_BR;
    int grid = bh * nqt;
    size_t dbg_n = (size_t)grid * 8;
    unsigned long long *clk_before_d, *clk_after_d;
    CK(cudaMalloc(&clk_before_d, dbg_n * sizeof(unsigned long long)));
    CK(cudaMalloc(&clk_after_d, dbg_n * sizeof(unsigned long long)));
    CK(cudaMemset(clk_before_d, 0, dbg_n * sizeof(unsigned long long)));
    CK(cudaMemset(clk_after_d, 0, dbg_n * sizeof(unsigned long long)));
    ...
    CK(cudaFuncSetAttribute(fa111_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    float scale = 1.0f / sqrtf((float)hd);
    fprintf(stderr, "clkprobe: bh=%d sl=%d grid=%d probe_iter=%d\n",
            bh, sl, grid, probe_iter);
    fa111_kernel<<<grid, FA_THREADS, smem>>>(
        Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd,
        clk_before_d, clk_after_d, probe_iter);
    CK(cudaDeviceSynchronize());

    unsigned long long *clk_b = (unsigned long long *)malloc(dbg_n * sizeof(unsigned long long));
    unsigned long long *clk_a = (unsigned long long *)malloc(dbg_n * sizeof(unsigned long long));
    CK(cudaMemcpy(clk_b, clk_before_d, dbg_n * sizeof(unsigned long long), cudaMemcpyDeviceToHost));
    CK(cudaMemcpy(clk_a, clk_after_d, dbg_n * sizeof(unsigned long long), cudaMemcpyDeviceToHost));

    // Per-block stats. With 1P+3C, 1 warp is producer (wid=0), 3 warps are consumers (wid=1..3).
    // FA_THREADS = 128 means warps 0..3 (4 warps total).
    int n_blocks_with_data = 0;
    long long sum_skew_before = 0, sum_skew_after = 0, sum_wait_max = 0, sum_wait_min = 0;
    long long max_skew_before = 0, max_wait = 0;
    int hist_skew[10] = {0};  // log10 bins
    int n_warps = FA_THREADS / 32;
    for (int b = 0; b < grid; b++) {
        unsigned long long min_b = ~0ULL, max_b = 0, min_a = ~0ULL, max_a = 0;
        long long max_wait_b = 0, min_wait_b = (long long)1e18;
        int any = 0;
        for (int w = 0; w < n_warps; w++) {
            unsigned long long cb = clk_b[b * 8 + w];
            unsigned long long ca = clk_a[b * 8 + w];
            if (cb == 0) continue;
            any = 1;
            ...
            long long wait_w = (long long)ca - (long long)cb;
            if (wait_w > max_wait_b) max_wait_b = wait_w;
            if (wait_w < min_wait_b) min_wait_b = wait_w;
        }
        if (!any) continue;
        n_blocks_with_data++;
        long long skew_b = (long long)(max_b - min_b);
        long long skew_a = (long long)(max_a - min_a);
        sum_skew_before += skew_b;
        sum_skew_after += skew_a;
        sum_wait_max += max_wait_b;
        sum_wait_min += min_wait_b;
        if (skew_b > max_skew_before) max_skew_before = skew_b;
        if (max_wait_b > max_wait) max_wait = max_wait_b;
        // log10 hist
        int bin = 0;
        long long s = skew_b;
        while (s >= 10 && bin < 9) { s /= 10; bin++; }
        hist_skew[bin]++;
    }
    printf("\n=== Clock probe results ===\n");
    printf("Blocks measured: %d / %d\n", n_blocks_with_data, grid);
    if (n_blocks_with_data == 0) {
        printf("ERROR: no data captured. probe_iter=%d may be out of range.\n", probe_iter);
        return 1;
    }
    printf("Mean arrival skew BEFORE sync (max-min per block): %lld cycles\n",
           sum_skew_before / n_blocks_with_data);
    printf("Max  arrival skew BEFORE sync (worst block):       %lld cycles\n",
           max_skew_before);
    printf("Mean exit    skew AFTER  sync (max-min per block): %lld cycles\n",
           sum_skew_after / n_blocks_with_data);
    printf("Mean WAIT per block (max warp time at sync):       %lld cycles\n",
           sum_wait_max / n_blocks_with_data);
    printf("Mean WAIT per block (min warp time at sync):       %lld cycles\n",
           sum_wait_min / n_blocks_with_data);
    printf("Max  WAIT (single warp worst case):                %lld cycles\n",
           max_wait);
    printf("\nDistribution of arrival skew (log10 bins) — %d blocks:\n", n_blocks_with_data);
    const char *labels[10] = {"0-9","10-99","100-999","1k-9k","10k-99k","100k-999k","1M-10M","10M+","100M+","1B+"};
    for (int i = 0; i < 10; i++) {
        if (hist_skew[i] > 0) {
            printf("  %s cycles: %d blocks (%.1f%%)\n",
                   labels[i], hist_skew[i], 100.0 * hist_skew[i] / n_blocks_with_data);
        }
    }
    free(Q_h); free(K_h); free(V_h); free(clk_b); free(clk_a);
    cudaFree(Q_d); cudaFree(K_d); cudaFree(V_d); cudaFree(O_d);
    cudaFree(clk_before_d); cudaFree(clk_after_d);
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 2 && strcmp(argv[1], "--clkprobe") == 0) {
        int cfg = (argc >= 3) ? atoi(argv[2]) : 9;
        int it = (argc >= 4) ? atoi(argv[3]) : 5;
        return clkprobe_mode(cfg, it);
    }
    ...
    for (auto &c : configs)
    {
        ...
        fa111_kernel<<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd, nullptr, nullptr, -1);
        CK(cudaDeviceSynchronize());
    ...
    for (auto &c : bench_configs)
    {
        ...
        for (int i = 0; i < 5; i++)
            fa111_kernel<<<bh * nqt, FA_THREADS, smem>>>(
                Q_d, K_d, V_d, O_d, sl, hd, ca_bench, scale, 1.0f, 1.0f, wnd, nullptr, nullptr, -1);
        CK(cudaDeviceSynchronize());
