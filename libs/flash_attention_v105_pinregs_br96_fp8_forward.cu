        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    uint8_t *smQ = raw;
    // v102 B-perf probe: drop smV_T (-8.7 KB) → 31.8 KB total/block.
    // PV reads from smV directly with K-major indexing as proxy for N-major (correctness
    // fails but perf pattern measurable). transpose_v removed. SMEM 3 blocks = 95 KB ≤ cap.
...
#pragma unroll
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v102: transpose_v REMOVED. PV reads from smV directly with K-major swizzle
        // as proxy for N-major layout. Correctness will fail (wrong bytes interpreted)
        // but SMEM bandwidth pattern + reg pressure measurable.

        // v101: smP MUST be different from smV (V[kv+1] cp.async writes smV).
        // Reuse smQ (dead after Qr register pre-load before kv-loop). smQ is 16 KB,
        // smP only needs 8 KB (Br × Bc = 128 × 64), so first 8 KB of smQ used.
        uint8_t *smP = smQ;
...
#pragma unroll
            ...
            {
                int br = nt * 8;
                // v102: read from smV with stride-64 swizzle (smV is K-major, but we
                // measure as if N-major; bytes wrong, perf pattern preserved).
                uint32_t b0 = *(uint32_t *)&smV[swz_byte_bc(br + gid, cl)];
                uint32_t b1 = *(uint32_t *)&smV[swz_byte_bc(br + gid, ch)];
#pragma unroll
                for (int mi = 0; mi < M_TILES; mi++)
                {
...
#pragma unroll
            ...
            {
                int br = nt * 8;
                // v102: read from smV (no smV_T). Perf-pattern proxy.
                uint32_t b0 = *(uint32_t *)&smV[swz_byte_bc(br + gid, cl)];
                uint32_t b1 = *(uint32_t *)&smV[swz_byte_bc(br + gid, ch)];
#pragma unroll
                for (int mi = 0; mi < M_TILES; mi++)
...
// Runs ONE warmup + ONE measured launch of fa105_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    // v101 single-K SMEM: smQ + 1×smK + 1×smV + smV_T = 16K + 8K + 8K + 8.7K = 40.7K
    // v102: no smV_T → 16K + 8K + 8K = 32K/block; 3 blocks × 32 = 96K ≤ 100K cap
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa105_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa105_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v105 hd=128 Br=96 + 6 warps + M_TILES=1 + P-in-regs (Pf_pair shfl) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa105_kernel));
    ...
    int smem_v96   = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                   + 128 * SMV_T_STRIDE;
    printf("v102 no-smV_T SMEM (hd=128): %d B (%.2f KB) vs v96=%d B (delta=%+d B)\n",
           smem_hd128, smem_hd128 / 1024.0, smem_v96, smem_hd128 - smem_v96);
    printf("3 blocks × %.2f KB = %.2f KB %s\n\n",
           smem_hd128 / 1024.0, 3 * smem_hd128 / 1024.0,
           (3 * smem_hd128 <= 102400) ? "≤ 100 KB cap (3 blocks SMEM-fits)" : "> 100 KB cap (BLOCKED)");
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa105_kernel<<<bh * nqt, FA_THREADS, smem>>>(
