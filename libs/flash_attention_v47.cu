// =============================================================================
// FlashAttention v47 — FP16 ex2 Softmax + Occupancy=3 (SM89)
// =============================================================================
//
// Identical to v54 except: __launch_bounds__(128, 3) instead of (128, 2)
//
// SM89 register budget:
//   65536 regs / SM ÷ (128 threads × 3 blocks) = 170 regs/thread max
//   vs occupancy=2: 65536 / (128 × 2) = 256 regs/thread max
//
// If nvcc can fit kernel in ≤170 regs → 3 blocks/SM → better latency hiding
// If kernel needs >170 regs → spills to local memory → regression
//
// Smem: 3 × 32KB = 96KB < 100KB SM89 limit ✓
// =============================================================================
...
// KEY CHANGE: occupancy hint = 3 instead of 2
__global__ void __launch_bounds__(FA_THREADS, 3)
    fa47_kernel(const __half *__restrict__ Q, const __half *__restrict__ K,
...
// C API
// =============================================================================
static int g_smem47 = 0;
extern "C"
{
    int flash_attention_v47_forward(
        ...
        int total_heads, int seq_len, int head_dim, int causal, void *stream)
    {
        ...
        if (smem > g_smem47)
        {
            cudaError_t e = cudaFuncSetAttribute(fa47_kernel,
        ...
        int nqt = (seq_len + FA_BR - 1) / FA_BR;
        fa47_kernel<<<total_heads * nqt, FA_THREADS, smem, (cudaStream_t)stream>>>(
