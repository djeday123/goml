...
__global__ void __launch_bounds__(FA_THREADS, 2)  // v123: 2 blocks/SM как v121
    fa96b_kernel(
...
#pragma unroll
        ...
        // v123 ГИБРИД: Sr_p × fs → Sr_tmp[4] FP32 (ephemeral) → mask FP32 → convert to half2 → reduce __hmax2.
        // Sr хранится как __half2 (32 regs), но конверсия + маска выполняются на FP32 копии
        // в локальном scope nt-iter — компилятор elide через #pragma unroll.
...
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
            // Step 1: Sr_p (half2 from QMMA) → tmp float × fs (FP32 для маски)
            float tmp[M_TILES][4];
#pragma unroll
            for (int mi = 0; mi < M_TILES; mi++)
            {
                ...
                tmp[mi][0] = __half2float(__low2half(v0)) * fs;
                tmp[mi][1] = __half2float(__high2half(v0)) * fs;
                tmp[mi][2] = __half2float(__low2half(v1)) * fs;
                tmp[mi][3] = __half2float(__high2half(v1)) * fs;
            }
            // Step 2: causal/window mask в FP32 (как v121, источник убирает window regression)
            if (causal)
            {
...
#pragma unroll
                for (int mi = 0; mi < M_TILES; mi++)
                {
                    ...
                    if (gk0 > gq0) tmp[mi][0] = -1e30f;
                    if (gk1 > gq0) tmp[mi][1] = -1e30f;
                    if (gk0 > gq8) tmp[mi][2] = -1e30f;
                    if (gk1 > gq8) tmp[mi][3] = -1e30f;
                    if (gk0 < kmin0) tmp[mi][0] = -1e30f;
                    if (gk1 < kmin0) tmp[mi][1] = -1e30f;
                    if (gk0 < kmin8) tmp[mi][2] = -1e30f;
                    if (gk1 < kmin8) tmp[mi][3] = -1e30f;
                    if (gq0 >= seq_len) tmp[mi][0] = tmp[mi][1] = -1e30f;
                    if (gq8 >= seq_len) tmp[mi][2] = tmp[mi][3] = -1e30f;
                    if (gk0 >= seq_len) tmp[mi][0] = tmp[mi][2] = -1e30f;
                    if (gk1 >= seq_len) tmp[mi][1] = tmp[mi][3] = -1e30f;
                }
            }
            // Step 3: convert FP32 → __half2 (source +2-6% peak от half2-pipeline дальше)
#pragma unroll
            for (int mi = 0; mi < M_TILES; mi++)
            {
                Sr[nt][mi][0] = __floats2half2_rn(tmp[mi][0], tmp[mi][1]);
                Sr[nt][mi][1] = __floats2half2_rn(tmp[mi][2], tmp[mi][3]);
            }
        }
