...
#pragma unroll
        ...
        // v96: Option C ks-batching. v98 + K-preload to break short_scoreboard chain.
        // Each QK ks-batch preloads b0_arr[8]/b1_arr[8] from cur_K BEFORE nt MMA loop.
        // Separates LDS dependency from MMA consumer — scheduler can issue MMA on
        // already-prefetched K bytes without waiting for SMEM. Targets short_scb 6.5%.
        // === QK ks=0 batch with K-preload ===
        {
...
// Runs ONE warmup + ONE measured launch of fa98_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa98_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa98_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v98 hd=128 + K-preload (port v90 — 4 QK batches × b0_arr[8]/b1_arr[8]) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa98_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa98_kernel<<<bh * nqt, FA_THREADS, smem>>>(
