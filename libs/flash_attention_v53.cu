// =============================================================================
// FlashAttention v21 — Br=128 (8 warps) over v20 (Br=64, 4 warps)
// =============================================================================
//
// Key change: double Q tile from 64→128 rows.
//   - Each warp still owns 16 Q rows (same register pressure)
//   - 8 warps × 16 = 128 Q rows per block
//   - K/V tiles still Bc=64
//   - Double K/V reuse: each K/V tile used by 128 Q rows instead of 64
//   - Arithmetic intensity: 2× more compute per SMEM load
//
// Architecture:
//   Threads: 256 (8 warps)
//   Br=128, Bc=64, D=128
//   Q: permanent in registers (Qr[8][4] per warp, same as v20)
//   S/P: in registers (Sr[8][4], Pr[4][4] per warp, same as v20)
//   O: in registers (Or[16][4] per warp, same as v20)
//   SMEM: Q load needs 128×128×2=32KB, KV double-buf needs 2×64×128×2=32KB
//         But Q buf freed after load → reuse as KV buf.
//         Layout: 2 buffers × 64×128 = 32KB total (same as v20!)
//         Q loaded in 2 passes: top 64 rows → regs, bottom 64 rows → regs
//
// Expected gain: +5-15% from doubled arithmetic intensity
//
// Occupancy: 256 threads × 2 blocks/SM = 512 threads (33%)
//            vs v20: 128 threads × 2 blocks/SM = 256 threads (16.7%)
//            → better latency hiding too
//
// Build:
...
#define V20_BR 64
#define V20_BC 64
#define V20_THREADS 128
#define V20_STRIDE 128

// v21 params
#define V21_BR 128
#define V21_BC 64
#define V21_THREADS 256
#define V21_STRIDE 128
...
// Swizzled async load: rows × 128 cols
template <int THREADS>
__device__ __forceinline__ void load_tile_t(
    __half *dst, const __half *src, int start, int rows,
    int seq_len, int head_dim)
{
    constexpr int CPR = 16; // 128 cols / 8 per cp.async
    int total = rows * CPR;
#pragma unroll 4
    for (int c = threadIdx.x; c < total; c += THREADS)
    {
        int row = c / CPR, lch = c % CPR, gr = start + row;
        int pch = lch ^ (row & 7);
        cpa16(&dst[row * V21_STRIDE + pch * 8],
              &src[gr * head_dim + lch * 8], (gr < seq_len) ? 16 : 0);
    }
}

// =============================================================================
// v20 kernel (baseline, Br=64, 128 threads)
// =============================================================================

__device__ __forceinline__ void load_tile_v20(
    __half *dst, const __half *src, int start, int rows,
    int seq_len, int head_dim)
{
    ...
    int total = rows * CPR;
#pragma unroll 4
    for (int c = threadIdx.x; c < total; c += V20_THREADS)
    {
        int row = c / CPR, lch = c % CPR, gr = start + row;
        int pch = lch ^ (row & 7);
        cpa16(&dst[row * V20_STRIDE + pch * 8],
              &src[gr * head_dim + lch * 8], (gr < seq_len) ? 16 : 0);
    }
}

__global__ void __launch_bounds__(V20_THREADS, 2)
...
{
    int nqt = (seq_len + V20_BR - 1) / V20_BR;
    int bh = blockIdx.x / nqt, qt = blockIdx.x % nqt, qs = qt * V20_BR;
    int wid = threadIdx.x / 32, lane = threadIdx.x % 32;
    int gid = lane >> 2, tid = lane & 3;
    extern __shared__ char raw[];
    __half *buf0 = (__half *)raw;
    __half *buf1 = (__half *)(raw + V20_BC * V20_STRIDE * sizeof(__half));
    int hs = seq_len * head_dim;
    const __half *Qh = Q + bh * hs, *Kh = K + bh * hs, *Vh = V + bh * hs;
    __half *Oh = O + bh * hs;

    load_tile_v20(buf0, Qh, qs, V20_BR, seq_len, head_dim);
    cpa_commit();
    cpa_wait<0>();
    __syncthreads();
    int mrb = wid * 16;
...
#pragma unroll
    for (int ks = 0; ks < 8; ks++)
        ld_a_sw(Qr[ks][0], Qr[ks][1], Qr[ks][2], Qr[ks][3],
                buf0, V20_STRIDE, mrb, ks * 16, lane);
    __syncthreads();
...
#pragma unroll
    ...
    int nkv = (seq_len + V20_BC - 1) / V20_BC;
    load_tile_v20(buf0, Kh, 0, V20_BC, seq_len, head_dim);
    cpa_commit();

    for (int kv = 0; kv < nkv; kv++)
    {
        int kvs = kv * V20_BC;
        if (causal && kvs > qs + V20_BR - 1)
            break;
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_sw(b0, b1, cur, V20_STRIDE, nt * 8, ks * 16, lane);
        ...
        __syncthreads();
        load_tile_v20(cur, Vh, kvs, V20_BC, seq_len, head_dim);
        cpa_commit();
        int nkvs = (kv + 1) * V20_BC;
        bool has_nxt = (kv + 1 < nkv) && (!causal || nkvs <= qs + V20_BR - 1);
        if (has_nxt)
            load_tile_v20(nxt, Kh, nkvs, V20_BC, seq_len, head_dim);
        cpa_commit();
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_vt(b0, b1, cur, V20_STRIDE, nt * 8, ks * 16, lane);
...
// =============================================================================
// v21 kernel (Br=128, 256 threads, 8 warps)
// =============================================================================
// Each warp: 16 Q rows, same register layout as v20.
// 8 warps × 16 = 128 Q rows per block.
// Q loaded in 2 passes (top 64, bottom 64) through same 64-row buffer.
// K/V double-buffered in same 2 × 64×128 SMEM.

__global__ void __launch_bounds__(V21_THREADS, 2)
    fa21_kernel(const __half *__restrict__ Q, const __half *__restrict__ K,
                const __half *__restrict__ V, __half *__restrict__ O,
                int seq_len, int head_dim, int causal, float scale)
{
    int nqt = (seq_len + V21_BR - 1) / V21_BR;
    int bh = blockIdx.x / nqt, qt = blockIdx.x % nqt, qs = qt * V21_BR;
    int wid = threadIdx.x / 32, lane = threadIdx.x % 32;
    int gid = lane >> 2, tid = lane & 3;

    extern __shared__ char raw[];
    __half *buf0 = (__half *)raw;
    __half *buf1 = (__half *)(raw + V21_BC * V21_STRIDE * sizeof(__half));
    // buf0/buf1: each 64×128 = 16KB

    int hs = seq_len * head_dim;
    const __half *Qh = Q + bh * hs, *Kh = K + bh * hs, *Vh = V + bh * hs;
    __half *Oh = O + bh * hs;

    // ---- Load Q to registers in 2 passes ----
    // Each warp owns 16 rows: warp 0-3 → rows 0-63, warp 4-7 → rows 64-127
    int mrb = wid * 16; // warp's row base within Br=128
    uint32_t Qr[8][4];

    // Pass 1: load Q rows 0-63 (warps 0-3 read their fragments)
    load_tile_t<V21_THREADS>(buf0, Qh, qs, 64, seq_len, head_dim);
    cpa_commit();
    cpa_wait<0>();
    __syncthreads();
...
#pragma unroll
        for (int ks = 0; ks < 8; ks++)
            ld_a_sw(Qr[ks][0], Qr[ks][1], Qr[ks][2], Qr[ks][3],
                    buf0, V21_STRIDE, mrb, ks * 16, lane);
    }
    __syncthreads();

    // Pass 2: load Q rows 64-127 (warps 4-7 read their fragments)
    load_tile_t<V21_THREADS>(buf0, Qh, qs + 64, 64, seq_len, head_dim);
    cpa_commit();
    cpa_wait<0>();
    __syncthreads();
...
#pragma unroll
        for (int ks = 0; ks < 8; ks++)
            ld_a_sw(Qr[ks][0], Qr[ks][1], Qr[ks][2], Qr[ks][3],
                    buf0, V21_STRIDE, mrb - 64, ks * 16, lane);
    }
    __syncthreads();
...
#pragma unroll
    ...
    int nkv = (seq_len + V21_BC - 1) / V21_BC;

    // Prefetch K[0]
    load_tile_t<V21_THREADS>(buf0, Kh, 0, V21_BC, seq_len, head_dim);
    cpa_commit();

    for (int kv = 0; kv < nkv; kv++)
    {
        int kvs = kv * V21_BC;
        if (causal && kvs > qs + V21_BR - 1)
            break;
        ...
        // ---- QK^T: all 8 warps compute independently ----
        float Sr[8][4];
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
            Sr[nt][0] = Sr[nt][1] = Sr[nt][2] = Sr[nt][3] = 0;
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_sw(b0, b1, cur, V21_STRIDE, nt * 8, ks * 16, lane);
                mma16816(Sr[nt][0], Sr[nt][1], Sr[nt][2], Sr[nt][3],
                         Qr[ks][0], Qr[ks][1], Qr[ks][2], Qr[ks][3],
                         b0, b1, Sr[nt][0], Sr[nt][1], Sr[nt][2], Sr[nt][3]);
            }
        ...
        // K consumed — prefetch V[kv] and K[kv+1]
        __syncthreads();
        load_tile_t<V21_THREADS>(cur, Vh, kvs, V21_BC, seq_len, head_dim);
        cpa_commit();
        int nkvs = (kv + 1) * V21_BC;
        bool has_nxt = (kv + 1 < nkv) && (!causal || nkvs <= qs + V21_BR - 1);
        if (has_nxt)
            load_tile_t<V21_THREADS>(nxt, Kh, nkvs, V21_BC, seq_len, head_dim);
        cpa_commit();
...
#pragma unroll
        ...
        // ---- Online softmax (same as v20 — per-warp, 4-thread butterfly) ----
        float nm[2] = {-1e30f, -1e30f};
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_vt(b0, b1, cur, V21_STRIDE, nt * 8, ks * 16, lane);
                mma16816(Or[nt][0], Or[nt][1], Or[nt][2], Or[nt][3],
                         Pr[ks][0], Pr[ks][1], Pr[ks][2], Pr[ks][3],
                         b0, b1, Or[nt][0], Or[nt][1], Or[nt][2], Or[nt][3]);
            }
...
{
    int smem = 2 * V20_BC * V20_STRIDE * (int)sizeof(__half);
    if (smem > g_smem20)
    {
    ...
    int nqt = (sl + V20_BR - 1) / V20_BR;
    fa20_kernel<<<th * nqt, V20_THREADS, smem>>>(Q, K, V, O, sl, hd, ca, sc);
}

void launch_v21(const __half *Q, const __half *K, const __half *V, __half *O,
                int th, int sl, int hd, int ca)
{
    int smem = 2 * V21_BC * V21_STRIDE * (int)sizeof(__half);
    if (smem > g_smem21)
    {
    ...
    float sc = 1.0f / sqrtf((float)hd);
    int nqt = (sl + V21_BR - 1) / V21_BR;
    fa21_kernel<<<th * nqt, V21_THREADS, smem>>>(Q, K, V, O, sl, hd, ca, sc);
}

void test_correctness()
{
    printf("--- Correctness: v20 (Br=64) vs v21 (Br=128) vs CPU ---\n");
    int configs[][3] = {
...
void bench()
{
    printf("\n--- Performance: v20 (Br=64, 4w) vs v21 (Br=128, 8w) ---\n");
    ...
    printf("\nPeak = 165.2 T | v20 record: 151T @ 7B-8K\n");
    printf("v21: Br=128 → 2x K/V reuse, 2x occupancy (256 vs 128 thr/block)\n");
}
...
int main()
{
    ...
    printf("=== FlashAttention v21 — Br=128 (8 warps) over v20 ===\n");
