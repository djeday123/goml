// v44c: start from v44b (PASS), fuse scale into exp2f
// Change: remove Sr *= scale, use exp2f(Sr * scale_log2 - max_scaled)
// Keep max on SCALED scores to match v44b exactly, then switch to unscaled
...
__global__ void __launch_bounds__(THREADS, 2)
    flash_attention_v44c_kernel(
        const __half *__restrict__ Q, const __half *__restrict__ K,
        const __half *__restrict__ V, __half *__restrict__ O,
        int seq_len, int head_dim, int causal, float scale)
{
    int nqt = (seq_len + BR - 1) / BR;
    int bh = blockIdx.x / nqt, qt = blockIdx.x % nqt, qs = qt * BR;
    int wid = threadIdx.x / 32, lane = threadIdx.x % 32;
    int gid = lane >> 2, tid = lane & 3;

    extern __shared__ char raw[];
    __half *buf0 = (__half *)raw;
    __half *buf1 = (__half *)(raw + BC * STRIDE * sizeof(__half));
    ...
    load_tile(buf0, Qh, qs, BR, seq_len, head_dim);
    cpa_commit();
    cpa_wait<0>();
    __syncthreads();
...
#pragma unroll
    ...
    // KEY: rmax stores SCALED max (same as v20)
    float rmax[2] = {-1e30f, -1e30f}, rsexp[2] = {0, 0};
    int nkv = (seq_len + BC - 1) / BC;

    load_tile(buf0, Kh, 0, BC, seq_len, head_dim);
    cpa_commit();

    for (int kv = 0; kv < nkv; kv++)
    {
        int kvs = kv * BC;
        if (causal && kvs > qs + BR - 1)
            break;
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_sw(b0, b1, cur, STRIDE, nt * 8, ks * 16, lane);
        ...
        __syncthreads();
        load_tile(cur, Vh, kvs, BC, seq_len, head_dim);
        cpa_commit();
        int nkvs = (kv + 1) * BC;
        bool has_nxt = (kv + 1 < nkv) && (!causal || nkvs <= qs + BR - 1);
        if (has_nxt)
            load_tile(nxt, Kh, nkvs, BC, seq_len, head_dim);
        cpa_commit();

        // Scale scores first, then mask (same order as v20)
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
            Sr[nt][0] *= scale;
        ...
        // Max over SCALED scores (same as v20)
...
#pragma unroll
        ...
        // rsc using exp2f (rmax is scaled)
...
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
            // Sr is already scaled. Use exp2f((Sr - max) * log2e)
            // This is exp2f(Sr * log2e - max * log2e) = exp(Sr - max)
            Sr[nt][0] = exp2f((Sr[nt][0] - rmax[0]) * 1.4426950408889634f);
            Sr[nt][1] = exp2f((Sr[nt][1] - rmax[0]) * 1.4426950408889634f);
            Sr[nt][2] = exp2f((Sr[nt][2] - rmax[1]) * 1.4426950408889634f);
            Sr[nt][3] = exp2f((Sr[nt][3] - rmax[1]) * 1.4426950408889634f);
...
#pragma unroll
            ...
            {
                uint32_t b0, b1;
                ld_b_vt(b0, b1, cur, STRIDE, nt * 8, ks * 16, lane);
...
extern "C"
{
    int flash_attention_v44c_forward(
        const void *Q, const void *K, const void *V, void *O,
        int total_heads, int seq_len, int head_dim, int causal, void *stream)
    {
        if (head_dim != 128)
            return -1;
        int smem = 2 * BC * STRIDE * (int)sizeof(__half);
        if (smem > g_smem)
        {
            cudaError_t e = cudaFuncSetAttribute(flash_attention_v44c_kernel,
        ...
        int nqt = (seq_len + BR - 1) / BR;
        flash_attention_v44c_kernel<<<total_heads * nqt, THREADS, smem, (cudaStream_t)stream>>>(
