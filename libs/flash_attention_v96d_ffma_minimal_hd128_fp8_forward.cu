...
#pragma unroll
        for (int mi = 0; mi < M_TILES; mi++)
        {
            ...
            // v96d: explicit FFMA for rsexp update (same arity, zero new live values).
            // Compiler may already do this; explicit fmaf guarantees FFMA emission.
            rsexp[mi][0] = fmaf(rsexp[mi][0], rsc[mi][0], ns[mi][0]);
...
// Runs ONE warmup + ONE measured launch of fa96d_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa96d_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa96d_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    CK(cudaFuncGetAttributes(&attr, fa96d_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa96d_kernel<<<bh * nqt, FA_THREADS, smem>>>(
