...
// v80f: Br=32 with 2-producer + 2-consumer warp specialization.
// Warps 0,1 = producers (cp.async only). Warps 2,3 = consumers (compute, cover 32 rows).
#define FA_BR 32                  // 2 consumer warps × M_TILES=1 × 16 rows = 32
#define FA_BC 64
#define FA_THREADS 128            // 4 warps total (2 prod + 2 cons)
...
// v80f: 2-producer cp.async loader. Threads 0..63 (warps 0+1) do cp.async, others skip.
__device__ __forceinline__ void load_tile_fp8_prod(
    uint8_t *dst, const uint8_t *src, int start, int rows,
    int seq_len, int head_dim)
{
    constexpr int CHUNK = 16;
    int chunks_per_row = head_dim / CHUNK;
    int total = rows * chunks_per_row;
    // Threads 0..63 each do total/64 iterations.
    int tid = threadIdx.x;  // 0..63 for producers (wid 0,1)
#pragma unroll 4
        ...
        int window)  // 0 = no window. >0 = causal sliding window.
{
    ...
    // v80f: wid 0,1 = producers. wid 2,3 = consumers. compute_wid = wid - 2 (0..1).
    int compute_wid = wid - 2;
    int mrb = compute_wid * 16;  // consumer warps cover rows 0..15 and 16..31 (Br=32)
    ...
    // Qr[ks][mi][r] — v80f: CONSUMER WARPS ONLY load Qr.
...
#pragma unroll
    ...
    // v80f: pre-load K + V via PRODUCER WARPS (0,1).
    if (wid < 2) {
        load_tile_fp8_prod(smK, Kh, kv_min_blocks * FA_BC, FA_BC, seq_len, head_dim);
        load_tile_fp8_prod(smV, Vh, kv_min_blocks * FA_BC, FA_BC, seq_len, head_dim);
    }
    cpa_commit();
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v80f: QK MMA — consumer warps only.
...
#pragma unroll
        ...
        // Next iter's transpose_v will read smV (containing V[kv+1]).
        {
            ...
            // v80f: V[kv+1] mid-iter cp.async — producer warps only.
            if (wid < 2) {
                load_tile_fp8_prod(smV, Vh, kv_p * FA_BC, rows_p, seq_len, head_dim);
            }
        }
        cpa_commit();

        // v80f: softmax + P_top/P_bot computation — CONSUMER WARPS ONLY.
...
#pragma unroll
        ...
        // v80f: P_top/P_bot declared above (cross-block scope).
...
#pragma unroll
        ...
        }  // end softmax `if (wid >= 2)` block

        __syncthreads();

        // v80f: smP STS — consumer warps only.
        if (wid >= 2)
        {
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
...
#pragma unroll
        ...
        }  // end smP STS `if (wid >= 2)` block
        __syncthreads();

        // v80f: PV MMA — consumer warps only.
        if (wid >= 2)
        {
#pragma unroll
...
#pragma unroll
        ...
        }  // end PV MMA `if (wid >= 2)` block
        __syncthreads();

        // v80f: end-of-iter K cp.async — PRODUCER WARPS ONLY.
        int kv_p_k = kv + 1;
        int rows_p_k = (kv_p_k < kv_max_blocks) ? FA_BC : 0;
        if (wid < 2) {
            load_tile_fp8_prod(smK, Kh, kv_p_k * FA_BC, rows_p_k, seq_len, head_dim);
        }
        cpa_commit();
    }

    // v80f: output writeback — consumer warps only.
    if (wid >= 2)
    {
#pragma unroll
    for (int mi = 0; mi < M_TILES; mi++)
...
#pragma unroll
    ...
    }  // end output writeback `if (wid >= 2)` block
}
...
int main()
{
    ...
    printf("=== FA v80f (Br=32 + 2-producer/2-consumer warp spec) — EXPERIMENT: 4 blocks/SM via Br=64 + door 2 + single-K ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa80f_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        CK(cudaFuncSetAttribute(fa80f_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa80f_kernel<<<bh * nqt, FA_THREADS, smem>>>(
