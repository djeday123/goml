...
// =====================================================================
__global__ void kernel_dv_mma_p1_alpha(
    ...
    float scale)
{
    ...
    // SMEM layout — α-probe:
    //   smK 8K + smQ 8K + smdO 16K + smdO_T 16K + smPT 8K + smL 256B = ~57 KB
    //   → forced 1 block/SM (opt-in needed in launcher).
    extern __shared__ uint8_t smem_raw[];
    uint8_t *smK    = smem_raw;
    uint8_t *smQ    = smK  + Bc * Hd;
    __half  *smdO   = reinterpret_cast<__half*>(smQ + Bc * Hd);      // row-major [Br, Hd]
    __half  *smdO_T = smdO + Br * Hd;                                // transposed [Hd, Br]
    __half  *smPT   = smdO_T + Br * Hd;                              // compute output
    ...
    for (int qt = 0; qt < n_qt; ++qt) {
        ...
        // ===== step A.5: in-block transpose smdO → smdO_T (α path) =====
        //   smdO is row-major [Br, Hd] FP16 (i,d).
        //   smdO_T is row-major [Hd, Br] FP16 (d,i) — i.e. col-major view of dO.
        //   8192 elements / 128 threads = 64 per thread, FP16 reads/writes.
        {
            constexpr int total_dO_elems = Br * Hd;        // 8192
            for (int e = tid; e < total_dO_elems; e += FA_DV_MMA_THREADS) {
                int i_local = e / Hd;
                int d       = e % Hd;
                __half v    = smdO[i_local * Hd + d];
                smdO_T[d * Br + i_local] = v;
            }
        }
        __syncthreads();
        ...
        for (int kb = 0; kb < KB_DV; ++kb) {
            ...
            #pragma unroll
            for (int ni = 0; ni < NI_DV; ++ni) {
                // α path: smdO_T row-major [d, i] → single LDS.U32 per b_i.
                //   For B col-major operand: lane needs half2 (k, k+1) at same n.
                //   In smdO_T storage [d * Br + i], (k_pair, k_pair+1) at same d
                //   are adjacent (stride 2 bytes) → single uint32 LDS captures both.
                int n     = ni * 8 + l_div4;
                int k_lo  = kb * 16 + l_mod4 * 2 + 0;
                int k_hi  = kb * 16 + l_mod4 * 2 + 8;
                uint32_t Br0 = *reinterpret_cast<uint32_t*>(&smdO_T[n * Br + k_lo]);
                uint32_t Br1 = *reinterpret_cast<uint32_t*>(&smdO_T[n * Br + k_hi]);
...
{
    if (hd != FA_DV_MMA_HD) {
        fprintf(stderr, "fa_bwd_dv_mma_p1_alpha: hd=%d, expected %d\n", hd, FA_DV_MMA_HD);
        exit(1);
    }
    ...
    const int smem_bytes =
      ...
      + Br * hd * sizeof(__half)             // smdO   16K (row-major)
      + Br * hd * sizeof(__half)             // smdO_T 16K (transposed)
      + Bc * Br * sizeof(__half)             // smPT    8K
      + Br      * sizeof(float);             // smL  256B

    // α-probe requires ~57 KB SMEM/block → 1 block/SM forced (opt-in).
    cudaFuncSetAttribute(kernel_dv_mma_p1_alpha,
                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                         smem_bytes);
    kernel_dv_mma_p1_alpha<<<grid, FA_DV_MMA_THREADS, smem_bytes, stream>>>(
