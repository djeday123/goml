    ...
    uint8_t *smV_T, const uint8_t *smV, int head_dim)
{
    ...
    {
        // v114: SWAP mapping — adjacent threads handle adjacent K-cols (not N-rows).
        // Goal: reduce smV_T WRITE bank conflict from 4-way to 2-way.
        // Trade-off: smV READ pattern changes (may worsen — empirical test).
        int tk = t % tiles_k;          // 0..15  (was t / tiles_n)
        int tn = t / tiles_k;          // 0..31  (was t % tiles_n)
        int k0 = tk * TILE;
        int n0 = tn * TILE;
...
// Runs ONE warmup + ONE measured launch of fa114_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa114_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa114_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v114 = v96 + transpose SWAP mapping (adjacent threads = adjacent K-cols) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa114_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa114_kernel<<<bh * nqt, FA_THREADS, smem>>>(
