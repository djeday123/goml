    ...
    float scale)
{
    ...
    for (int kt = 0; kt < kt_end; ++kt) {
        ...
        for (int np = 0; np < NI_DP / 2; ++np) {
            ...
            smdS[i_local_lo * SMDS_STRIDE + ja_lo] = dsa_lo_fp8 & 0xFF;
            smdS[i_local_lo * SMDS_STRIDE + jb_lo] = dsb_lo_fp8 & 0xFF;
            smdS[i_local_lo * SMDS_STRIDE + ja_hi] = (dsa_lo_fp8 >> 8) & 0xFF;
            smdS[i_local_lo * SMDS_STRIDE + jb_hi] = (dsb_lo_fp8 >> 8) & 0xFF;
            smdS[i_local_hi * SMDS_STRIDE + ja_lo] = dsa_hi_fp8 & 0xFF;
            smdS[i_local_hi * SMDS_STRIDE + jb_lo] = dsb_hi_fp8 & 0xFF;
            smdS[i_local_hi * SMDS_STRIDE + ja_hi] = (dsa_hi_fp8 >> 8) & 0xFF;
            smdS[i_local_hi * SMDS_STRIDE + jb_hi] = (dsb_hi_fp8 >> 8) & 0xFF;
        }
...
{
    if (hd != FA_DQ_HD) {
        fprintf(stderr, "fa_bwd_dq: hd=%d, expected %d\n", hd, FA_DQ_HD);
        exit(1);
    }
    ...
    // P1a SMEM: smQ → Qr regs, smV↔smdS aliased. smL/smD KEPT.
    constexpr int SMV_SMDS = (FA_DQ_BC * FA_DQ_HD > FA_DQ_BR * FA_DQ_SMDS_STRIDE)
                             ? FA_DQ_BC * FA_DQ_HD
                             : FA_DQ_BR * FA_DQ_SMDS_STRIDE;
    const int smem_bytes =
        SMK_AREA                                       // smK_area (K/K_T aliased), 8704 w/ STRIDE=68
      + SMV_SMDS                                       // smV/smdS aliased, 8192
      + Br * hd * sizeof(__half)                       // smdO (resident) 16384
      + 2 * Br * sizeof(float);                        // smL + smD 512

    cudaFuncSetAttribute(kernel_dq,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes);
    kernel_dq<<<grid, FA_DQ_THREADS, smem_bytes, stream>>>(
        Q, K, V, dO_g, L, D, dQ, bh, sl, hd, causal, window, scale);
}
