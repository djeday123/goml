...
#pragma unroll
        ...
        // v79c: P-in-registers. NO smP STS, NO __syncthreads, NO smP LDS.
        // Pr fragments constructed via __shfl_sync within 4-lane quad-group.
        //
        // Layout: smP-equivalent in regs is P_top/P_bot[nt][mi] as __half2.
        // Reader at lane (gid, tid_r) wants b32 at col = ks*32 + tid_r*4 (cl) or +16 (ch).
        // From v79b smP write pattern (col = nt*8 + tid_owner*2):
        //   - col bytes [4k, 4k+1] owned by lane tid_owner=2k mod 4 at nt = col/8
        //   - For tid_r reading cl=tid_r*4: covers cols [tid_r*4, tid_r*4+3]
        //   - All 4 bytes share same nt = (ks*4 + tid_r/2). Source lanes: 2 lanes per (mi, half).
        //   - tid_r even (0,2): src lanes (group_base+0, group_base+1) at nt = ks*4 + tid_r/2
        //   - tid_r odd  (1,3): src lanes (group_base+2, group_base+3) at same nt
        //   - cl half: nt_cl = ks*4 + (tid >> 1)
        //   - ch half: nt_ch = nt_cl + 2

        // O += P · V — V B-operand loaded once per (nt, ks), reused across mi.
        int group_base = lane & ~3;
#pragma unroll
        for (int ks = 0; ks < 2; ks++)
        {
            int k_off = ks * 32;
            int cl = k_off + tid * 4, ch = cl + 16;
            int nt_cl = ks * 4 + (tid >> 1);
            int nt_ch = nt_cl + 2;
            int src_a = group_base + ((tid & 1) ? 2 : 0);
            int src_b = group_base + ((tid & 1) ? 3 : 1);

            // Construct Pr fragments via shfl from quad-group lanes, JIT quantize FP16→FP8.
            uint32_t Pr[M_TILES][4];
#pragma unroll
            for (int mi = 0; mi < M_TILES; mi++)
            {
                // Gather P_top (for row g0) and P_bot (for row g8) from 2 src lanes.
                uint32_t Pt_cl_a = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_top[nt_cl][mi]), src_a);
                uint32_t Pt_cl_b = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_top[nt_cl][mi]), src_b);
                uint32_t Pb_cl_a = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_bot[nt_cl][mi]), src_a);
                uint32_t Pb_cl_b = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_bot[nt_cl][mi]), src_b);
                uint32_t Pt_ch_a = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_top[nt_ch][mi]), src_a);
                uint32_t Pt_ch_b = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_top[nt_ch][mi]), src_b);
                uint32_t Pb_ch_a = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_bot[nt_ch][mi]), src_a);
                uint32_t Pb_ch_b = __shfl_sync(0xFFFFFFFF, *reinterpret_cast<uint32_t *>(&P_bot[nt_ch][mi]), src_b);

                // Quantize each FP16x2 → FP8x2 (uint16_t) and pack to b32.
                uint16_t fa_t_cl = fp16x2_to_e4m3x2(Pt_cl_a);
                uint16_t fb_t_cl = fp16x2_to_e4m3x2(Pt_cl_b);
                uint16_t fa_b_cl = fp16x2_to_e4m3x2(Pb_cl_a);
                uint16_t fb_b_cl = fp16x2_to_e4m3x2(Pb_cl_b);
                uint16_t fa_t_ch = fp16x2_to_e4m3x2(Pt_ch_a);
                uint16_t fb_t_ch = fp16x2_to_e4m3x2(Pt_ch_b);
                uint16_t fa_b_ch = fp16x2_to_e4m3x2(Pb_ch_a);
                uint16_t fb_b_ch = fp16x2_to_e4m3x2(Pb_ch_b);

                Pr[mi][0] = (uint32_t)fa_t_cl | ((uint32_t)fb_t_cl << 16);  // row g0, cl
                Pr[mi][1] = (uint32_t)fa_b_cl | ((uint32_t)fb_b_cl << 16);  // row g8, cl
                Pr[mi][2] = (uint32_t)fa_t_ch | ((uint32_t)fb_t_ch << 16);  // row g0, ch
                Pr[mi][3] = (uint32_t)fa_b_ch | ((uint32_t)fb_b_ch << 16);  // row g8, ch
            }
#pragma unroll
...
int main()
{
    ...
    printf("=== FA v79c FP8 forward — P-in-regs — f16 P_local — v69 + V[N+1] mid-iter prefetch into dead smK[buf] ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa79c_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa79c_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa79c_kernel<<<bh * nqt, FA_THREADS, smem>>>(
