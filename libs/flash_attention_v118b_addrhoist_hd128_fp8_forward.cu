        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    // v118b: kv-loop swizzle invariants (port v121). mrb = cwid*32 кратно 4 → не влияет
    // на gid7/gid3 hoist. br = nt*8 кратно 8 → (br+gid)&7 = gid7; кратно 4 → &3 = gid3.
    ...
    //
    // v118 LOCAL-MEM FIX: replace `uint8_t *smK[K_STAGES]` array (16-byte stack
    // frame + LDL.64 in hot loop on dynamic index buf=kv&1, 22.5M local sectors
    // per launch from PC-sampling at 0x76c240) with base+arithmetic-stride.
    // Compiler now emits IMAD instead of LDL.
    constexpr int K_STRIDE_BYTES = FA_BC * FA_STRIDE;   // 64 * 128 = 8192 B per stage
...
#pragma unroll
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        int buf = kv & 1;
        int nxt_buf = (kv + 1) & 1;
        // v118: smK[buf] → arithmetic. buf is uniform per-iter, so this becomes a single IMAD.
...
#pragma unroll
        ...
        // v118b QK B-hoist (port v121): base = gid_lane_base + xor_part; addr(nt) = base + 1024*nt.
        // === QK ks=0 batch ===
        {
...
#pragma unroll
        ...
        if (!is_producer)
        {
        // v118b PV hoist (port v121): smV_T addr(nt) = base + 544*nt; smP_row*64 hoisted.
...
// Runs ONE warmup + ONE measured launch of fa118_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa118_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa118_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    CK(cudaFuncGetAttributes(&attr, fa118_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa118_kernel<<<bh * nqt, FA_THREADS, smem>>>(
