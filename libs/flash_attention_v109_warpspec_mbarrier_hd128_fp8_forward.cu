...
// Runs ONE warmup + ONE measured launch of fa109_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa109_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa109_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v109 STEP 2 hd=128 warp-spec + named-bar consumer-only sync ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa109_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa109_kernel<<<bh * nqt, FA_THREADS, smem>>>(
