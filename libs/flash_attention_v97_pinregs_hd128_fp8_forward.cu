...
#pragma unroll
        ...
        // v97 P-in-regs: skip smP write + 2 __syncthreads entirely. Pack P_top/P_bot
        // fp8x2 per thread into Pf_pair[nt][mi]: low16 = fp8x2(P_top), high16 = fp8x2(P_bot).
        // PV ks=0/1 will gather Pr via uniform-nt __shfl_sync — no SMEM round-trip.
        // Pf_pair structure identical to v89 hd=64 (8 nt × 2 mi = 16 uint32).
...
#pragma unroll
        ...
        // Helper for warp-local lane base (same gid, different tid). Used in PV shfl gather.
        uint32_t gid_lane_base = (threadIdx.x & 0x1c);  // (lane & ~3)

        // v97 P-in-regs PV: gather Pr0/Pr1 via shfl_sync from Pf_pair[8][M_TILES].
        // Pf_pair[0..3] → PV ks=0 (K-cols 0..31); Pf_pair[4..7] → PV ks=1 (K-cols 32..63).
        // Same uniform-nt pattern as v89 hd=64; only the inner nt loop runs 16× here (hd=128).
        // === PV ks=0 batch ===
        {
            int cl = tid * 4, ch = cl + 16;
...
int main()
{
    ...
    printf("=== FA v97 hd=128 + P-in-regs (port v89 — Pf_pair shfl gather, skip smP) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa97_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa97_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa97_kernel<<<bh * nqt, FA_THREADS, smem>>>(
    ...
    for (auto &c : bench_configs)
    {
        ...
        // Sort: tf_runs[0] = worst, [1] = median, [2] = best (asc sort)
        ...
        printf("  bh=%-3d sl=%-4d hd=%d ca=%d wnd=%-4d  perf best/med/worst: %.1f / %.1f / %.1f TFLOPS\n",
               bh, sl, hd, ca_bench, wnd, tf_runs[2], tf_runs[1], tf_runs[0]);
        cudaFree(Q_d); cudaFree(K_d); cudaFree(V_d); cudaFree(O_d);
    }
