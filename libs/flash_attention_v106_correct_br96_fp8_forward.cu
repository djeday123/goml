// =============================================================================
// FlashAttention v106 — FP8 Forward, hd=128 Br=96 6-warp + P-in-regs + smV_T fix
// =============================================================================
// v106 = v105 architecture (Br=96, 6 warps, M_TILES=1, P-in-regs via Pf_pair shfl)
// with smV_T + transpose_v RESTORED. v102/v104/v105 all dropped smV_T for SMEM
// budget; this broke correctness because PV reads bytes from smV with the wrong
// layout (K-major instead of N-major). v106 restores it: 36.7 KB/block, 2 blocks/SM.
//
// Baseline ref: v96 = 568T peak hd=128 (production champion, 564T sustained).
// v105 perf was 549T peak (close to v96) but correctness FAILED for all sl≥256.
// v106 must pass correctness 8/8 AND retain v105's stall-reduction wins
// (mio_throttle 21.36% → 15.06%, Eligible 22.23% → 25.66%) — though restoring
// smV_T+transpose_v adds a sync + extra SMEM traffic that may erase part of the
// gain. Honest A/B vs v96 with correct kernel is the question this answers.
//
...
// 6 warps × 1 m16 tile × 16 rows = Br=96. 12 warps/SM at 2 blocks (same as v102 LB=3).
// Goal: less SMEM bank contention than v102's 3-blocks-per-SM with same warp count.
#define FA_BR 96
#define FA_BC 64
#define FA_THREADS 192   // 6 warps × M_TILES=1 × 16 rows = Br=96
#define FA_STRIDE 128
#define M_TILES 1        // each warp owns 1 m16 sub-tile (was 2)
        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    int mrb = wid * 16;  // M_TILES=1 → each warp owns 16 rows. 6 warps × 16 = Br=96

    extern __shared__ uint8_t raw[];
    uint8_t *smQ = raw;
    // v106: smV_T restored for correctness. SMEM = smQ(12) + smK(8) + smV(8) + smV_T(8.7) = 36.7 KB.
...
#pragma unroll
    ...
    float rmax[M_TILES][2], rsexp[M_TILES][2];
    #pragma unroll
    for (int _mi = 0; _mi < M_TILES; _mi++) {
        rmax[_mi][0] = -1e30f; rmax[_mi][1] = -1e30f;
        rsexp[_mi][0] = 0.0f;  rsexp[_mi][1] = 0.0f;
    }
    ...
    // v101: pre-load K[kv_min] and V[kv_min]. Single-K: K always in smK, V always in smV.
    ...
    {
        int kvs = kv * FA_BC;
        uint8_t *cur_K = smK;  // v101 single-K: always smK

        // Wait until prev iter's K[kv] and V[kv] cp.async completed.
        cpa_wait<0>();
        __syncthreads();

        // v106: transpose_v RESTORED for correctness. smV (K-major) → smV_T (N-major).
        // PV B-operand reads use smV_T (uint32 = 4 consecutive k for fixed n).
        transpose_v(smV_T, smV, head_dim);
        __syncthreads();

        // v101: smP MUST be different from smV (V[kv+1] cp.async writes smV).
        // Reuse smQ (dead after Qr register pre-load before kv-loop).
        uint8_t *smP = smQ;  // (not actually used in v106 since v105 P-in-regs killed smP STS)

        // v101: K cp.async DEFERRED to AFTER QK done (line ~390). With single-K,
        // K[kv+1] would overwrite K[kv] currently being read by QK. So K cp.async
        // moves to after QK ends → K[kv+1] only in flight during softmax + PV
        // (~50% of iter time vs v96's full-iter overlap).
        // K rows_p kept here for branch-free pattern below.
        int kv_p = kv + 1;
...
#pragma unroll
        ...
        // v101: BOTH K[kv+1] and V[kv+1] cp.async issued AFTER QK done.
        // K[kv+1] → smK (overwrites K[kv] — safe, QK finished).
        // V[kv+1] → smV (smV dead since transpose_v copied to smV_T at iter start).
        // Both overlap with softmax + smP STS + PV MMA (~50% of iter time).
        int rows_v = (kv + 1 < kv_max_blocks) ? FA_BC : 0;
        load_tile_fp8(smK, Kh, (kv + 1) * FA_BC, rows_v, seq_len, head_dim);
        load_tile_fp8(smV, Vh, (kv + 1) * FA_BC, rows_v, seq_len, head_dim);
        cpa_commit();
...
#pragma unroll
        ...
        float ns[M_TILES][2];
        #pragma unroll
        for (int _mi = 0; _mi < M_TILES; _mi++) { ns[_mi][0] = 0.0f; ns[_mi][1] = 0.0f; }
...
#pragma unroll
        ...
        // v105 P-in-regs (ported from v89): skip smP STS + 2 __syncthreads entirely.
        // Pack P_top/P_bot fp8x2 per thread into Pf_pair[nt][mi]: low16=fp8x2_top,
        // high16=fp8x2_bot. PV ks=0/1 will gather Pr via uniform-nt __shfl_sync.
...
#pragma unroll
        ...
        // v105 P-in-regs: Pr0 gather via __shfl_sync from Pf_pair[0..3] (no smP).
        // === PV ks=0 batch ===
        {
            int cl = tid * 4, ch = cl + 16;
            uint32_t Pr0[M_TILES][4];
            uint32_t src_low  = gid_lane_base | ((tid & 1) << 1);       // 0,2,0,2
...
#pragma unroll
            for (int mi = 0; mi < M_TILES; mi++)
            {
                ...
                Pr0[mi][0] = (low_a & 0xFFFF) | ((high_a & 0xFFFF) << 16);
                Pr0[mi][1] = (low_a >> 16) | ((high_a >> 16) << 16);
...
#pragma unroll
            ...
            {
                int br = nt * 8;
                // v106: read from smV_T (N-major) via padded swizzle for correctness.
...
// Runs ONE warmup + ONE measured launch of fa106_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    // v106 Br=96 SMEM: smQ + smK + smV + smV_T = 12K + 8K + 8K + 8.7K = 36.7K/block.
    int smem = FA_BR * FA_STRIDE + FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
             + hd * SMV_T_STRIDE;
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa106_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa106_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v106 hd=128 Br=96 + 6 warps + M_TILES=1 + P-in-regs + smV_T (correctness fix) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa106_kernel));
    printf("Kernel attrs: numRegs=%d  binSize=%d  sharedSizeBytes(static)=%zu\n",
           attr.numRegs, attr.binaryVersion, attr.sharedSizeBytes);
    int smem_hd128 = FA_BR * FA_STRIDE + FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                   + 128 * SMV_T_STRIDE;
    int smem_v96   = 128 * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                   + 128 * SMV_T_STRIDE;
    printf("v106 Br=96 +smV_T SMEM (hd=128): %d B (%.2f KB) vs v96 Br=128=%d B (delta=%+d B)\n",
           smem_hd128, smem_hd128 / 1024.0, smem_v96, smem_hd128 - smem_v96);
    printf("2 blocks × %.2f KB = %.2f KB %s\n\n",
           smem_hd128 / 1024.0, 2 * smem_hd128 / 1024.0,
           (2 * smem_hd128 <= 102400) ? "≤ 100 KB cap (2 blocks SMEM-fits)" : "> 100 KB cap (BLOCKED)");
    ...
    for (auto &c : configs)
    {
        ...
        // v106 Br=96 single-K SMEM: smQ + smK + smV + smV_T
        // = 12K + 8K + 8K + 8.7K = 36.7K/block. 2 blocks × 36.7 = 73.4 KB ≤ 100 KB cap.
        int smem = FA_BR * FA_STRIDE + FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa106_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa106_kernel<<<bh * nqt, FA_THREADS, smem>>>(
