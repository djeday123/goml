...
#pragma unroll
    ...
    // v88 step 0 probe: Or_p moved from registers to static SMEM.
    // 32 uint32 per thread × 128 threads = 16 KB static smem (added to dynamic ~24 KB).
    // Per-thread storage: smOr_p[threadIdx.x * 32 + nt*4 + mi*2 + r]
    __shared__ uint32_t smOr_p[FA_THREADS * N_TILES_PV * M_TILES * 2];
    auto OR_P = [&](int nt, int mi, int r) -> uint32_t& {
        return smOr_p[threadIdx.x * (N_TILES_PV * M_TILES * 2) + nt * (M_TILES * 2) + mi * 2 + r];
    };
#pragma unroll
...
#pragma unroll
            for (int t = 0; t < N_TILES_PV; t++)
            {
                uint32_t r0 = OR_P(t, mi, 0), r1 = OR_P(t, mi, 1);
                __half2 v0 = *reinterpret_cast<__half2 *>(&r0);
                __half2 v1 = *reinterpret_cast<__half2 *>(&r1);
...
#pragma unroll
                for (int mi = 0; mi < M_TILES; mi++)
                {
                    // v88 probe: load Or_p from SMEM → MMA → store back
                    uint32_t or0 = OR_P(nt, mi, 0);
                    uint32_t or1 = OR_P(nt, mi, 1);
                    mma_fp8_f16(or0, or1,
...
#pragma unroll
        for (int nt = 0; nt < N_TILES_PV; nt++)
        {
            ...
            uint32_t r0 = OR_P(nt, mi, 0), r1 = OR_P(nt, mi, 1);
            __half2 v0 = *reinterpret_cast<__half2 *>(&r0);
            __half2 v1 = *reinterpret_cast<__half2 *>(&r1);
...
// Runs ONE warmup + ONE measured launch of fa88_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa88_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        // warmup (NCu skips with --launch-skip)
        fa88_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        // measured (NCu profiles this one)
        fa88_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa88_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa88_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa88_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v88_step0 hd=64 — Or_p in SMEM probe (4-block feasibility) ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa88_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa88_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        fa88_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
        ...
        CK(cudaFuncSetAttribute(fa88_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa88_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa88_kernel<2>);
            r3[v] = measure(fa88_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
