...
// v80c: Br=96 (3 consumer warps × M_TILES=2 × 16 rows = 96)
// warp 0 = producer (Phase 3+), warps 1-3 = consumers. Phase 1: warp 0 idle on compute.
#define FA_BR 96
#define FA_BC 64
#define FA_THREADS 128   // 4 warps: 1 producer + 3 consumers
#define FA_STRIDE 128
#define M_TILES 2        // each consumer warp owns 2 m16 sub-tiles
...
// v80c Phase 3: producer-warp-only cp.async loader. Uses only 32 threads of warp 0.
// Each producer thread does 4× more work than load_tile_fp8 (since 32 vs 128 threads).
__device__ __forceinline__ void load_tile_fp8_warp0(
    uint8_t *dst, const uint8_t *src, int start, int rows,
    int seq_len, int head_dim)
{
    constexpr int CHUNK = 16;
        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    // v80c: wid=0 is producer (Phase 3+); wids 1-3 are consumers. compute_wid = wid-1.
    // mrb is only meaningful for consumer warps (skipped on producer side).
    int compute_wid = wid - 1;            // 0..2 for consumers, -1 for producer (unused)
    int mrb = compute_wid * 32;            // M_TILES=2 × 16 = 32 rows per consumer warp
    ...
    // v80c Phase 2: mbarrier infrastructure. Allocate but don't use yet.
    // Phase 3 will use these for producer/consumer signaling.
    // Init count = 32 (one warp) so each warp can arrive/wait as a unit.
    __shared__ cuda::barrier<cuda::thread_scope_block> bar_K_ready, bar_V_ready;
    __shared__ cuda::barrier<cuda::thread_scope_block> bar_K_free, bar_V_free;
    if (threadIdx.x == 0) {
        init(&bar_K_ready, FA_THREADS);  // Phase 2: full-block count. Phase 3 will adjust.
        init(&bar_V_ready, FA_THREADS);
        init(&bar_K_free, FA_THREADS);
        init(&bar_V_free, FA_THREADS);
    }
    __syncthreads();

    // Qr[ks][mi][r] — 4 k-steps × 2 M-tiles × 4 uint32 (m16k32 A operand)
    // v80c: only consumer warps load Qr (producer doesn't compute).
...
#pragma unroll
    ...
    // v80c Phase 3: Pre-load K + V — PRODUCER WARP ONLY (warp 0).
    if (wid == 0)
    {
        load_tile_fp8_warp0(smK[kv_min_blocks & 1], Kh, kv_min_blocks * FA_BC, FA_BC, seq_len, head_dim);
        load_tile_fp8_warp0(smV, Vh, kv_min_blocks * FA_BC, FA_BC, seq_len, head_dim);
    }
    cpa_commit();
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v80c Phase 3: K cp.async PRODUCER WARP ONLY.
        int kv_p = kv + 1;
        int rows_p = (kv_p < kv_max_blocks) ? FA_BC : 0;
        if (wid == 0)
        {
            load_tile_fp8_warp0(nxt_K, Kh, kv_p * FA_BC, rows_p, seq_len, head_dim);
        }
        cpa_commit();

        // S = Q · Kᵀ — K B-operand loaded once per (nt, ks), reused across mi.
        // v80c: Sr_p declared outside if-block so it's accessible in both QK MMA and softmax sections.
...
#pragma unroll
        ...
        // v80c Phase 1: QK MMA — consumer warps only.
        if (wid > 0)
        {
#pragma unroll
            for (int ks = 0; ks < 4; ks++)
...
#pragma unroll
        ...
        // v80c Phase 3: V cp.async PRODUCER WARP ONLY (mid-iter, into smK[buf] alias).
        int rows_v = (kv + 1 < kv_max_blocks) ? FA_BC : 0;
        if (wid == 0)
        {
            load_tile_fp8_warp0(smK[buf], Vh, (kv + 1) * FA_BC, rows_v, seq_len, head_dim);
        }
        prev_V_slot = smK[buf];
        cpa_commit();

        // v80c Phase 1: softmax + P_top/P_bot computation — consumer warps only.
...
#pragma unroll
        ...
        // v80c: P_top/P_bot moved OUTSIDE this if-block (declared above for cross-block access).
...
#pragma unroll
        ...
        }  // end of `if (wid > 0)` softmax block (v80c Phase 1)

        __syncthreads();

        // v80c Phase 1: smP STS — consumer warps only.
        if (wid > 0)
        {
...
#pragma unroll
        ...
        }  // end of `if (wid > 0)` smP STS block
        __syncthreads();

        // v80c Phase 1: PV MMA — consumer warps only.
        if (wid > 0)
        {
...
#pragma unroll
        ...
        }  // end of `if (wid > 0)` PV MMA block (v80c Phase 1)
    ...
    // v80c Phase 1: Output writeback — consumer warps only.
    if (wid > 0)
    {
#pragma unroll
    for (int mi = 0; mi < M_TILES; mi++)
...
#pragma unroll
    ...
    }  // end of `if (wid > 0)` output writeback block (v80c Phase 1)
}
...
int main()
{
    ...
    printf("=== FA v80c FP8 forward — REAL warp spec MVP (Br=96) — v69 + V[N+1] mid-iter prefetch into dead smK[buf] ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa80c_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa80c_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa80c_kernel<<<bh * nqt, FA_THREADS, smem>>>(
