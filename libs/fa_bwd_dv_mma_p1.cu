// =====================================================================
//  fa_bwd_dv_mma_p1.cu — B2.2 P1 axis: cp.async cooperative loads.
//
//  Change vs B2.1 baseline (fa_bwd_dv_mma.cu):
//      - Cooperative tile loads (K warmup + Q/dO per qt) converted from
//        synchronous LDG.E + STS.U8/U16 pairs to cp.async.cg.shared.global
//        16-byte chunks (cpa16 kirpich from fa_bwd_common.cuh).
//      - Single-buffer (no double buffer → no SMEM delta → 2 blocks/SM held).
//      - Immediate cp.async.wait_group(0) before each __syncthreads.
//      - L sync (256 B total, OOB-cleaner with direct LDG+STS).
//      - smP_T write STS NOT touched (compute output, not staging).
//
//  Goal: isolate the cp.async MECHANISM effect at fixed occupancy.
//  Expected: 10-25% wall improvement (no qt→qt overlap; that's P3 axis).
//  Mio_throttle 18% wall (path β LDS.U16) untouched — separate axis.
//
//  Validation: 11 forms + canary sl=300 wnd=96 via fa_bwd_dv_mma_p1_test.cu
//  (clone of fa_bwd_dv_mma_test.cu). FP8 floor must NOT shift (shift = race).
// =====================================================================
    ...
    float scale)
{
    ...
    // Causal-aware KV-skip (mirror dK Pass 2): skip qt < kt (tile fully masked).
    ...
    for (int qt = qt_start; qt < n_qt; ++qt) {
        ...
        // ===== step A: cp.async Q + dO; sync L =====
        {
            ...
            for (int c = tid; c < dO_total; c += FA_DV_MMA_THREADS) {
                ...
                int i_g      = qt_base + i_local;
                int dO_xor   = (i_local & 7) << 4;    // smdO-swizzle (byte space, 256B stride → 16 chunks; XOR keeps in-range)
        ...
        for (int ni = 0; ni < NI_QK; ++ni) {
            ...
            // smPT-swizzle (element-space, row stride Br=64 elems = 128 bytes):
            // (j_local_lo & 7) = 2*l_mod4  →  XOR = (2*l_mod4) << 3 = l_mod4 << 4
            // (j_local_hi & 7) = 2*l_mod4 + 1  →  XOR = XOR_even + 8
            const int PT_xor_even_wr = l_mod4 << 4;
            const int PT_xor_odd_wr  = PT_xor_even_wr + 8;
            smPT[j_local_lo * Br + (i_local_lo ^ PT_xor_even_wr)] = h_p00;
            smPT[j_local_hi * Br + (i_local_lo ^ PT_xor_odd_wr)]  = h_p01;
            smPT[j_local_lo * Br + (i_local_hi ^ PT_xor_even_wr)] = h_p10;
            smPT[j_local_hi * Br + (i_local_hi ^ PT_xor_odd_wr)]  = h_p11;
        }
        ...
        for (int kb = 0; kb < KB_DV; ++kb) {
            ...
            // smPT-swizzle read: (m_lo&7) = (m_hi&7) = l_div4 → XOR = l_div4 << 3 (element-space)
            const int PT_xor_rd = l_div4 << 3;
            uint32_t Ar0 = *reinterpret_cast<uint32_t*>(&smPT[m_lo * Br + (k_lo ^ PT_xor_rd)]);
            uint32_t Ar1 = *reinterpret_cast<uint32_t*>(&smPT[m_hi * Br + (k_lo ^ PT_xor_rd)]);
            uint32_t Ar2 = *reinterpret_cast<uint32_t*>(&smPT[m_lo * Br + (k_hi ^ PT_xor_rd)]);
            uint32_t Ar3 = *reinterpret_cast<uint32_t*>(&smPT[m_hi * Br + (k_hi ^ PT_xor_rd)]);
            ...
            for (int ni = 0; ni < NI_DV; ++ni) {
                ...
                // smdO-swizzle read: XOR n with ((row & 7) << 3) element-space (row stride 256B).
                // (kA0&7) = (kB0&7) = 2*l_mod4 → XOR = l_mod4<<4 (even-parity rows)
                // (kA1&7) = (kB1&7) = 2*l_mod4+1 → XOR = (l_mod4<<4)+8 (odd-parity rows)
                const int dO_xor_even = l_mod4 << 4;
                const int dO_xor_odd  = dO_xor_even + 8;
                uint16_t lo0 = *reinterpret_cast<uint16_t*>(&smdO[kA0 * Hd + (n ^ dO_xor_even)]);
                uint16_t hi0 = *reinterpret_cast<uint16_t*>(&smdO[kA1 * Hd + (n ^ dO_xor_odd)]);
                uint16_t lo1 = *reinterpret_cast<uint16_t*>(&smdO[kB0 * Hd + (n ^ dO_xor_even)]);
                uint16_t hi1 = *reinterpret_cast<uint16_t*>(&smdO[kB1 * Hd + (n ^ dO_xor_odd)]);
...
{
    if (hd != FA_DV_MMA_HD) {
        fprintf(stderr, "fa_bwd_dv_mma_p1: hd=%d, expected %d\n", hd, FA_DV_MMA_HD);
        exit(1);
    }
    ...
    kernel_dv_mma_p1<<<grid, FA_DV_MMA_THREADS, smem_bytes, stream>>>(
