// flash_attention_v44_bench.cu — v44 (exp2f) vs v20 (__expf)
...
{
    printf("--- Correctness (v44 vs v1-scalar) ---\n");
    struct
    {
        int h, s, d;
    } cfgs[] = {
    ...
    for (auto &c : cfgs)
    {
        ...
        flash_attention_v44_forward(dQ, dK, dV, dO, c.h, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());
...
void bench()
{
    printf("\n--- Performance: v20 (__expf) vs v44 (exp2f+ffma) ---\n");
    printf("%-12s %10s %10s  %6s  %6s\n", "Config", "v20", "v44", "ratio", "peak%%");
    printf("---------------------------------------------------------\n");
    ...
    for (auto &c : cfgs)
    {
        ...
        for (int i = 0; i < 3; i++)
            flash_attention_v44_forward(dQ, dK, dV, dO, heads, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());
        t.start();
        for (int i = 0; i < it; i++)
            flash_attention_v44_forward(dQ, dK, dV, dO, heads, c.s, c.d, 1, nullptr);
        float ms44 = t.stop();
        CK(cudaGetLastError());
        double t44 = flops / (ms44 / it / 1000.0) / 1e12;

        printf("%-12s %8.2f T %8.2f T  %5.2fx  %5.1f%%\n",
               c.l, t20, t44, t44 / t20, t44 / peak * 100.0);
...
int main()
{
    ...
    printf("=== FlashAttention v44 (exp2f + ffma fusion) ===\n");
