// flash_attention_v8_bench.cu — v8 (persistent) vs v7
...
void bench()
{
    printf("\n--- Performance: v7 vs v8 (persistent, causal FLOPS ÷ 2) ---\n");
    printf("%-12s %10s %10s  %6s  %6s\n", "Config", "v7", "v8-pk", "v8/v7", "%%peak");
    printf("----------------------------------------------------------\n");
    ...
    for (auto &c : cfgs)
    {
        ...
        float ms8 = t.stop();
        CK(cudaGetLastError());
        double t8 = flops / (ms8 / it / 1000.0) / 1e12;

        printf("%-12s %8.2f T %8.2f T  %5.2fx  %5.1f%%\n",
               c.l, t7, t8, t8 / t7, t8 / peak * 100.0);
...
int main()
{
    ...
    printf("=== FlashAttention v8 (Persistent Kernel) Benchmark ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, p.clockRate / 1000);
    printf("FP16 peak: 165.2 TFLOPS\n\n");
    test_correctness();
    bench();
    return 0;
}
