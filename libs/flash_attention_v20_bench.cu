...
{
    printf("--- Correctness (v20 vs v1-scalar) ---\n");
    struct
    {
        int h, s, d;
    } cfgs[] = {
    ...
    for (auto &c : cfgs)
    {
        ...
        flash_attention_v20_forward(dQ, dK, dV, dT, c.h, c.s, c.d, 1, nullptr);
        CK(cudaDeviceSynchronize());
...
void bench()
{
    printf("\n--- Performance: v7 vs v20 (reg-based S/P) ---\n");
    printf("%-12s %10s %10s  %6s  %6s\n", "Config", "v7", "v20", "ratio", "peak%");
    printf("---------------------------------------------------------\n");
...
int main()
{
    ...
    printf("=== FlashAttention v20 (Reg-based S/P, 4 warps) ===\n");
