    ...
    for (int qt = qt_start; qt < n_qt; ++qt) {
        ...
        for (int kb = 0; kb < KB_DK; ++kb) {
            ...
            for (int np = 0; np < NI_DK / 2; ++np) {
                ...
                // 071 x1-variant: replace x2 with 4 x1 calls (2 per lo/hi covering a+b).
                // Probe 071 shows x1.trans.b8 on sm_120a returns 2 regs where R1 = dup R0 (hw quirk).
                // Semantic addr for b requires a bridge (unknown offset) — placeholder +8 for ptxas discriminator only.
                int row_lo = kb * 32 + lane;
                int addr_lo_a = swz_byte(row_lo, np * 16);
                int addr_lo_b = swz_byte(row_lo, np * 16 + 8);
                uint32_t sm_addr_lo_a = __cvta_generic_to_shared(&smQ[addr_lo_a]);
                uint32_t sm_addr_lo_b = __cvta_generic_to_shared(&smQ[addr_lo_b]);
                ...
                uint32_t B0b_lo, Dlo_b_dup;
                asm volatile("ldmatrix.sync.aligned.m16n16.x1.trans.shared.b8 {%0,%1},[%2];\n"
                    : "=r"(B0b_lo), "=r"(Dlo_b_dup) : "r"(sm_addr_lo_b));

                int row_hi = kb * 32 + (lane & 15) + 16;
                int addr_hi_a = swz_byte(row_hi, np * 16);
                int addr_hi_b = swz_byte(row_hi, np * 16 + 8);
                uint32_t sm_addr_hi_a = __cvta_generic_to_shared(&smQ[addr_hi_a]);
                uint32_t sm_addr_hi_b = __cvta_generic_to_shared(&smQ[addr_hi_b]);
                uint32_t B0a_hi, Dhi_a_dup;
                asm volatile("ldmatrix.sync.aligned.m16n16.x1.trans.shared.b8 {%0,%1},[%2];\n"
                    : "=r"(B0a_hi), "=r"(Dhi_a_dup) : "r"(sm_addr_hi_a));
                uint32_t B0b_hi, Dhi_b_dup;
                asm volatile("ldmatrix.sync.aligned.m16n16.x1.trans.shared.b8 {%0,%1},[%2];\n"
                    : "=r"(B0b_hi), "=r"(Dhi_b_dup) : "r"(sm_addr_hi_b));
