...
#pragma unroll
        ...
        // v93 cool-temp probe: gid_lane_base/src_low/src_high REMOVED.
        // Each shfl_sync recomputes lane source inline. If ptxas regs drop ~3-5
        // → cool slots were real → 4-block plan viable. If unchanged → CSE'd back
        // → mirage (compiler always wins). Correctness identical (same arithmetic).
        ...
        // === PV ks=0 batch ===
        {
            ...
            // v93 probe: src_low/src_high INLINED — no caching across shfl_sync calls.
#pragma unroll
            for (int mi = 0; mi < M_TILES; mi++)
            {
                // Pr0[mi][0]/[1] from cols cl..cl+3 of rows g0/g8 → nt=0 or 1
                uint32_t p_nt0_l = __shfl_sync(0xffffffff, Pf_pair[0][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1));
                uint32_t p_nt0_h = __shfl_sync(0xffffffff, Pf_pair[0][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1) | 1);
                uint32_t p_nt1_l = __shfl_sync(0xffffffff, Pf_pair[1][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1));
                uint32_t p_nt1_h = __shfl_sync(0xffffffff, Pf_pair[1][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1) | 1);
                ...
                uint32_t p_nt2_l = __shfl_sync(0xffffffff, Pf_pair[2][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1));
                uint32_t p_nt2_h = __shfl_sync(0xffffffff, Pf_pair[2][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1) | 1);
                uint32_t p_nt3_l = __shfl_sync(0xffffffff, Pf_pair[3][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1));
                uint32_t p_nt3_h = __shfl_sync(0xffffffff, Pf_pair[3][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1) | 1);
...
#pragma unroll
        ...
        // === PV ks=1 batch ===
        {
            ...
            // v93 probe: src_low/src_high INLINED.
#pragma unroll
            for (int mi = 0; mi < M_TILES; mi++)
            {
                uint32_t p_nt4_l = __shfl_sync(0xffffffff, Pf_pair[4][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1));
                uint32_t p_nt4_h = __shfl_sync(0xffffffff, Pf_pair[4][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1) | 1);
                uint32_t p_nt5_l = __shfl_sync(0xffffffff, Pf_pair[5][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1));
                uint32_t p_nt5_h = __shfl_sync(0xffffffff, Pf_pair[5][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1) | 1);
                ...
                uint32_t p_nt6_l = __shfl_sync(0xffffffff, Pf_pair[6][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1));
                uint32_t p_nt6_h = __shfl_sync(0xffffffff, Pf_pair[6][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1) | 1);
                uint32_t p_nt7_l = __shfl_sync(0xffffffff, Pf_pair[7][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1));
                uint32_t p_nt7_h = __shfl_sync(0xffffffff, Pf_pair[7][mi], (threadIdx.x & 0x1c) | ((tid & 1) << 1) | 1);
...
// Runs ONE warmup + ONE measured launch of fa93_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa93_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        // warmup (NCu skips with --launch-skip)
        fa93_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        // measured (NCu profiles this one)
        fa93_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa93_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa93_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa93_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v93 cool-temp probe (gid_lane_base/src_low/src_high INLINED) ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa93_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa93_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        fa93_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
        ...
        CK(cudaFuncSetAttribute(fa93_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa93_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa93_kernel<2>);
            r3[v] = measure(fa93_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
