...
#define FA_BR 128
#define FA_BC 64
#define FA_THREADS 256   // v69_s1m1: 8 warps × M_TILES=1 × 16 rows = Br=128 (was 128/4/2)
#define FA_STRIDE 128
#define M_TILES 1        // v69_s1m1: 1 m16 per warp (was 2). Halves per-lane regs.
...
// v69_s1m1: launch_bounds = 1 block/SM. With FA_THREADS=256, 2 blocks/SM hint
// caps ptxas at 128 regs/thread → massive spill. Allow 255 regs with 1 block.
// Per-SM occupancy = 8 warps × 1 block = 16.67% (2× v68's 8.33%).
        ...
        float qk_descale, float v_descale)
{
    ...
    int mrb = wid * 16;  // v69_s1m1: M_TILES=1 → each warp owns 16 rows (was 32)
...
#pragma unroll
    ...
    // v69_s1m1: M_TILES=1 → only 1 inner-row init pair (was 2 for M_TILES=2).
...
#pragma unroll
        ...
        float ns[M_TILES][2]; ns[0][0] = 0.f; ns[0][1] = 0.f;  // v69_s1m1: M_TILES=1
...
int main()
{
    ...
    printf("=== FA v69_s1m1 — Stage 1 on M_TILES=1 base (8 warps, halve regs) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa69_s1m1_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        CK(cudaFuncSetAttribute(fa69_s1m1_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa69_s1m1_kernel<<<bh * nqt, FA_THREADS, smem>>>(
