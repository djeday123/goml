// =============================================================================
// FlashAttention v65 — FP8 Forward + Bigger tiles (Br=128, 256 threads)
// =============================================================================
// Doubles Br from 64 to 128, threads from 128 to 256 (4→8 warps per block).
// Per-SM occupancy unchanged: was 2 blocks × 4 warps = 8 warps/SM, now
// 1 block × 8 warps = 8 warps/SM.
//
// The key win: V transpose + P quantize overhead is paid PER BLOCK. With
// 2× Q rows per block, each per-row amortizes those fixed costs across
// 2× more MMAs.
//
// SMEM layout:
//   smQ:   128 × 128 = 16 KB
//   smK0,1: 64 × 128 = 8 KB each, 16 KB total
//   smV0,1: 64 × 128 = 8 KB each, 16 KB total
//   smV_T: 128 × 64 = 8 KB
//   smP overlaps cur_V (saved 16 KB by reuse)
//   Total: 56 KB
//
...
#pragma unroll
        ...
        // Quantize P → smP. smP uses bc-stride (64) since P has only Bc cols.
        // Storage: [Br × Bc] = [128 × 64] = 8 KB, fits in cur_V's freed 8 KB.
#pragma unroll
        for (int nt = 0; nt < 8; nt++)
        {
...
#pragma unroll
        for (int ks = 0; ks < 2; ks++)
        {
            ...
            int g0 = mrb + gid, g8 = g0 + 8;
            uint32_t Pr[4];
            Pr[0] = *(uint32_t *)&smP[swz_byte_bc(g0, cl)];
            Pr[1] = *(uint32_t *)&smP[swz_byte_bc(g8, cl)];
            Pr[2] = *(uint32_t *)&smP[swz_byte_bc(g0, ch)];
            Pr[3] = *(uint32_t *)&smP[swz_byte_bc(g8, ch)];
#pragma unroll
...
int main()
{
    ...
    printf("=== FA v65 FP8 forward — bigger tiles (Br=128, 256 threads) ===\n");
    ...
    for (auto &c : configs)
    {
        ...
        CK(cudaFuncSetAttribute(fa65_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa65_kernel<<<bh * nqt, FA_THREADS, smem>>>(
