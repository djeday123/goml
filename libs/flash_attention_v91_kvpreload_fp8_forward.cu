...
#pragma unroll
        ...
        // === QK ks=0 batch with K-preload to regs ===
        // v90: Pre-load all 8 nt's b0/b1 from cur_K into 16 registers BEFORE MMA loop.
        // Targets short_scoreboard 5.5% by decoupling SMEM read latency from MMA chain.
        // Cost: +16 regs/thread temporarily, but ptxas may share liveness ranges.
        {
            int cl = tid * 4, ch = cl + 16;
            uint32_t b0_arr[8], b1_arr[8];
#pragma unroll
            for (int nt = 0; nt < 8; nt++) {
                int br = nt * 8;
                b0_arr[nt] = *(uint32_t *)&cur_K[swz_byte(br + gid, cl)];
                b1_arr[nt] = *(uint32_t *)&cur_K[swz_byte(br + gid, ch)];
            }
#pragma unroll
            for (int nt = 0; nt < 8; nt++)
            {
#pragma unroll
                for (int mi = 0; mi < M_TILES; mi++)
                {
                    mma_fp8_f16(Sr_p[nt][mi][0], Sr_p[nt][mi][1],
                                Qr[0][mi][0], Qr[0][mi][1],
                                Qr[0][mi][2], Qr[0][mi][3],
                                b0_arr[nt], b1_arr[nt], Sr_p[nt][mi][0], Sr_p[nt][mi][1]);
                }
            }
        }
        // === QK ks=1 batch with K-preload to regs ===
        {
...
#pragma unroll
            ...
            // v91 V-preload: pre-load all 8 nt's b0/b1 from smV_T into 16 regs
            // before MMA loop. Same idea as K-preload, but for PV smV_T reads.
            uint32_t vb0_arr[N_TILES_PV], vb1_arr[N_TILES_PV];
#pragma unroll
            for (int nt = 0; nt < N_TILES_PV; nt++) {
                int br = nt * 8;
                vb0_arr[nt] = *(uint32_t *)&smV_T[swz_byte_smvt(br + gid, cl)];
                vb1_arr[nt] = *(uint32_t *)&smV_T[swz_byte_smvt(br + gid, ch)];
            }
#pragma unroll
            for (int nt = 0; nt < N_TILES_PV; nt++)
            {
#pragma unroll
                for (int mi = 0; mi < M_TILES; mi++)
                {
                    mma_fp8_f16(Or_p[nt][mi][0], Or_p[nt][mi][1],
                                ...
                                vb0_arr[nt], vb1_arr[nt], Or_p[nt][mi][0], Or_p[nt][mi][1]);
                }
            }
        }
        // === PV ks=1 batch ===
...
// Runs ONE warmup + ONE measured launch of fa91_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa91_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        // warmup (NCu skips with --launch-skip)
        fa91_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        // measured (NCu profiles this one)
        fa91_kernel<2><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa91_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa91_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa91_kernel<3><<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v91 hd=64 + KV combined preload (QK + PV) ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa91_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa91_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        fa91_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
        ...
        CK(cudaFuncSetAttribute(fa91_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa91_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    // -------------------------------------------------------------------------
    printf("\n--- Phase 2: A/B perf — small + large grid coverage ---\n");
    int bench_configs[][4] = {
        // Small/medium reference configs (where 413T was found in v89)
        {16, 4096, 64, 0},   // grid 512 WIN: v89 = 413T
        {32, 2048, 64, 0},   // grid 512 WIN: v89 = 389T
        {8, 8192, 64, 1024}, // grid 512 WIN sliding: v89 = 280T
        // Large grid configs (where 466T peak lives in v89)
        {64, 4096, 64, 0},   // v89 = 462T peak
        {64, 8192, 64, 0},   // v89 = 463T (absolute peak)
        {128, 2048, 64, 0},  // v89 = 436T
        {128, 4096, 64, 0},  // v89 = 455T
        // Mid-grid configs to track regression
        {8, 4096, 64, 0},
        {16, 2048, 64, 0},
    };
    int n_bench = sizeof(bench_configs) / sizeof(bench_configs[0]);
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa91_kernel<2>);
            r3[v] = measure(fa91_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
