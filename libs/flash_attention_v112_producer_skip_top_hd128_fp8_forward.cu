...
#pragma unroll
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // v112: TOP-OF-ITER producer-skip. Producer does cpa_wait + threadfence
        // (memory fence makes its cp.async writes visible to other warps), then
        // RACES AHEAD to mid-iter — no block-wide sync.
        // Consumer threads do cpa_wait (no-op for them) + bar.sync 4, 96
        // (consumer-only cross-warp PV-done sync, fence implicit).
        cpa_wait<0>();
        if (is_producer) {
            __threadfence_block();  // make producer's cp.async writes visible
        } else {
            asm volatile("bar.sync 4, 96;");
        }
...
// Runs ONE warmup + ONE measured launch of fa112_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa112_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa112_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v112 PRODUCER-SKIP top+post-transpose (consumer-only bar.sync 96 both places) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa112_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa112_kernel<<<bh * nqt, FA_THREADS, smem>>>(
