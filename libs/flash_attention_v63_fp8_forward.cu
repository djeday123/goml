// =============================================================================
// FlashAttention v63 — FP8 Forward (optimized over v62)
// =============================================================================
// Fixes v62's four bottlenecks:
//   1. Pre-transpose V on load: cp.async into smV_raw, SMEM-to-SMEM transpose
//      into smV_T. Eliminates per-MMA gather_v_b (was 8 byte-reads/MMA).
//   2. Hardware FP8 cvt via cvt.rn.satfinite.e4m3x2.f16x2 (sm_89+) replaces
//      manual fp16_to_e4m3_d in the inner softmax loop.
//   3. (Deferred to v64) Double-buffered K, V loads.
//   4. Reduced syncs by overlapping P quantize with V transpose work.
//
// Same algorithmic pipeline as v62. Same correctness.
//
        ...
        float qk_descale, float v_descale)
{
    ...
    uint8_t *smV_T = smV + FA_BC * FA_STRIDE;  // [head_dim, Bc] = 128*64 = 8KB
    uint8_t *smP = smV_T + head_dim * FA_BC;
...
#pragma unroll
    ...
    for (int kv = 0; kv < nkv; kv++)
    {
        ...
        // Pre-transpose V → smV_T so the P·V MMA can use the same scalar
        // uint32_t read pattern as K (no per-MMA byte-gather).
...
int main()
{
    ...
    printf("=== FA v63 FP8 forward — optimized (pre-transpose V + hw FP8 cvt) ===\n");
    ...
    for (auto &c : configs)
    {
        ...
        int smem = (FA_BR + 2 * FA_BC + FA_BR) * FA_STRIDE + hd * FA_BC;  // + smV_T
        CK(cudaFuncSetAttribute(fa63_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa63_kernel<<<bh * nqt, FA_THREADS, smem>>>(
