...
__device__ __forceinline__ void cpa16(void *s, const void *g, int n)
{
    ...
    // v99 probe: .cg → .ca (Cache All = use both L1 and L2 vs .cg bypass L1).
    // Hypothesis: maybe L1 caching helps reduce long_scoreboard 1.77% on hd=128 peak.
    // Counter-risk: L1 pollution may slow regular SMEM, OR .ca for 16B may have
    // different completion model. NCu will show actual stall delta.
    asm volatile("cp.async.ca.shared.global [%0],[%1],16,%2;" ::"r"(sa), "l"(g), "r"(n));
}
...
// Runs ONE warmup + ONE measured launch of fa99_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa99_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa99_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v99 hd=128 + cp.async.ca probe (Cache All vs .cg = L1 caching) ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa99_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa99_kernel<<<bh * nqt, FA_THREADS, smem>>>(
