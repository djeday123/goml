        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    uint8_t *smQ = raw;
    // v79d: single-K (8KB) + double-V (16KB). Same total SMEM as v79b (48.5KB).
    uint8_t *smK = smQ + FA_BR * FA_STRIDE;
    uint8_t *smV[2] = {
        smK + FA_BC * FA_STRIDE,
        smK + FA_BC * FA_STRIDE + FA_BC * FA_STRIDE,
    };
...
#pragma unroll
    ...
    // v79d: pre-load K (single slot) + V[0] (first slot of double-V).
    load_tile_fp8(smK, Kh, kv_min_blocks * FA_BC, FA_BC, seq_len, head_dim);
    load_tile_fp8(smV[0], Vh, kv_min_blocks * FA_BC, FA_BC, seq_len, head_dim);
    cpa_commit();

    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        int kvs = kv * FA_BC;
        int v_buf = (kv - kv_min_blocks) & 1;  // alternates V buffer per iter
        int v_nxt = v_buf ^ 1;

        // Wait for K[N] (smK) and V[N] (smV[v_buf]).
        cpa_wait<0>();
        __syncthreads();

        // Read V[N] from smV[v_buf], transpose to smV_T.
        transpose_v(smV_T, smV[v_buf], head_dim);
        __syncthreads();

        uint8_t *smP = smV[v_buf];  // smP reuses current V slot (now dead after transpose)

        // v79d: K cp.async stays mid-iter (after QK MMA, smK is dead). See line after QK.
...
#pragma unroll
        ...
        // v79d: After QK MMA, smK is dead (mma.sync operands committed). Issue BOTH:
        //   K[N+1] cp.async → smK (overwrites K[N], no more reads)
        //   V[N+1] cp.async → smV[v_nxt] (free slot, transpose already consumed smV[v_buf])
        // Both overlap with softmax + smP STS + PV MMA (~60% iter).
        int kv_p = kv + 1;
        int rows_p = (kv_p < kv_max_blocks) ? FA_BC : 0;
        load_tile_fp8(smK, Kh, kv_p * FA_BC, rows_p, seq_len, head_dim);
        load_tile_fp8(smV[v_nxt], Vh, kv_p * FA_BC, rows_p, seq_len, head_dim);
        cpa_commit();
...
int main()
{
    ...
    printf("=== FA v79d FP8 forward — single-K + double-V — v69 + V[N+1] mid-iter prefetch into dead smK[buf] ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa79d_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa79d_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa79d_kernel<<<bh * nqt, FA_THREADS, smem>>>(
