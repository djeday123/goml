...
#pragma unroll
    ...
    for (int kv = kv_min_blocks; kv < kv_max_blocks; kv++)
    {
        ...
        // Prefetch K[kv+1] → nxt_K. Overlaps with compute.
...
#pragma unroll
        ...
        // with the entire softmax + smP STS + PV MMA path (~80% of iter time).
        // smK[buf] is safe to overwrite here: QK MMA fragment loads have committed,
        // and next iter's cur_K = smK[buf^1] (different slot, has K[kv+1] from above).
        // The mid-iter cp.async puts V[kv+1] where next iter's transpose_v will find it.
        if (kv + 1 < kv_max_blocks)
        {
            load_tile_fp8(smK[buf], Vh, (kv + 1) * FA_BC, FA_BC, seq_len, head_dim);
            prev_V_slot = smK[buf];
        }
        cpa_commit();
...
#pragma unroll
        ...
        // v78: V prefetch was MOVED to after QK MMA (line 318 area) — no longer here.
        // smV is no longer needed for V storage after iter kv_min (smK[buf] holds V[kv+1]
        // for next iter's transpose). smV remains valid as smP scratchpad.
    }
...
int main()
{
    ...
    printf("=== FA v78 FP8 forward — v69 + V[N+1] mid-iter prefetch into dead smK[buf] ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa78_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int smem = FA_BR * FA_STRIDE + 2 * FA_BC * FA_STRIDE + FA_BC * FA_STRIDE
                 + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa78_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa78_kernel<<<bh * nqt, FA_THREADS, smem>>>(
