// =============================================================================
// FlashAttention v82 — FP8 Forward, hd=64 + TMA-Q (Q-only TMA descriptor)
// =============================================================================
// v81 base + replace cp.async.cg Q-load with cp.async.bulk.tensor.2d (TMA).
// Q is loaded once per block before kv loop, so single-lane TMA dispatch cost
// is paid ONCE (unlike v67 which used TMA per kv-iter and lost to 128-thread
// parallel cp.async). Q's address-arith IMAD/ULEA chains are encapsulated in
// the tensormap descriptor.
//
// Two measurable signals:
//   (1) numRegs at LB=3 — does it drop from 168? Target ≤ 127 opens 4 blocks/SM
//       (SMEM 24.25KB × 4 = 97KB ≤ 100KB cap).
//   (2) No Eligible % in WIN config — does latency exposure drop from 43% (v81)?
//
// smQ layout change: TMA writes row-major plain (no manual swizzle). Qr register
// load uses direct addressing `smQ[g*FA_HD + col]`. After Qr loaded, smP overlap
// proceeds as v81 (smQ dead → smP swizzled writes).
//
...
// v82: TMA-Q. q_tmap is encoded once on host (2D tensormap over global Q
// [bh×seq_len, FA_HD]). Single-thread bulk.tensor dispatch per block.
template <int LB>
        ...
        int window)  // 0 = no window (full attention or causal). >0 = sliding window.
{
    ...
    // v82: TMA bulk.tensor requires 128-byte aligned destination in SMEM.
    // Default extern __shared__ alignment is 16 — explicitly bump to 128.
    ...
    // ===== v82 TMA-Q =====
    // Single-thread bulk.tensor dispatch loads FA_BR × FA_HD bytes of Q.
    // Coordinates into 2D tensormap [bh*seq_len, FA_HD]: row = bh*seq_len + qs, col = 0.
    // mbarrier signals tx completion. ALL threads wait via try_wait.parity.
    __shared__ alignas(8) uint64_t q_bar;
    constexpr int Q_TILE_BYTES = FA_BR * FA_HD;  // 128 × 64 = 8192 bytes

    if (threadIdx.x == 0) {
        uint32_t bar_s  = __cvta_generic_to_shared(&q_bar);
        uint32_t smq_s  = __cvta_generic_to_shared(smQ);
        int row_coord = bh * seq_len + qs;
        asm volatile("mbarrier.init.shared.b64 [%0], 1;" :: "r"(bar_s));
        asm volatile("mbarrier.arrive.expect_tx.shared.b64 _, [%0], %1;"
                     :: "r"(bar_s), "n"(Q_TILE_BYTES));
        asm volatile(
            "cp.async.bulk.tensor.2d.shared::cluster.global.tile.mbarrier::complete_tx::bytes "
            "[%0], [%1, {%2, %3}], [%4];"
            :: "r"(smq_s), "l"(&q_tmap), "r"(0), "r"(row_coord), "r"(bar_s));
    }
    // ALL threads wait for Q tile to land.
    asm volatile(
        "{ .reg .pred P;\n"
        "  WAITQ: mbarrier.try_wait.parity.shared.b64 P, [%0], 0;\n"
        "  @P bra DONEQ;\n"
        "  bra WAITQ;\n"
        "  DONEQ: }\n"
        :: "r"((uint32_t)__cvta_generic_to_shared(&q_bar)));
    __syncthreads();

    // Qr[ks][mi][r] — KS_QK k-steps × 2 M-tiles × 4 uint32 (m16k32 A operand)
    // v82: TMA wrote smQ row-major plain (no manual swizzle). Use direct addressing.
...
#pragma unroll
        for (int mi = 0; mi < M_TILES; mi++)
        {
            ...
            Qr[ks][mi][0] = *(uint32_t *)&smQ[g0 * FA_HD + cl];
            Qr[ks][mi][1] = *(uint32_t *)&smQ[g8 * FA_HD + cl];
            Qr[ks][mi][2] = *(uint32_t *)&smQ[g0 * FA_HD + ch];
            Qr[ks][mi][3] = *(uint32_t *)&smQ[g8 * FA_HD + ch];
        }
    }
...
// v82: encode 2D tensormap over Q [bh*seq_len, FA_HD]. One tmap per (Q_base, bh, sl).
// Reused across all blocks in a kernel launch (each block picks its own row coord).
static void encode_q_tmap(CUtensorMap *tmap, uint8_t *Q_base, int bh, int seq_len, int hd)
{
    cuuint64_t globalDim[2]    = {(cuuint64_t)hd, (cuuint64_t)bh * seq_len};
    cuuint64_t globalStride[1] = {(cuuint64_t)hd};  // bytes between rows (uint8_t)
    cuuint32_t boxDim[2]       = {(cuuint32_t)hd, (cuuint32_t)FA_BR};
    cuuint32_t elementStride[2]= {1, 1};
    CUresult res = cuTensorMapEncodeTiled(
        tmap, CU_TENSOR_MAP_DATA_TYPE_UINT8, 2,
        Q_base, globalDim, globalStride, boxDim, elementStride,
        CU_TENSOR_MAP_INTERLEAVE_NONE,
        CU_TENSOR_MAP_SWIZZLE_NONE,   // smQ store is row-major plain
...
// Runs ONE warmup + ONE measured launch of fa82_kernel<LB> on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx, int LB)
{
    ...
    CUtensorMap q_tmap;
    encode_q_tmap(&q_tmap, Q_d, bh, sl, hd);

    if (LB == 2) {
        CK(cudaFuncSetAttribute(fa82_kernel<2>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa82_kernel<2><<<grid, FA_THREADS, smem>>>(q_tmap, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa82_kernel<2><<<grid, FA_THREADS, smem>>>(q_tmap, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    } else {
        CK(cudaFuncSetAttribute(fa82_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        fa82_kernel<3><<<grid, FA_THREADS, smem>>>(q_tmap, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        fa82_kernel<3><<<grid, FA_THREADS, smem>>>(q_tmap, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
    }
    CK(cudaDeviceSynchronize());
...
int main(int argc, char **argv)
{
    ...
    printf("=== FA v82 hd=64 + TMA-Q — reg/perf delta vs v81 ===\n");
    ...
    CK(cudaFuncGetAttributes(&a2, fa82_kernel<2>));
    CK(cudaFuncGetAttributes(&a3, fa82_kernel<3>));
    printf("--- Kernel attrs ---\n");
    ...
    for (auto &c : configs)
    {
        ...
        // v82: encode tmap once per config (covers all heads via row coord).
        CUtensorMap q_tmap;
        encode_q_tmap(&q_tmap, Q_d, bh, sl, hd);
        ...
        fa82_kernel<2><<<bh * nqt, FA_THREADS, smem>>>(
            q_tmap, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        ...
        CK(cudaFuncSetAttribute(fa82_kernel<3>,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        CK(cudaMemset(O_d, 0, n_elems * 2));
        fa82_kernel<3><<<bh * nqt, FA_THREADS, smem>>>(
            q_tmap, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
        CK(cudaDeviceSynchronize());
        CK(cudaMemcpy(O_cpu, O_d, n_elems * 2, cudaMemcpyDeviceToHost));
    ...
    for (int ci = 0; ci < n_bench; ci++)
    {
        ...
        // v82: tmap encoded once per config (Q_d is stable for this config).
        CUtensorMap q_tmap;
        encode_q_tmap(&q_tmap, Q_d, bh, sl, hd);
        ...
        auto measure = [&](auto kernel_ptr) -> double {
            for (int i = 0; i < WARMUP; i++)
                kernel_ptr<<<grid, FA_THREADS, smem>>>(q_tmap, K_d, V_d, O_d, sl, hd, ca_bench, scale, 1.0f, 1.0f, wnd);
            CK(cudaDeviceSynchronize());
            cudaEventRecord(t0);
        ...
        for (int v = 0; v < VARIANCE_RUNS; v++) {
            r2[v] = measure(fa82_kernel<2>);
            r3[v] = measure(fa82_kernel<3>);
        }
        // sort each
        auto sort3 = [](double *a) {
