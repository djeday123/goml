// =============================================================================
// FlashAttention v56 — Backward pass, Step 1 perf tuning over v55
//
// Changes vs v55:
//   1. Vectorized writeback: pack adjacent FP16 col pairs into __half2 (32-bit)
//      stores, halving the number of global stores in the dQ/dK/dV output loop.
//   2. __launch_bounds__(128, 1) — at 96KB SMEM/block sm_120 only fits 1
//      block/SM anyway; explicit (.,1) gives the compiler more register budget.
//   3. Cleaned up the unused Pass-1 nqt warning.
//
// Targets sm_80+ (works on Ampere/Ada/Hopper/Blackwell). Same fat-binary
// build conventions as the rest of libs/.
//
// Standalone test: nvcc -O3 -arch=sm_120 flash_attention_v56_backward.cu -o fa_v56_bw
...
extern "C" void launch_v56_backward_dq(
    ...
    int th, int sl, int hd, int ca)
{
    ...
    if (smem > g_smem_dq)
    {
        cudaFuncSetAttribute(fa56_backward_dq_kernel,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
        g_smem_dq = smem;
    }

    dim3 grid(th, nqt);
    fa56_backward_dq_kernel<<<grid, FA_THREADS, smem>>>(
...
extern "C" void launch_v56_backward_dkdv(
    const __half *Q, const __half *K, const __half *V,
    const __half *dO, const float *LSE, const float *D,
    ...
    int th, int sl, int hd, int ca)
{
    ...
    if (smem > g_smem_dkdv)
    {
        cudaFuncSetAttribute(fa56_backward_dkdv_kernel,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
        g_smem_dkdv = smem;
    }

    dim3 grid(th, nkv);
    fa56_backward_dkdv_kernel<<<grid, FA_THREADS, smem>>>(
...
extern "C" void launch_v56_backward(
    const __half *Q, const __half *K, const __half *V,
    ...
    int th, int sl, int hd, int ca)
{
    ...
    launch_v56_backward_dq(Q, K, V, dO, LSE, D_scratch, dQ_out, th, sl, hd, ca);
    launch_v56_backward_dkdv(Q, K, V, dO, LSE, D_scratch, dK_out, dV_out, th, sl, hd, ca);
}
...
void test_correctness()
{
    ...
    for (auto &c : configs)
    {
        ...
        launch_v56_backward_dq(Q_d, K_d, V_d, dO_d, LSE_d, D_d, dQ_d, th, sl, hd, ca);
        launch_v56_backward_dkdv(Q_d, K_d, V_d, dO_d, LSE_d, D_d, dK_d, dV_d, th, sl, hd, ca);
...
int main()
{
    ...
    printf("=== FlashAttention v56 backward — vectorized writeback + (.,1) launch ===\n");
