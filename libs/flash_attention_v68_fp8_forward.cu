// =============================================================================
// FlashAttention v68 — FP8 Forward + smV_T padding to break 32-way conflict
// =============================================================================
// v66 had 91.67% shared-store bank conflict rate (NCu source-attributed to
// 4 STS at +0xc000+ = smV_T writes in transpose_v). Math: lane stride =
// 4*64 = 256 B in smV_T (= 2× bank period 128 B) → all 32 lanes hit bank 0.
//
// Fix (Variant C, diagnostic filter): pad smV_T row stride 64 → 68.
//   gcd(68/4, 32) = gcd(17, 32) = 1 → 32 lanes cover 32 distinct banks.
// Only smV_T uses padded stride; smQ/smK/smV/smP keep original layout.
//
// SMEM cost: +FA_BR*4 = +512 B → total 57344 → 57856 B. Still 1 block/SM
// (occupancy unchanged at 8.33%). C is the conflict-fix-only branch; if it
// delivers near NCu-predicted +40%, A-aggressive (drop smV_T entirely +
// register-transpose P + warp shuffles for V) is justified for additional
// occupancy 2× win. If C yields <10%, conflict wasn't dominant stall and
// A-aggressive isn't worth the risk.
//
...
int main()
{
    ...
    printf("=== FA v68 FP8 forward — Variant C: smV_T stride 64→68 ===\n");
    printf("GPU: %s (%d SMs, %d MHz)\n", p.name, p.multiProcessorCount, clk / 1000);

    // Report actual kernel attributes (real SMEM use + reg count).
    cudaFuncAttributes attr;
    CK(cudaFuncGetAttributes(&attr, fa68_kernel));
    printf("Kernel attrs: numRegs=%d  binSize=%d  sharedSizeBytes(static)=%zu\n",
           attr.numRegs, attr.binaryVersion, attr.sharedSizeBytes);
    int smem_hd128 = (FA_BR + 2 * FA_BC + 2 * FA_BC) * FA_STRIDE + 128 * SMV_T_STRIDE;
    int smem_v66   = (FA_BR + 2 * FA_BC + 2 * FA_BC) * FA_STRIDE + 128 * FA_BC;
    printf("Dynamic SMEM (hd=128): v68=%d B (%.2f KB) vs v66=%d B (delta=%+d B)\n\n",
           smem_hd128, smem_hd128 / 1024.0, smem_v66, smem_hd128 - smem_v66);
    ...
    for (auto &c : configs)
    {
        ...
        // v68: smV_T size is hd * SMV_T_STRIDE (was hd * FA_BC).
        // For hd=128: 128*68=8704 bytes (was 128*64=8192), +512 B.
        int smem = (FA_BR + 2 * FA_BC + 2 * FA_BC) * FA_STRIDE + hd * SMV_T_STRIDE;
        CK(cudaFuncSetAttribute(fa68_kernel,
                                cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa68_kernel<<<bh * nqt, FA_THREADS, smem>>>(
