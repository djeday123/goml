...
// v116 word-position-rotation swizzle for smV_T (stride STAYS = 68).
// Mathematical closure of 4-way write bank conflict in transpose_v_consumer:
//   At STRIDE=68, lane_stride = 4*STRIDE = 272 B, bank_step per-lane = STRIDE mod 32 = 4.
//   gcd(4,32)=4 → 4-way conflict (NCu: 25.2M excessive/STS line confirmed).
//   Conflicting lanes are l vs l+8 vs l+16 vs l+24 (period 8 in lane = period 32 in row).
//   Adding f(row)*4 to address with f(row)=(row>>5)&3 maps {row, row+32, row+64, row+96}
//   to {0,1,2,3} → breaks the period-8 collision. Implementation: rotate the 4 uint32
//   word positions within each 16-byte chunk by word_rot. SMEM size unchanged, alignment
//   preserved, addr-arithmetic +2 ALU ops.
// Math verification:
//   Writes (STS #0, row=4l, col=0): bank = (4l + (l/8)&3) mod 32.
//     l=0..7: banks {0,4,8,12,16,20,24,28}
//     l=8..15: banks {1,5,9,13,17,21,25,29}
//     l=16..23: banks {2,6,10,14,18,22,26,30}
//     l=24..31: banks {3,7,11,15,19,23,27,31}
//     ALL 32 unique → 0-way conflict.
//   Reads (PV nt-loop, row=br+gid): word_rot uniform across 32 lanes per nt (gid<8 doesn't
//     affect bits 5..6 of row). Bijection over banks per nt → conflict count preserved
//     (7 banks × 2-way per LDS, same as current, but rotated across banks each nt).
__device__ __forceinline__ int swz_byte_smvt(int row, int col_bytes)
{
    int chunk = col_bytes >> 4;
    int within = col_bytes & 15;
    int word_pos = within >> 2;         // 0..3: uint32-word index within 16-B chunk
    int byte_off = within & 3;          // 0..3: byte within uint32 (always 0 at call sites)
    int word_rot = (row >> 5) & 3;      // f(row): distinguishes row bits 5..6
    int word_pos_new = (word_pos + word_rot) & 3;
    return row * SMV_T_STRIDE + ((chunk ^ (row & 3)) << 4) + (word_pos_new << 2) + byte_off;
}
...
// Runs ONE warmup + ONE measured launch of fa116_kernel on bench_configs[cfg_idx].
...
static int ncu_mode(int cfg_idx)
{
    ...
    fprintf(stderr, "NCu config: bh=%d sl=%d hd=%d wnd=%d grid=%d\n",
            bh, sl, hd, wnd, grid);
    CK(cudaFuncSetAttribute(fa116_kernel,
                            cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    fa116_kernel<<<grid, FA_THREADS, smem>>>(Q_d, K_d, V_d, O_d, sl, hd, ca, scale, 1.0f, 1.0f, wnd);
...
int main(int argc, char **argv)
{
    ...
    CK(cudaFuncGetAttributes(&attr, fa116_kernel));
    ...
    for (auto &c : configs)
    {
        ...
        int nqt = (sl + FA_BR - 1) / FA_BR;
        float scale = 1.0f / sqrtf((float)hd);
        fa116_kernel<<<bh * nqt, FA_THREADS, smem>>>(
